...
2. Writes `.env` with: `FRONTEND_URL`, `BACKEND_URL`, `POSTGRES_USER/PASSWORD/DB/PORT`, `JWT_SECRET` (generated), `LOCAL_SECRETS_ENCRYPTION_KEY` (generated), `VITE_API_BASE_URL`, `VITE_BACKEND_REDIRECT_URL`, `VITE_SOCKET_URL`, `NODE_ENV=development`, and `TERSE_IMAGE` pinned to the create-terse release.
...
- **Image pull fails**: check `TERSE_IMAGE` in `.env`. If the user is on a restricted network, they may need to allowlist `us-central1-docker.pkg.dev` or mirror the image.
