...
Settle shared state separately from run durability. If several runs or workflows need to mutate the same logical entity, and those mutations must be serialized, use a durable actor. Use workflow `states` for values owned by one workflow when read-modify-write races are not a concern. An actor does not make a job durable, and a durable job does not make an actor call an implicit step. Follow the "Durable actors" conventions and https://docs.useterse.ai/core-concepts/actors.
...
- **Shared serialized state** — define a named `Actor` subclass in `src/durable-objects.ts` and address it with a stable domain ID. In a durable job, call it through a module-scope helper wrapped in `step()`.
...
`terse test` loads actor definitions from the latest successful deployment. If `src/durable-objects.ts` is new or has changed since that deployment, use the typechecker for pre-deploy verification and defer the actor behavior test until after the user approves deployment. The post-deploy test uses an isolated `test.<projectId>` actor namespace, not production actor state.
...
- Actors: `src/durable-objects.ts` has only named direct `Actor` subclasses; IDs are stable; state is plain JSON; new fields handle old snapshots; durable handlers journal actor calls through module-scope helpers; `outcome_unknown` mutations are not blindly retried.
...
- If the user says yes, run `terse deploy` and report the outcome. When this build added or changed an actor, run the planned `terse test run` after a successful deployment and report the result; it exercises the deployed actor definition in the isolated test namespace.
