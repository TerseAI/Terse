...
Before running the numbered process, silently judge how much of it this build actually needs. Do not ask the user which path to take and do not announce the choice — just take the right one.

Take the **fast path** only when all four hold:

1. **Unambiguous intent** — the request already pins down what should happen, on which events, and for whom; there is nothing an interview would resolve.
2. **No new integrations** — every integration the job needs is already connected (present under `src/terse.generated/`); nothing requires `terse integrate connect`, a new secret, or a `terse generate`.
3. **Non-durable** — no wait for human input or time, and fewer than three side-effecting milestones.
4. **Trivial shape** — the whole job is one or two deterministic `toolbox` calls, or a single narrow `generateText`.

If any one fails, run the full process (steps 0–12).

**On the fast path you still keep the safety floor.** Ensure a project exists (step 0), read the generated files as the source of truth and never invent constants, point side effects at test targets per the Testing Safety Conventions, prove the job green (a `tsc --noEmit` plus one `terse test run`), and ask before deploying (step 12). What you skip is the ceremony — the interview, the researchers, the shared-understanding diagram, and the milestone slicing — not the verification or the deploy gate. Write the job directly, verify it, and hand it back.

**Escalate back to the full process** the moment the build turns out non-trivial — a needed integration isn't connected, a wait or approval appears, three or more side effects pile up, or the intent proves ambiguous. Drop onto the full process from step 1 rather than forcing the shortcut.
...
When the user gives a schedule in their local time, keep it in their timezone: `Triggers.schedule.cron` takes an optional `timezone` (a typed IANA name like `"America/New_York"`, autocompleted from the `Timezone` union) and evaluates the expression as wall-clock time in that zone, DST-aware. Do not convert the user's time to UTC by hand; omitting `timezone` means UTC.
