...
- **Event type**: Is the event typed with the correct class for the language? Use `GitHubPullRequestTriggerEvent` in TypeScript, or the matching `terse_sdk` event class in Python.
- **Type guards**: When handling events from multiple trigger types, use `isGitHubPullRequestTriggerEvent()`, `isGitHubPushTriggerEvent()`, etc.
...
import { GitHubPullRequestTriggerEvent, isGitHubPullRequestTriggerEvent } from "terse-sdk"

onTrigger: async (event: GitHubPullRequestTriggerEvent, Agent: TerseAgent) => {
    if (!isGitHubPullRequestTriggerEvent(event)) return
