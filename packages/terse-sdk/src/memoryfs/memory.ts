import { MEMORY_ROOT, formatHumanSize, resolveRelativePath } from "./paths.js"
import type { LocalFsStore } from "./store.js"
...
export type MemoryCommandResult = { success: true; message: string }
...
function resolveMemoryPath(inputPath: string): string {
    ...
    return resolveRelativePath(pathPart)
}
...
async function listDirectoryView(store: LocalFsStore, relativePath: string, displayPath: string): Promise<string> {
    const entries = await store.list(relativePath)
    const lines = [`Here're the files and directories up to 2 levels deep in ${displayPath}, excluding hidden items and node_modules:`]

    const rootStat = relativePath ? await store.stat(relativePath) : null
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory) {
            const children = await store.list(entry.path)
...
async function viewFile(store: LocalFsStore, relativePath: string, displayPath: string, viewRange?: [number, number] | null): Promise<string> {
    const content = await store.read(relativePath)
...
export async function executeMemoryCommand(store: LocalFsStore, input: MemoryCommandInput): Promise<MemoryCommandResult> {
    switch (input.command) {
        case "view": {
            const relativePath = resolveMemoryPathOptional(input.path)
            const displayPath = memoryDisplayPath(relativePath)
            const exists = relativePath === "" ? true : await store.exists(relativePath)
            if (!exists) {
                return { success: true, message: `The path ${displayPath} does not exist. Please provide a valid path.` }
            }

            const stat = relativePath === "" ? { isDirectory: true } : await store.stat(relativePath)
            if (stat.isDirectory) {
                return { success: true, message: await listDirectoryView(store, relativePath, displayPath) }
            }
            return { success: true, message: await viewFile(store, relativePath, displayPath, input.view_range ?? undefined) }
        }
        case "create": {
            ...
            await store.write(relativePath, input.file_text)
        ...
        case "str_replace": {
            ...
            const stat = await store.stat(relativePath)
            if (stat.isDirectory) {
                return { success: true, message: `Error: The path ${displayPath} does not exist. Please provide a valid path.` }
            }
            ...
            await store.write(relativePath, updated)
        ...
        case "insert": {
            ...
            await store.write(relativePath, lines.join("\n"))
        ...
        case "delete": {
            ...
            await store.deletePath(relativePath)
...
export const MUTATING_MEMORY_COMMANDS: ReadonlySet<MemoryCommand> = new Set(["create", "str_replace", "insert", "delete", "rename"])
