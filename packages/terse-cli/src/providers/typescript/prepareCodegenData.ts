...
function toolNameToInterfaceName(name: string): string {
    return name.split("_").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join("") + "Params"
}

function buildResourceClassContext(
    className: string,
    ...
    const constructorParams = fields
        .map(field => `public readonly ${field.classField}: ${field.type}`)
        .join(", ")
    ...
    for (const item of items) {
        ...
        const argsText = fields.map(field => {
...
function prepareGmailSection(instances: IntegrationInstanceData[], tools: ToolDefinition[]): SectionContext<GmailSectionContext> {
    if (instances.length === 0) return sectionData([])
    return sectionData(
        ["GmailConfig", "GmailOutputConfig", "GmailDraftOutputConfig", "TypedSkill", "GmailEventType"],
        {
            id: instances[0].id,
...
function prepareAtlassianSection(
    instances: AtlassianInstanceData[],
    tools: ToolDefinition[]
): SectionContext<AtlassianSectionContext> {
    if (instances.length === 0) return sectionData([])
    const inst = instances[0]
...
function prepareDatadogSection(instances: DatadogInstanceData[], tools: ToolDefinition[]): SectionContext<DatadogSectionContext> {
    ...
    return sectionData(["DatadogConfig", "TypedSkill"], {
        ...
        indexClass: buildResourceClassContext(
            "DatadogIndex",
            [{ classField: "name", type: "string", sourceField: "name" }],
            "name",
            inst.indexes
...
function prepareLaunchDarklySection(
    instances: LaunchDarklyInstanceData[],
    tools: ToolDefinition[]
): SectionContext<LaunchDarklySectionContext> {
    if (instances.length === 0) return sectionData([])
    const inst = instances[0]
...
function buildGeneratedAttioObjects(
    instances: AttioInstanceData[]
): Array<AttioObjectContext & { attributes: Array<AttioAttributeData & { api_slug: string }> }> {
...
function prepareToolsSection(tools: ToolDefinition[], input: CodegenInput): SectionContext<ToolsSectionContext> {
    ...
    instanceMap.set("slack", input.slack.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("github", input.github.map(inst => ({ id: inst.integration.id, displayName: inst.integration.account_name || "" })))
    instanceMap.set("gmail", input.gmail.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("figma", input.figma.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("linear", input.linear.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("jira", input.atlassian.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("confluence", input.atlassian.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("atlassian", input.atlassian.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("notion", input.notion.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("posthog", input.posthog.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("datadog", input.datadog.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("launchdarkly", input.launchdarkly.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("workos", input.workos.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("attio", input.attio.map(inst => ({ id: inst.id, displayName: inst.displayName })))
    instanceMap.set("snowflake", input.snowflake.map(inst => ({ id: inst.id, displayName: inst.name })))
    ...
    if (tools.some(isAttioTool)) {
        ...
        attioPreludeLines.push('type __AttioFilterValue<T> = __AttioFilterShorthand<T> | { $eq?: __AttioFilterShorthand<T>; $contains?: string; $starts_with?: string; $ends_with?: string } | Record<string, unknown>')
        attioPreludeLines.push('type __AttioFilterExpression<TValues extends Record<string, unknown>> = Partial<{ [K in keyof TValues]: __AttioFilterValue<TValues[K]> }> & { $and?: Array<__AttioFilterExpression<TValues>>; $or?: Array<__AttioFilterExpression<TValues>> }')
        ...
        if (attioGeneratedObjects.length > 0) {
            ...
            for (const object of attioGeneratedObjects) {
                const inputShape = object.attributes.length === 0
                    ? 'Record<string, __AttioValue>'
                    : renderAttioObjectValueShape(object.attributes, "input")
            ...
            for (const object of attioGeneratedObjects) {
                const recordShape = object.attributes.length === 0
                    ? 'Record<string, __AttioValue>'
                    : renderAttioObjectValueShape(object.attributes, "record")
        ...
        attioPreludeLines.push('')
        attioPreludeLines.push('export type AttioValuesFor<TObject extends GeneratedAttioObject = GeneratedAttioObject> = TObject extends { __inputValues: infer TInputValues } ? TInputValues : AttioInputValuesByObject[TObject["apiSlug"]]')
        attioPreludeLines.push('export type AttioRecordValuesFor<TObject extends GeneratedAttioObject = GeneratedAttioObject> = TObject extends { __recordValues: infer TRecordValues } ? TRecordValues : AttioRecordValuesByObject[TObject["apiSlug"]]')
        ...
        attioPreludeLines.push('export type AttioQueryRecordsParams<TObject extends GeneratedAttioObject = GeneratedAttioObject> = { object: TObject; filter?: AttioFilterFor<TObject> | null; limit?: number | null }')
        attioPreludeLines.push('export type AttioUpsertRecordParams<TObject extends GeneratedAttioObject = GeneratedAttioObject> = { object: TObject; matchingAttribute: AttioAttributeSlug<TObject>; records: AttioValuesFor<TObject>[] }')
        attioPreludeLines.push('export type AttioListObjectsParams = Record<string, never>')
        attioPreludeLines.push('export type AttioQueryRecordsResult<TObject extends GeneratedAttioObject = GeneratedAttioObject> = Omit<ToolOutputByName["attio_query_records"], "records"> & { records: Array<AttioRecordFor<TObject>> }')
        attioPreludeLines.push('export type AttioUpsertRecordResult<TObject extends GeneratedAttioObject = GeneratedAttioObject> = Omit<ToolOutputByName["attio_upsert_record"], "records"> & { records?: Array<AttioRecordFor<TObject>> }')
        attioPreludeLines.push('')
        ...
        attioPreludeLines.push('}')
        attioPreludeLines.push('')
        attioPreludeLines.push('function __enhanceAttioRecord<TObject extends GeneratedAttioObject>(object: TObject, record: unknown): __AttioRecordWithValues<AttioRecordValuesFor<TObject>> | undefined {')
        ...
        attioPreludeLines.push('}')
        attioPreludeLines.push('')
        attioPreludeLines.push('function __enhanceAttioQueryResult<TObject extends GeneratedAttioObject>(object: TObject, result: ToolOutputByName["attio_query_records"]): AttioQueryRecordsResult<TObject> {')
        ...
        attioPreludeLines.push('}')
        attioPreludeLines.push('')
        attioPreludeLines.push('function __enhanceAttioUpsertResult<TObject extends GeneratedAttioObject>(object: TObject, result: ToolOutputByName["attio_upsert_record"]): AttioUpsertRecordResult<TObject> {')
        attioPreludeLines.push('    return {')
        attioPreludeLines.push('        ...result,')
        attioPreludeLines.push('        records: (result.records || []).map(record => __enhanceAttioRecord(object, record)).filter(Boolean) as Array<AttioRecordFor<TObject>>,')
        attioPreludeLines.push('    }')
    ...
    if (rawGroups.some(group => group.integration === "github")) {
        const githubRepoFullNames = Array.from(
            new Set(
                input.github.flatMap(inst =>
                    inst.repositories
                        .map(repo => (repo.owner && repo.name ? `${repo.owner}/${repo.name}` : repo.name).trim())
                        .filter(Boolean)
                )
            )
        )
