...
import { EXTERNAL_INTEGRATION_TYPES, type IntegrationType, TriggerDefinitions } from "terse-types"
...
function buildLeafSpecs(context: TemplateContext): LeafSpec[] {
    ...
    for (const type of EXTERNAL_INTEGRATION_TYPES) {
        ...
        if (hasLeafTemplate(`${type}.schemas`)) {
            specs.push({ fileName: `${type}.schemas.ts`, templateFile: `${type}.schemas`, kind: "schemas", integration: type })
        }
        specs.push({ fileName: `${type}.tools.ts`, templateFile: `${type}.tools`, kind: "tools", integration: type })
        if (INTEGRATIONS_WITH_TRIGGERS.has(type)) {
            specs.push({ fileName: `${type}.triggers.ts`, templateFile: `${type}.triggers`, kind: "triggers", integration: type })
        }
    }
    return specs
}
...
function hasLeafTemplate(templateFile: string): boolean {
    return fs.existsSync(path.join(getTemplatesRoot(), "typescript", "codegen", "files", `${templateFile}.hbs`))
}
