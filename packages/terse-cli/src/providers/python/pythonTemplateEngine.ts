...
import { pyListRepr, pyRepr, type PythonTemplateContext } from "./preparePythonCodegenData.js"
...
function getHandlebars(): typeof Handlebars {
    ...
    handlebars.registerHelper("pyListRepr", (values: unknown) =>
        Array.isArray(values) ? pyListRepr(values.map(value => String(value))) : "[]"
    )
...
function getCompiledTemplate(): Handlebars.TemplateDelegate<PythonTemplateContext> {
    if (!template) {
        ...
        template = handlebars.compile<PythonTemplateContext>(
            readTemplateFile("python/codegen/generated.hbs"),
            { noEscape: true }
        )
    }
    return template
