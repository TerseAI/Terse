...
function generateWorkOSSection(instances: IntegrationInstanceData[]): SectionResult {
    ...
    parts.push(`    trigger(opts?: { eventTypes?: WorkOSEventType[] }): TypedTrigger<WorkOSInputEvent | WorkOSUserInputEvent | WorkOSMembershipInputEvent | WorkOSInvitationInputEvent> {`)
    parts.push(`        return new WorkOSInputConfig("${id}", opts?.eventTypes) as TypedTrigger<WorkOSInputEvent | WorkOSUserInputEvent | WorkOSMembershipInputEvent | WorkOSInvitationInputEvent>`)
    parts.push("    },")
    parts.push(`    /** Use in \`skills[]\` */`)
