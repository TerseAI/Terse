...
## Input Event Sample Methods Implementation Guide

All input events (triggers) must implement two key methods to support agent testing and sample event functionality:

### Required Methods

1. **`getSampleEvents(config: ConfigInstance): Promise<SampleEvent[]>`**
   - Fetches recent real events from the integration to show users what data is available
   - Should return the last 3 relevant items based on the input type
   - Must return an array of `SampleEvent` objects with proper structure

2. **`addEventToAgent(event: SampleEvent, agentId: string): Promise<void>`**
   - Allows users to send a sample event to their agent for testing
   - Processes the event and triggers the agent's workflow
   - Must handle event transformation and agent invocation

### Implementation by Integration Type

#### Slack Input Events
- **Sample Events**: Return last 3 messages from configured source
- **Sources to Support**:
  - Public channels
  - Private channels
  - Direct messages (DMs)
- **Config Requirement**: Pass the Slack config to properly query the right channel/DM
- **Example**: See `backend/src/inputs/slack/SlackInput.ts`

#### Linear Input Events
- **Sample Events**: Return last 3 tickets/issues created
- **Query**: Fetch most recent issues from the configured Linear workspace/team
- **Data to Include**: Issue title, description, status, assignee

#### Jira Input Events
- **Sample Events**: Return last 3 tickets/issues created
- **Query**: Fetch most recent issues from the configured Jira project
- **Data to Include**: Issue summary, description, status, assignee

#### GitHub Input Events
- **Sample Events**: Return last 3 commits in the repository
- **Query**: Fetch recent commits from the configured GitHub repository
- **Data to Include**: Commit message, author, SHA, timestamp

#### Figma Input Events
- **Sample Events**: Return last 3 comments in the file
- **Query**: Fetch recent comments from the configured Figma file
- **Data to Include**: Comment text, author, timestamp, resolved status

### Implementation Pattern

```typescript
// In your InputEvent class (e.g., SlackInput, LinearInput, etc.)

async getSampleEvents(config: ConfigInstance): Promise<SampleEvent[]> {
    // 1. Get integration credentials from database
    // 2. Initialize API client with credentials
    // 3. Query API for last 3 relevant items based on config
    // 4. Transform API responses into SampleEvent format
    // 5. Return array of sample events
}

async addEventToAgent(event: SampleEvent, agentId: string): Promise<void> {
    // 1. Fetch agent and channel configuration
    // 2. Transform sample event into proper event format
    // 3. Invoke agent with the event
...
The Gmail Input Event serves as the reference implementation:
- Location: `backend/src/inputs/gmail/GmailInput.ts`
- Study the `getSampleEvents()` and `addEventToAgent()` methods
- Follow the same patterns for error handling, data transformation, and agent invocation

### Testing Checklist

After implementing these methods:
- [ ] Verify `getSampleEvents()` returns exactly 3 events (or fewer if less available)
- [ ] Ensure events contain all necessary fields for the agent to process
- [ ] Test `addEventToAgent()` successfully triggers the agent workflow
- [ ] Confirm error handling works for missing integrations or invalid configs
- [ ] Validate that config-specific filtering works (channels, projects, repos, etc.)
