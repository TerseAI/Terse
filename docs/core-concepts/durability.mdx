...
On Terse Cloud, each durable run keeps its journal in a project-scoped durable object. Terse also snapshots the sandbox's filesystem when the run suspends and attempts a snapshot when it fails. Resuming from a snapshot preserves files, installed packages, and Git configuration while completed steps replay from the journal.

If the sandbox disappears before a snapshot can be saved, the journal still survives, but unsnapshotted filesystem changes do not. Retrying without a snapshot starts from the run's original deploy image.
