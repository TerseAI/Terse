...
- **Import `step` from `terse-sdk`.** The CLI only rewrites that binding, so a user-defined `step()` elsewhere is left alone.
...
- `inputSchema` and `outputSchema` are zod schemas that validate the values crossing the durability boundary on every run, which `step()` does not do. `outputSchema` is optional.
...
<Note>
`sleep()`, `step()`, `waitForInput()`, and `jobStep()` are only available in durable jobs. Call them in a non-durable job and you get a clear error asking you to add `durable: true`.
</Note>
