...
Arguments are serialized into the journal like any step input, so log data, not functions. Inside a step body, plain `console.log` is already fine: the body runs once, so its logs print once. `log()` also works in non-durable jobs, where it simply forwards to `console.log`.
...
During local testing with `terse test`, `waitForInput()` shows an interactive terminal prompt instead of posting to Slack. The run does not suspend locally. The prompt runs as a journaled step, so durable replays return your answer from the journal instead of asking again.
