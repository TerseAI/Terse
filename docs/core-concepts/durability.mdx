...
## Making your own calls durable with .asStep()

Anything that is not a Terse SDK call becomes a durable step by chaining `.asStep()` onto it. That covers third-party SDKs like Octokit or Resend, a raw `fetch`, a database write, anything with a side effect.

<Warning>
Code outside a step runs again on **every replay**, not just once. If it has a side effect and is not idempotent (sending an email, charging a card, creating a record), it will happen multiple times and cause real problems. Chain `.asStep()` onto every side effect so it runs exactly once.
</Warning>
...
createJob({
    ...
    onTrigger: async (event) => {
        const { data: contact } = await resend.contacts
            .create({ email: event.user.email, unsubscribed: false })
            .asStep()
...
For a step that is more than one call, write a module-scope function and chain `.asStep()` onto that. This is also the pattern for callback-taking APIs: the callback stays inside the step, and only data crosses.
