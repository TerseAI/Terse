...
During local testing with `terse test`, `waitForInput()` shows an interactive terminal prompt instead of posting to Slack. The run does not suspend locally. The prompt runs as a journaled step, so durable replays return your answer from the journal instead of asking again. The run progress indicator pauses while you answer, the same way it does for tool approvals.
