...
When Terse starts an OAuth flow, it mints a short-lived signed state token that records which Terse user and organization you authenticated as. Optional flow metadata is merged first; your user and organization IDs are written last so they cannot be replaced by extra fields in the state payload.
