...
An integration is a connection to an external service. When you run `terse integrate` or connect through the Terse app, the platform stores your OAuth tokens or API keys securely, scoped to the active Terse organization. The integration itself is just an authenticated link to a service for that org.
