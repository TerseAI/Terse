...
| Type      | Flow                                                        | Examples                                            |
| --------- | ----------------------------------------------------------- | --------------------------------------------------- |
| **OAuth** | Opens your browser for authorization                        | GitHub, Slack, Gmail, Linear, Notion, Attio, WorkOS |
...
| **Controlled by** | `skills` array on `TerseAgent.create()`              | `agent.tools.*` (filtered by `skills`) or `toolbox.*` (unfiltered) |
...
```ts
import { toolbox, Skills, SlackChannel, AttioObject, Triggers } from "./terse.generated"
