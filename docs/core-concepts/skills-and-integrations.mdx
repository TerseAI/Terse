...
- `src/terse_generated.py` for Python
...
def run_job(event, agent: TerseAgent) -> None:
    # The model can use Attio, Apollo, and Slack tools
    summary = agent.run_and_wait("Enrich and summarize this deal.", event)
```
...
def run_job(event, agent: TerseAgent) -> None:
    # Agentic: model can only use Attio (the declared skill)
    summary = agent.run_and_wait("Summarize this week's pipeline activity.", event)
