...
Running `terse generate` writes a single typed file to your project at `src/terse.generated.ts`. Every trigger, skill, deterministic tool wrapper, and real workspace resource (channels, repos, objects, owners) is exported as a typed value. Your workflows import from it. Your coding agent reads it. The compiler enforces it.
...
export class AttioObject<TSlug extends string, TRecordValues, TInputValues> {
    constructor(
    ...
    static Deal = new AttioObject("deals", "deal", "obj_f1e3c0b2…", [/* attributes */])
    static Company = new AttioObject("companies", "company", "obj_8d22a17c…", [/* attributes */])
}
```
...
export const Skills = {
    ...
    slack(opts: { channel: SlackChannel }): TypedSkill<"slack_send_message"> { /* … */ },
    ...
    attio(opts: { object: AttioObject<any> }): TypedSkill<AttioTools> { /* … */ }
}
```
...
`agent.tools.*` is gated by the agent's `skills`: only integrations declared as skills appear on the agent's typed tool object. The same file also exports `toolbox`: identical typed namespaces, but callable **without** a `TerseAgent` and **not** filtered by `skills`. Prefer `toolbox` for workflows that are purely deterministic.

```ts src/terse.generated.ts
export type GeneratedTools = {
    slack: {
        /** Send to a channel, existing DM (`channelId`), or member DM (`slackUserId`). */
        sendMessage(args: SlackSendMessageParams): Promise<SlackSendMessageResult>
        /** Read message history from a Slack channel or DM */
        readConversation(args: SlackReadConversationParams): Promise<SlackReadConversationResult>
    }
    attio: {
        /** Query records on an Attio object */
        queryRecords<T extends GeneratedAttioObject>(args: AttioQueryRecordsParams<T>): Promise<AttioQueryRecordsResult<T>>
        /** Upsert records on an Attio object */
        upsertRecord<T extends GeneratedAttioObject>(args: AttioUpsertRecordParams<T>): Promise<AttioUpsertRecordResult<T>>
    }
    github: {
        /** Search code by semantic meaning */
        searchCode(args: SearchGitHubCodeParams): Promise<SearchGitHubCodeResult>
    }
}
...
createJob({
    ...
    onTrigger: async (event) => {
        const agent = TerseAgent.create({
            prompt: "You summarize PRs and post concise thread replies in Slack.",
        ...
        await agent.runAndWait(`Summarize this PR and reply in thread (ts: ${message.message_ts}).`)
    }
})
```
...
- If an agent invents `SlackChannel.SalesTeam` or `Repos.TerseAI.Website`, the TypeScript compiler rejects the build. `terse deploy` refuses to ship code that doesn't type-check, so a made-up resource has no path to production.
