...
You declare which tools need approval in your job definition. When the model (or your code) calls one of those tools, the run pauses, notifies you, and waits for a decision. Approve to continue and execute the tool. Reject blocks that tool call from running; **Reject** in Slack ends the run as **Cancelled**.
