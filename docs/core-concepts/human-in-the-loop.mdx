...
List the tool names that require approval in `toolApprovals` (TypeScript) or `tool_approvals` (Python):
...
@app.job(
    ...
    tool_approvals=["attio.updateRecord", "slack.sendMessage"],
)
def run_job(event, agent: TerseAgent) -> None:
    # This call runs immediately
    ...
    agent.tools.slack.send_message(
        ...
        message=f"New deal enriched: {event.record.values.company_name}",
    )
```
