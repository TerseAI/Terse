...
```ts
const [video] = (await agent.tools.higgsfield.generateVideo({ prompt, imageUrl })).videos

const review = await waitForInput({
    via: slack({ channel: SlackChannel.Creative.channelId }),
    prompt: "Ship this creative to Meta?",
    media: [{ kind: "video", url: video.url, altText: "Spring promo, 9:16" }],
    options: [
        { id: "ship", label: "Ship it" },
        { id: "revise", label: "Request changes", freeText: true }
    ]
})

if (review.choice === "ship") {
    await agent.tools.metaAds.createAd({ videoUrl: video.url, ... })
}
```
...
<Note>Video uploads need the `files:write` Slack scope. Workspaces that connected Slack before this feature shipped must reconnect the integration from the **Integrations** page to grant it.</Note>
