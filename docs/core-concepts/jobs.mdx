...
In the SDK, you define jobs with `createJob()`. In the Terse app and across these docs, we call them workflows. Same thing, different name.
...
3. **Handler executes.** Your `onTrigger` runs with the event payload and a `TerseAgent`. You control the logic: call tools deterministically, hand off to the model, or both.
...
For jobs that write to production systems, you can require human approval before specific tools execute. List the tool names in `toolApprovals`. During local testing, the CLI prompts in your terminal. In production, approval requests appear in the Terse app under **Notifications**.
