---
title: "Jobs"
description: "A job is the unit of automation in Terse — a trigger, a set of skills, and a handler that runs every time the trigger fires."
icon: "boxes"
---

A job is the core building block in Terse. It connects an event (the trigger) to the code that should run when that event happens (the handler), with a set of integrations available to use along the way (skills). When you deploy a Terse project, you're deploying one or more jobs.

In the SDK, you define jobs with `createJob()` in TypeScript or `@app.job()` in Python. In the Terse app and across these docs, we call them **workflows** — same thing, different name.
...
@app.job(
    name="new-deal-enrichment",
    triggers=[Attio.on_record_created(list=AttioList.Pipeline.NewDeals)],
    skills=[
...
def run_job(event, agent: TerseAgent) -> None:
    # your logic here
    pass
```
...
A unique string identifier. Terse uses this to match the job across deploys — if you rename it, the platform treats it as a new job. Keep names stable and descriptive.
...
The integrations available to the model during agentic calls (`run` and `runAndWait`). Skills don't limit what your code can do with deterministic tool calls — they control what the LLM can reach for when it's reasoning.
...
3. **Handler executes.** Your `onTrigger` (TypeScript) or decorated function (Python) runs with the event payload and a `TerseAgent`. You control the logic: call tools deterministically, hand off to the model, or both.
4. **Run is recorded.** Every execution — success or failure — is logged in Activity with the full action trace, so you can inspect what happened.
...
def run_job(event, agent: TerseAgent) -> None:
    # Deterministic: always enrich
    company = agent.tools.apollo.enrich_company(
        domain=event.record.values.company_domain,
    )

    # Agentic: let the model reason about the data
    summary = agent.run_and_wait(
        f"Summarize this company for the account owner.\n"
    ...
    # Deterministic: always write the result back
    agent.tools.attio.update_record(
        list=AttioList.Pipeline.NewDeals,
...
```python Python
def contract_stage_only(event) -> bool:
    return event.record.values.stage == "contract-sent"

@app.job(
    name="contract-alerts",
    triggers=[Attio.on_record_updated(list=AttioList.Pipeline.OpenDeals)],
    skills=[Slack.skill(channel=SlackChannel.DealDesk)],
    filter=contract_stage_only,
)
def run_job(event, agent: TerseAgent) -> None:
    agent.tools.slack.send_message(
        channel=SlackChannel.DealDesk,
        message=f"Contract sent for {event.record.values.company_name}.",
    )
```
...
For jobs that write to production systems, you can require human approval before specific tools execute. List the tool names in `toolApprovals` (TypeScript) or `tool_approvals` (Python). During local testing, the CLI prompts in your terminal. In production, approval requests appear in the Terse app under **Notifications**.

## Lifecycle

Jobs follow a straightforward path from code to production:

1. **Define** — write the job in `src/index.ts` or `src/main.py`.
