...
2. **Filter evaluates.** If you defined a `filter` function on `createJob`, your TypeScript code runs it with the enriched trigger event. Return `false` to skip the run entirely—no handler, no tokens spent, no side effects. This is deterministic code you control, not a platform-side relevance model.
