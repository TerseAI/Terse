...
In the SDK, you define jobs with `createJob()`. Across these docs, we often call them **workflows** in prose; in the Terse app they appear under each project as **jobs** (for example `https://app.useterse.ai/app/jobs/<id>`). `TerseAgent` is separate: it is the in-handler runtime for agentic loops, not the deployed job itself.

<Note>App URLs moved from `/app/agents/*` to `/app/jobs/*`. Update bookmarks and deep links accordingly.</Note>
