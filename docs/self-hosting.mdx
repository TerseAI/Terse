...
<Steps>
    <Step title="Attach the project">
        ...
        This authenticates, links the repo to a Terse project, writes `terse.config.json` with the self-hosted data plane enabled, and prints `TERSE_API_KEY` and `TERSE_SIGNING_SECRET` once. Save them into your data plane environment before you start the server. See [`terse attach`](/reference/cli#terse-attach) for the full flow.
    </Step>
    ...
    <Step title="Set the required environment variables">
        Your data plane needs two values that `terse attach` prints once. Copy them into your environment before you start the server:
        ...
        | Variable               | Purpose                                                                                                                  |
        | ---------------------- | ------------------------------------------------------------------------------------------------------------------------ |
        | `TERSE_API_KEY`        | Authenticates outbound calls from your data plane to the control plane: opening a session, executing tools, requesting approvals. `terse attach` prints the organization key. The first `terse deploy` may print a project-scoped key — replace the organization key with that when it appears. |
        ...
        <Warning>Treat `TERSE_SIGNING_SECRET` like any other webhook secret. Never log it, never ship it to the client, never commit it. If you lose it, rotate from the dashboard after `remoteServerUrl` is set.</Warning>
