...
<Steps>
    ...
    <Step title="Point Terse at your server">
        ...
        Terse validates this URL when you run `terse deploy`. It must use **HTTPS** and resolve to a **public** IP address. Private, link-local, and metadata addresses are rejected. For local development against `http://localhost` or `127.0.0.1`, use a Terse backend running in development mode — production deploys require HTTPS.
    </Step>
...
| Requirement | Details |
| ----------- | ------- |
| **HTTPS** | Required in production. `http://` is allowed only for `localhost`, `127.0.0.1`, or `::1` when the Terse backend runs in development mode. |
| **Public DNS** | Hostnames must resolve to addresses that are not private or reserved (for example RFC1918, link-local, or cloud metadata ranges). |
