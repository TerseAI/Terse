...
<Steps>
    ...
    <Step title="Deploy your code, then sync workflows">
        Ship your service the way you normally ship Node code (Render, Fly, ECS, your own Kubernetes, …). Use **Node.js 20 or later** on the host (22 recommended). If you mount **`handleTrigger`** with Hono, confirm your `@hono/node-server` version’s engine requirement—current releases expect Node 20+. Once the service is reachable at `remoteServerUrl`, run:
