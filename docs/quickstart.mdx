...
`terse login` during init stores CLI credentials in your **user config**, not in the project folder. Copy **`.env.example`** to **`.env`** for local development. For hosted deploys, upload runtime secrets to Terse with `terse secrets import .env` (or `terse secrets add`) so they are injected into the sandbox — `.env` itself is never uploaded.
