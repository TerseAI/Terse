...
my-project/
  src/
    terse.jobs.ts         # your workflow definitions
...
Open `src/terse.jobs.ts`. The scaffolded starter workflow imports `createJob` and `TerseAgent` from `terse-sdk`, `Triggers` from `./terse.generated`, and `z` from `zod` for structured output in `runAndWait`—it type-checks with `npm run build` right after init. As you connect integrations and run `terse generate`, add trigger builders, skill factories, resource constants, and deterministic tool wrappers from `src/terse.generated.ts`.
