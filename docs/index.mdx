...
await client.createJob({
    ...
    onTrigger: async (event: GithubPRTrigger) => {
        const Agent = TerseAgent.create({
            prompt: "You summarize GitHub pull requests and post updates to Slack.",
            skills: [
...
```python Python
from terse_sdk import CronTrigger, SkillConfig, Terse, TriggerConfig
...
Terse curates the tools available to the LLM based on the skills you pass when you create a `TerseAgent`. Instead of dumping hundreds of MCP tools into context and hoping the model picks the right one, your agent only sees what it needs. Fewer tokens in, more reliable outputs out.
