...
- `CronTrigger` and other typed trigger event models
- `FinalOutput` if you want to stream and inspect final output explicitly

Import these from `terse_generated`:

- `TerseAgent`: the generated subclass with typed `agent.tools`
- `Schedule`, `Attio`, `Slack`, `Snowflake`: generated trigger and skill helpers
...
| `skills`           | `list[SkillConfig]`              | Capabilities available to the model during `run` and `run_and_wait`. |
...
```python
from terse_sdk import FinalOutput
...
```python
from terse_generated import SlackChannel

result = agent.tools.slack.send_message(
    channel=SlackChannel.Alerts,
    message="Deploy finished",
)
```

Use `agent.tools.*` when a generated wrapper exists. These wrappers are available when the integration is present in the workflow's triggers or declared skills. By contrast, `skills` control which integrations the model can use during `run` and `run_and_wait`.
...
- `Schedule.cron(...)`
- `Attio.skill(...)`
- `Slack.on_message(...)`, `Slack.on_dm(...)`, `Slack.trigger(...)`, `Slack.skill(...)`
- `Snowflake.skill()`
- generated `SlackChannel.*` constants
- deterministic wrappers on `agent.tools.attio`
- deterministic wrappers on `agent.tools.slack`
- deterministic wrappers on `agent.tools.snowflake`
- generated `TerseAgent`, `GeneratedTools`, `create_tools()`, and `attach_tools()`

<Warning>Re-run `terse generate` when your integration context changes. Do not edit `src/terse_generated.py` by hand.</Warning>
...
- Slack triggers, skills, event models, channel constants, and deterministic Slack tool wrappers
- Snowflake skills and deterministic Snowflake tool wrappers

Use `agent.tools.slack.*` for direct deterministic Slack calls. Use `skills=[Slack.skill(...)]` when you want Slack to be model-visible during `run` or `run_and_wait`.
...
def weekday_only(event: CronTrigger) -> bool:
