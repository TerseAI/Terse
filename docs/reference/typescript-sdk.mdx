...
Integration trigger types (`GithubPROpenedTrigger`, `SlackMessageTrigger`, `AttioRecordCreatedTrigger`, ...) and tool `Params`/`Result` types are generated into your workspace by `terse generate`; import them from `"./terse.generated"`.
...
Trigger types exported from `./terse.generated` extend the canonical trigger object with two methods used everywhere Terse turns an event into prompts or logging:
