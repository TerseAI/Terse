...
To reply in a thread, pass `thread_ts` with the thread root's `ts` (each send returns the `thread_ts` to use for follow-ups); add `replyBroadcast: true` to also show the reply in the channel. See [Skills & Tools](/reference/skills#slack) for the full parameter list.
