...
- `Terse`, `TERSE_JOB_WEBHOOK_TRIGGER_PATH`: client and mount path for `handleTrigger` when you self-host the webhook endpoint
...
| System trigger event types           | `CronTrigger`, `ManualSampleTrigger`, `WebhookTrigger<BodyType>`                                                                                                                                                                    |
...
| Trigger config helpers               | `AttioInputConfig`, `HeyReachInputConfig`, `AttioEventType`, `HeyReachEventType`                                                                                                                                                    |
...
Project-scoped tokens are minted automatically inside hosted Modal sandboxes. They are limited to runtime endpoints for that project (agent runs, tool execution, approvals, and session streams) and cannot stand in for a user token on organization or integration-management routes. Terse removes the sandbox token when the run completes.
...
- `Triggers`: per-integration trigger builders (when connected), plus `Triggers.schedule`, `Triggers.webhook`, and `Triggers.webMonitor`. `Triggers.attio` and `Triggers.heyReach` appear when those integrations are connected.
- `Skills`: per-integration skill factories (when connected), plus the built-in `Skills.web()` and `Skills.imageEdit()`.
- Workspace resource constants such as `Repos`, `SlackChannel`, `SlackUser`, and the `AttioObject.*` statics generated for your workspace.
