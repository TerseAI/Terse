...
- Session streaming (advanced): `openSessionStream`, `openListenStream`, `SessionStreamError`, and their handle / option / event types. `Terse.handleTrigger` uses this path internally when you self-host; you can import the same helpers for custom tooling that talks to the Terse session SSE endpoints.

<Note>
    Every request that sends **`TERSE_API_KEY`** as a Bearer token is checked against the live user and organization. Unknown, revoked, or stale tokens return **401** with `Invalid API token` or `API token is no longer valid for this organization`. Rotate the key in the Terse app (**Settings**) or fix org membership, then update your environment.
</Note>
