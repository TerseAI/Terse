...
- `openSessionStream`, `openListenStream`, `SessionStreamError`, and related stream types: low-level SSE helpers for session and live-listen flows (the CLI wraps these for `terse test`, `terse deploy`, and `terse listen`).
...
Hosted workflows reach Terse through the API base URL in job context or **`TERSE_BACKEND_URL`**.

| Client | Default when unset |
| ------ | ------------------ |
| `terse-sdk` | `https://app.useterse.ai/api` |
| `terse` CLI | `https://api.useterse.ai` |

Both defaults talk to the same hosted control plane. The web app serves API traffic under **`/api`**, which proxies to **`api.useterse.ai`**. Set **`TERSE_BACKEND_URL`** when you use a preview stack, a local backend, or another Terse deployment.

For self-hosted execution, set **`remoteServerUrl`** in **`terse.config.json`** to the public URL of **your** webhook server—not in `createJob()`.
