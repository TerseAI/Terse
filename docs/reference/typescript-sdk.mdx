...
- **User tokens** are the API tokens you create in the Terse app or get from **`terse auth login`**. The CLI completes a WorkOS device login in your browser, then exchanges the short-lived WorkOS access token for a long-lived Terse API key. Your workflow code and the SDK only ever use the Terse key (`TERSE_API_KEY`), not the WorkOS token. Use user tokens for local runs, the CLI, and self-hosted **`handleTrigger`** servers. They authenticate to the full set of SDK routes your workflow needs (including deploy, codegen, and runtime calls).
