...
- `TriggerEvent`: the base event interface
- Specific event types: `GithubTriggerEvent`, `GitHubPullRequestTriggerEvent`, `CronTriggerEvent`, `ManualSampleTriggerEvent`
...
await client.createJob({
    ...
    onTrigger: async (event: TriggerEvent, agent: TerseAgent) => {
        // workflow logic
    }
})
```
...
await client.createJob({
    ...
    filter: (event: TriggerEvent) => event.record.values.stage === "contract-sent",
    onTrigger: async (event: TriggerEvent, agent: TerseAgent) => {
        await agent.tools.slack.sendMessage({
            channelId: SlackChannel.DealDesk.channelId,
...
| --------------------------- | --------------------------- |
| Attio record events         | `TriggerEvent`              |
| Slack actions               | Typed Slack action payloads |
| Scheduled cron triggers     | `CronTriggerEvent`          |
| Scheduled interval triggers | `CronTriggerEvent`          |
| Manual trigger              | `ManualSampleTriggerEvent`  |

`TriggerEvent` is the common base interface. Use the most specific trigger event type your trigger exposes.
...
```ts
import { Terse, TerseAgent, TriggerEvent } from "terse-sdk"
