...
TypeScript projects from **`terse init`** already depend on **`zod`** (for structured output and schema-driven types such as `WebMonitorTriggerFor`). If you are on an older scaffold or added workflows by hand, add **`zod`** next to **`terse-sdk`**:
...
If you define schemas in workflow code, import them from `zod`. New TypeScript projects created with `terse init` include `zod` by default.
