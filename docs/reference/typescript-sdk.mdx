...
- `openSessionStream`, `openListenStream`, `SessionStreamError`: low-level SSE clients for session and live-listen endpoints (see [Session streams](#session-streams))
- Types: `SessionStreamHandle`, `ListenStreamHandle`, `SessionStreamEvent`, `OpenSessionStreamOptions`, `OpenListenStreamOptions`, `SessionStartedEvent`
...
`handleTrigger` opens a session stream for the duration of your `onTrigger` handler so agent runs, tool calls, and approvals stay tied to one `sessionId`. Call `session.close()` when you are done if you open a stream yourself.

## Session streams

Use **`openSessionStream`** and **`openListenStream`** when you need direct access to Terse SSE endpoints (for example a custom dev harness). Most workflows rely on **`Terse.handleTrigger`**, **`TerseAgent.run`**, or the CLI instead.

| Function | Endpoint | First event | Used by |
| -------- | -------- | ----------- | ------- |
| `openSessionStream(apiBaseUrl, apiKey, options?)` | `GET /sdk/session/events` | `{ type: "session_started", sessionId }` | `Terse.handleTrigger`, `terse test`, `terse deploy` |
| `openListenStream(apiBaseUrl, apiKey, options)` | `GET /sdk/listen?jobName=…&projectId=…` | `{ type: "listen_started", … }` | `terse listen` |

Both streams deliver JSON on `data:` lines. Terse also sends SSE **comment** keepalives between events so proxies do not close idle connections—the session endpoint emits `: keepalive` every **15 seconds**. The SDK and CLI parsers only read `data:` lines and ignore comments. If you write your own SSE client, skip lines that start with `:`.

On write or socket errors, the server tears down the stream instead of leaving a half-open listener attached. Per-user connection caps are not enforced on these endpoints yet.
