...
- **Project secrets** are custom environment variables you store for a hosted project (for example `OPENAI_API_KEY`). Terse injects them into the sandbox alongside `TERSE_API_KEY`, `TERSE_BACKEND_URL`, and `TERSE_RUN_ID`. Upload them with [`terse secrets`](/reference/cli#terse-secrets) or let `terse deploy` offer to sync missing keys from `.env`. Read them in workflow code with `process.env.YOUR_SECRET_NAME`. Secret names cannot start with `TERSE_`. Self-hosted servers manage their own env vars; project secrets are not available there.

- Tokens with an **expiration** stop working when they expire; the API responds with **401** and an expired-token message.

The API token list in the app shows **user** tokens only. Short-lived project tokens and project secret values do not appear there.

**Integration credentials** (OAuth tokens for Slack, GitHub, Attio, and other connected systems) are stored separately in Terse's secret manager. Connect integrations in the app or with `terse integrate`; your workflow code does not read those tokens directly.
