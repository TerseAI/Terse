...
- `getJobContext`, `runWithJobContext`, type `TerseJobContext`: async job context (`sessionId`, `runId`, `apiBaseUrl`). `TerseAgent` and `TerseAgent.executeTool` read this when each request is made and send `X-Terse-Session-Id` and `X-Terse-Run-Id` when a run id is available (from async context or `process.env.TERSE_RUN_ID`). Outside `runWithJobContext`, set **`TERSE_BACKEND_URL`** if needed (default **`https://app.useterse.ai/api`**).

### Run attribution

When the SDK sends **`X-Terse-Run-Id`**, Terse treats the call as a **production run**: actions, skills, approvals, and run history attach to that run record in Activity. The API checks that the run belongs to your organization before streaming a response. If the id is missing, deleted, or from another organization, agent-run requests return **404 Run not found** instead of writing to the wrong run.

| How you run the workflow | Where the run id comes from | Activity / run history |
| ------------------------ | --------------------------- | ---------------------- |
| `terse test`, `terse replay`, or `terse listen` | None (local sandbox) | Not updated from SDK callbacks |
| `Terse.handleTrigger` (self-hosted) | `runId` on the trigger payload via `runWithJobContext` | Updated for that production run |
| Hosted deploy (`terse deploy`) | `TERSE_RUN_ID` in the execution environment (set by Terse) | Updated for that production run |

You can set **`TERSE_RUN_ID`** manually only when you intend to attribute SDK calls to an existing run in **your** organization (for example debugging a deployed run). Remove stale values from `.env` — a deleted or foreign run id now fails with **404** rather than attaching elsewhere.
