...
- **User tokens** are the API tokens you create in the Terse app or get from **`terse login`**. Use them for local runs, the CLI, and self-hosted **`handleTrigger`** servers. They authenticate to the full set of SDK routes your workflow needs (including deploy, codegen, and runtime calls). Terse evaluates usage and plan limits on the server from your organization and signed-in user; your workflow code does not pass billing fields on SDK requests.
