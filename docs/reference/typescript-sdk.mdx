...
<Note>
    If you run the Terse API from the [open-source monorepo](https://github.com/TerseAI/Terse), optional billing integration is configured on the server with **`BILLING_ENABLED`**, **`BILLING_SERVICE_URL`**, and **`BILLING_JWT_SECRET`**. Those variables are documented in **`backend/.env.example`** next to the other backend settings.
</Note>
