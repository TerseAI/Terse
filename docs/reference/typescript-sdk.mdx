...
- `getJobContext`, `runWithJobContext`, type `TerseJobContext`: async context for the current run (session id, run id, API base URL). `TerseAgent` runs and **`TerseAgent.executeTool`** read this so requests are attributed to the right session and run when your handler runs on the platform or via the CLI.
...
Static method. Calls a named tool directly, bypassing the LLM. Uses the same backend route as generated wrappers.

```ts
await TerseAgent.executeTool("attio.updateRecord", {
