...
- **User tokens** are the API tokens you create in the Terse app or get from **`terse login`**. Use them for local runs, the CLI (including **`terse integrate`** OAuth flows), and self-hosted **`handleTrigger`** servers. They authenticate to the full set of SDK routes your workflow needs (including deploy, codegen, integration management, and runtime calls).
