...
- `sleep`, `step`, `jobStep`, `log`, `waitForInput`, `slack`: durable helpers for timed waits, custom steps, replay-safe logging, and human input prompts. See [Durable helpers](#durable-helpers)
...
- `Actor`, `ActorInvocationError`: define and call project-scoped durable actors. See [Durable actors](/core-concepts/actors)
- `configureDurableObjects`: configure actor credentials for a custom runtime. Terse Cloud and `terse test` inject these credentials automatically
...
`Terse`, `TerseAgent`, and other SDK calls that reach Terse send `Authorization: Bearer …` from `process.env.TERSE_PROJECT_KEY`. Terse API tokens use the `terse_` prefix and come in two flavors.

Project-scoped tokens are what running workflow code uses. They are limited to runtime endpoints for one project (agent runs, tool execution, approvals, and session streams) and cannot stand in for a user token on organization or integration-management routes. Inside the Terse Cloud data plane the control plane mints one per run and removes it when the run completes; on a self-hosted data plane `terse attach` prints one to put in your server's environment.

User tokens are the API tokens you create in the Terse app or get from `terse auth login`. They live in `TERSE_API_KEY` and belong to the CLI, which uses them to deploy, run codegen, and manage integrations. Local runs (`terse run`, `terse test`) have no project key, so the CLI falls back to your user token and sets `TERSE_PROJECT_KEY` from it for the duration of the run.
...
- `triggeredAt` is a `Date`: the moment Terse received the trigger. See [trigger time](#trigger-time).
...
const end = new Date(event.triggeredAt.getTime() - 3 * DAY_MS)
...
import { GithubPRTrigger } from "./terse.generated"
...
Workflow state is scoped to one workflow. Use a [durable actor](/core-concepts/actors) when multiple workflows or concurrent runs need to update the same entity with serialized read-modify-write behavior.

Deployed runs and `terse test` runs each have their own copy of a job's state, so testing never touches production values. Test state persists across `terse test` invocations, which lets you exercise dedupe and cooldown logic locally; reset it with `terse test run --fresh-state` or inspect it with the [`terse state` commands](/reference/cli#state).
...
Define named `Actor` subclasses in `src/durable-objects.ts`, then address an instance by its stable ID:
...
Import the actor into a job file and call it from the workflow:
...
Actor methods and state use plain JSON values. Calls to one actor ID are serialized. Inside a durable workflow, wrap actor calls in a journaled step through a module-scope helper. See [Durable actors](/core-concepts/actors) for the complete definition rules, replay guidance, deployment behavior, and testing model.
...
| `media`   | `InputMedia[]`           | Optional attachments, max 10. Each is `{ kind: "image" \| "video", url, altText? }`; URLs must be reachable without Terse auth. |
...
| Manual trigger           | `Trigger` (the base event type from `terse-sdk`)                                                         |
