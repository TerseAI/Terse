...
- `createJob`, `CreateJobParameters`: register a workflow (call at module top level or from imported modules). Set `durable: true` for [durable execution](/core-concepts/durability).
...
- `jobStep`, `sleep`, `waitForInput`, `slack`, `DurableOnlyError`: durable-only helpers. See [Durability](/core-concepts/durability).
- `TerseJobContext`: type for run attribution context (session, run, project, job name). The platform and CLI set this automatically; you rarely need to construct it yourself.
...
| `durable`   | `boolean`                                       | Optional. When `true`, the workflow can suspend, resume, and replay completed steps. See [Durability](/core-concepts/durability). |
