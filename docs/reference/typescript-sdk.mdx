...
### Skills recorded on the run

On **hosted** workflow runs (deployed with `terse deploy`, not local `terse test`), Terse stores the `skills` from each `TerseAgent` you invoke with `run()` or `runAndWait()`. If your handler starts more than one agent call in the same execution—whether you reuse one agent or call `TerseAgent.create()` again with different `skills`—Terse **unions** every skill config on that run instead of replacing earlier entries. Follow-up turns in [Activity](/core-concepts/activity-and-observability) (for example a new message in the run drawer) use that combined scope so the model keeps access to every integration your handler granted during the original run.
