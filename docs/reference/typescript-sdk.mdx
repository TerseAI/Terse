...
`event.triggeredAt` is the run's clock. It is stamped once when the trigger arrives, travels with the event, and is therefore identical on every read: after queue delay, after a retry, and on every durable replay of the same run. Anchor date math to it instead of `Date.now()` or `new Date()`, both of which drift between the moment the trigger fired and the moment your code reads them.

```typescript
createJob({
    name: "Weekly search performance report",
    ...
    onTrigger: async event => {
        ...
        // Query the reporting window with the toolbox or an agent skill.
        console.log(`Reporting window: ${startDate} to ${endDate}`)
    }
})
```

To exercise window logic against a fixed date, add a top-level `triggeredAt` (an ISO 8601 timestamp) to an event fixture passed to `terse test`; it overrides the trigger time the same way `formattedContent` overrides the derived prompt text.
