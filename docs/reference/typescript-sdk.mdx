...
Hosted runs also benefit from platform-side checks on the execution environment before your code runs (including recovery when a sandbox was recycled), without any change to the exported SDK surface—your handlers still use `createJob`, `Terse`, `TerseAgent`, and the same env vars.
