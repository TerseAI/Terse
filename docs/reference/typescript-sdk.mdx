...
- `TerseAgent`: create with `TerseAgent.create()` inside `onTrigger`; use `onApprovalRequired` and `submitApprovalDecision` for programmatic tool approvals when streaming with `run()`
...
Use a **`Terse`** instance for **`handleTrigger`**, which verifies signed webhook payloads from the Terse backend and runs the matching registered workflow. The method **awaits** your workflow’s `onTrigger` handler (and the agent session for that run) before it returns, so the HTTP response reflects a completed handler when the run succeeds. You do not need `new Terse()` only to call `createJob()`.
...
When you stream with **`run()`**, you can either handle **`ToolApprovalRequestedResult`** chunks yourself or set **`agent.onApprovalRequired`**. If you set the callback, the SDK calls it for each approval request and then posts the decision with **`submitApprovalDecision`** (same contract the app uses). If you omit the callback, approval events are still yielded but no decision is sent until you call **`submitApprovalDecision`** yourself.
