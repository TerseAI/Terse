...
- **Project secrets** (hosted projects only) are injected into the sandbox as environment variables when a workflow runs. Read them with `process.env.MY_SECRET_NAME` inside `onTrigger`. Upload secrets with [`terse secrets`](/reference/cli#terse-secrets); they are separate from integration credentials you connect with `terse integrate`. Secret names must be uppercase identifiers and cannot start with `TERSE_`.
