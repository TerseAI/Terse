...
After `terse generate`, integration trigger builders and skill factories are grouped under top-level **`Triggers`** and **`Skills`** objects (for example `Triggers.github.onPROpened(...)`, `Triggers.attio.onRecordCreated(...)`, `Triggers.heyReach.onMessageReplyReceived(...)`, `Skills.slack({ channel: ... })`). System-style helpers live there too: `Triggers.schedule.cron(...)`, `Triggers.webhook.onRequest()`, and `Triggers.webMonitor.onEvent(...)`. Prefer these namespaces instead of older per-integration root exports such as `GitHub.*`, `Slack.*`, or `Schedule.*` / `Webhook.*`.
...
- Specific event types: `GithubTrigger`, `GithubPRTrigger`, `CronTrigger`, `ManualSampleTrigger`, `WebhookTrigger` (optionally generic over the JSON body type), Attio types such as `AttioRecordCreatedTrigger`, `AttioRecordUpdatedTrigger`, and `AttioTrigger` (union), HeyReach types such as `HeyReachMessageReplyReceivedTrigger` and `HeyReachTrigger`, and other per-integration exports (see `terse-sdk` package entry)
- Config helpers re-exported for trigger construction: `AttioInputConfig`, `HeyReachInputConfig`, and the `AttioEventType` / `HeyReachEventType` objects for event-type constants
...
- `openListenStream`, `SessionStreamError`, and types `ListenStreamHandle`, `OpenListenStreamOptions`: authenticated SSE client for live trigger events (powers **`terse listen`**)
- `openSessionStream`, types `SessionStreamHandle`, `OpenSessionStreamOptions`, `SessionStreamEvent`: session event stream used by **`Terse.handleTrigger`** and the CLI during local runs
- `isAgentApprovalHandlingClaimed`: whether this process has claimed approval handling for the current agent run (so the CLI does not double-prompt)
...
- **User tokens** are the API tokens you create in the Terse app (**Settings > API Tokens**, when your workspace has the SDK interface enabled) or get from **`terse auth login`**. Use them for local runs, the CLI, and self-hosted **`handleTrigger`** servers. They authenticate to the full set of SDK routes your workflow needs (including deploy, codegen, and runtime calls).
...
The **API Tokens** page lists **user** tokens for your organization (create, rename, revoke). Short-lived project tokens do not appear there. The raw token value is shown only once at creation—copy it into **`TERSE_API_KEY`** or your secret store immediately.
...
### Programmatic approvals in custom runners

When you stream with **`agent.run()`** outside the Terse CLI (for example in a self-hosted server or a custom test harness), set **`onApprovalRequired`** on the agent instance. The callback receives **`ApprovalRequestInfo`** (`stepId`, `toolName`, `arguments`) and should return whether to approve. The SDK then calls **`submitApprovalDecision({ runId, stepId, approved })`** for you.

```ts
const agent = TerseAgent.create({ prompt: "...", skills: [...], toolApprovals: ["slack.sendMessage"] })
agent.onApprovalRequired = async info => {
    // your UI or policy
    return true
}

for await (const chunk of agent.run("Draft a reply.")) {
    // approval is submitted automatically when onApprovalRequired is set
}
```

If you handle approvals yourself, the CLI's session-stream listener skips duplicate prompts when **`isAgentApprovalHandlingClaimed()`** is true.

## Live event streams

**`openListenStream(apiBaseUrl, apiKey, { jobName, projectId, onEvent })`** opens the **`/sdk/listen`** SSE stream. Each event is a serialized trigger payload for the named deployed workflow. Use this when you build your own live-debug tooling; most developers use **`terse listen`** instead, which wraps the same API.

Throw **`SessionStreamError`** (with an HTTP **`status`**) when the stream cannot be opened—for example when the workflow is not deployed.
...
- **`Triggers`**: per-integration trigger builders (when connected), including **`Triggers.attio`** when Attio input triggers are available and **`Triggers.heyReach`** when HeyReach is connected, plus **`Triggers.schedule`**, **`Triggers.webhook`**, and **`Triggers.webMonitor`**
