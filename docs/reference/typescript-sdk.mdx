...
For GitHub triggers, deliveries without `sender.login` never reach your handler (see [GitHub trigger payloads](/reference/triggers#github)). Use `filter` to skip bot-authored events when `sender.login` is present, for example `!event.sender.login.includes("[bot]")`.
