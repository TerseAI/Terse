...
Every production workflow is a **project-scoped TypeScript job** you define with `createJob()` and deploy with `terse deploy` (or run on your own server with `Terse.handleTrigger`). Terse executes that code path only—there is no separate non-SDK automation runner.
...
- `TerseAgent`: create with `TerseAgent.create()` inside `onTrigger`; optional `onApprovalRequired` callback and `submitApprovalDecision()` for programmatic approval handling during `run()`
...
### Programmatic approvals during `run()`

By default, `tool_approval_requested` events are yielded on the stream and the run waits until something submits a decision. Set **`onApprovalRequired`** on the agent to handle approvals in code (for example in a custom host or test harness). The callback receives `{ stepId, toolName, arguments }` and should return `true` to approve or `false` to reject; the SDK then calls **`submitApprovalDecision({ runId, stepId, approved })`** for you.

```ts
const agent = TerseAgent.create({
    prompt: "You draft Slack replies for the deal desk.",
    skills: [Skills.slack({ channel: SlackChannel.DealDesk })],
    toolApprovals: ["slack.sendMessage"],
    onApprovalRequired: async ({ toolName }) => {
        // Your policy here — return false to reject without executing the tool
        return toolName !== "slack.sendMessage"
    }
})

for await (const chunk of agent.run("Draft a one-line reply.")) {
    // handle stream chunks
}
```

In production, humans approve through the Terse app, Slack, or email. **Reject** in Slack ends the run with status **Cancelled**. Rejecting from Activity or Notifications blocks the gated tool; the agent may continue unless the run is cancelled.
