...
<Note>
    Hosted workflow runs use **Node.js 22** (`node:22-slim`). For local development, `terse test`, and self-hosted **`handleTrigger`** servers, use **Node.js 20 or later** (22 recommended). The `terse-sdk` package and your generated `src/terse.generated.ts` run on that runtime. Terse does **not** ship a Python runtime or Python workflow SDK.
</Note>
...
If you wire **`handleTrigger`** with [Hono](https://hono.dev) and `@hono/node-server`, run **Node.js 20 or newer** so your server satisfies the adapter’s engine requirement.
