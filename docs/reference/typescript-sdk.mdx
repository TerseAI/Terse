...
| `durable`   | `boolean`                                       | Optional. When `true`, the job can use `sleep()`, `step()`, `jobStep()`, and `waitForInput()`. See [Durability](/core-concepts/durability). |
...
Wrapped around a direct call to a module-scope client or function, runs that call as a durable step: `step(resend.emails.send({...}))`. Import `step` from `terse-sdk` so the CLI can rewrite the call. Arguments evaluate in the handler and cross the boundary as serialized data; the resolved value is journaled. The macro only runs in files that call `createJob()`. See [Durability — Making your own calls durable with step()](/core-concepts/durability#making-your-own-calls-durable-with-step).
