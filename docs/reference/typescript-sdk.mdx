...
- **User tokens** are the API tokens you create in the Terse app or get from **`terse auth login`**. Use them for local runs, the CLI, and self-hosted **`handleTrigger`** servers. They authenticate to the full set of SDK routes your workflow needs (including deploy, codegen, and runtime calls). Each user token is tied to one organization (the active org when the token was issued, or after **`terse auth org switch`**).

- **Organization scope:** Authenticated routes that include an `organizationId` in the path only succeed when that id matches your token's organization. Requests for another organization's resources (for example `GET /organizations/:organizationId/logo`) return **403 Forbidden**.
