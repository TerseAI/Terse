...
- **User tokens** are the API tokens you create in the Terse app or get from **`terse auth login`**. Each token is scoped to one organization (the active org when it was issued, or after **`terse auth org switch`**). Use them for local runs, the CLI, and self-hosted **`handleTrigger`** servers. They authenticate to the full set of SDK routes your workflow needs (including deploy, codegen, and runtime calls).
