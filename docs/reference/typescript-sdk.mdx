...
Connecting OAuth integrations is not part of the `terse-sdk` API—use the Terse app **Integrations** page or [`terse integrate`](/reference/cli#terse-integrate). Terse mints a short-lived OAuth state JWT when a flow starts; your user and organization IDs are fixed in that token so optional metadata cannot replace them. The SDK surface (`IntegrationType`, trigger types, `TerseAgent`, and related exports) does not expose JWT signing or verification helpers.
