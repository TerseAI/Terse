...
The `terse-sdk` package exports workflow and agent-runtime types and helpers such as `createJob`, `TerseAgent`, triggers, `Terse`, session streaming, and run-history re-exports. It does **not** expose client methods or types for listing or bootstrapping **Agents** from a platform template catalog; those product flows were removed. For ready-made workflow patterns in code, see [GTM workflow templates](/templates).
