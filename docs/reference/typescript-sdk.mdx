...
- `sleep`, `jobStep`, `waitForInput`, `slack`: durable-only helpers for timed waits, custom steps, and human input prompts. `.asStep()` needs no import; it is available on every promise inside durable jobs. See [Durable helpers](#durable-helpers)
...
Chained onto any call to a module-scope client or function, runs that call as a durable step: `resend.emails.send({...}).asStep()`. Arguments evaluate in the handler and cross the boundary as serialized data; the resolved value is journaled. Available on every `Promise` via a global type augmentation. See [Durability — Making your own calls durable with .asStep()](/core-concepts/durability#making-your-own-calls-durable-with-asstep).

### `jobStep(...)`

The fully explicit form of `.asStep()`: declared `input`, zod validation at the boundary, no restrictions on the step body. See [Durability — jobStep: the fully explicit form](/core-concepts/durability#jobstep-the-fully-explicit-form).
