...
`CreateJobParameters` still includes an optional `remoteServerUrl` field for typing compatibility, but **`terse deploy` does not send it today**. Point self-hosted servers at Terse using **`remoteServerUrl`** in **`terse.config.json`** (see the [CLI reference](/reference/cli#terse-attach)).
...
When you stream with **`agent.run()`**, either set **`onApprovalRequired`** so the SDK calls **`TerseAgent.submitApprovalDecision`** for you, or handle **`ToolApprovalRequestedResult`** chunks and call **`submitApprovalDecision`** yourself with the **`runId`** from the stream.
