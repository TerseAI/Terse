...
For self-hosted workflows, set **`remoteServerUrl`** in **`terse.config.json`** (not on `createJob`). Terse validates that URL at deploy time and when calling your server. See [Self-hosting](/self-hosting#remote-server-url).
...
When Terse calls your server for the deploy handshake or trigger delivery, it connects to the validated hostname and **does not follow HTTP redirects**. Respond with **2xx** directly on **`TERSE_JOB_WEBHOOK_TRIGGER_PATH`** — if your framework issues a **3xx** (for example trailing-slash or HTTP→HTTPS redirects), the run fails with a redirect error. The handshake expects compact JSON (`{ challenge, signature }`) within **64 KiB**. If your handler returns a non-2xx status, Activity shows the HTTP status only, not your response body.
