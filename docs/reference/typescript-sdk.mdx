...
- Run history types (re-exported for Activity and API payloads): `RunHistoryRecord`, `RunHistoryStatus`, `RunHistoryTrigger`, `RunHistoryDecision`, `RunHistoryAction`. Each `RunHistoryRecord` includes a `trigger` object with `event`, `integration`, and `source`, plus optional `title`, `subheader`, and `url` used for readable Activity rows (for example GitHub events show repo, PR, and actor context in the list).
