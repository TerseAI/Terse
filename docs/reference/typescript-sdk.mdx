...
### Typed WebMonitor structured output

`WebMonitor.onEvent(...)` can infer the trigger `payload` from a Zod schema passed as `outputSchema`.
...
triggers: [
    WebMonitor.onEvent({
        query: "Company announcements with sentiment",
...
Inside `onTrigger`, `event.payload` is directly typed:

```ts
event.payload.company_name
event.payload.sentiment
event.rawPayload // present when outputSchema is used
event.sourceUrls
```

Important: for best inference, do not manually annotate the `onTrigger` parameter as plain `WebMonitorTrigger`, because that drops the schema-specific payload type.
...
```ts
onTrigger: async (event: WebMonitorTriggerFor<typeof schema>) => {
    event.payload.company_name
}
```
