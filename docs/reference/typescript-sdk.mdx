...
| `remoteServerUrl` | `string`                                       | Optional field on the type; the CLI does not send it when you deploy. For self-hosted workflows, set `remoteServerUrl` in `terse.config.json` so Terse can POST signed triggers to your server ([Self-hosting](/self-hosting)). |
