...
- `TerseAgent`: configure and run the model inside `onTrigger`
- `EventType`, `TextResult`, and other result classes: discriminate chunks from `agent.run()`
...
Defines the workflow contract: triggers, optional filtering, and the handler. Model-facing **skills** and **tool approvals** belong on `TerseAgent.create(...)` inside `onTrigger`, not on `createJob`. Terse infers which skills a workflow uses from the agents you run during execution (including run history shown in the app).

| Field             | Type                                     | Description                                                                                        |
| ----------------- | ---------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `name`            | `string`                                 | Unique workflow identifier. Used to match workflows across deploys.                                |
| `triggers`        | tuple of typed triggers                  | One or more triggers that start the workflow. Types narrow `filter` and `onTrigger` event types.   |
...
| `remoteServerUrl` | `string`                                 | Optional override for the Terse API base URL (defaults from `TERSE_BACKEND_URL` or the production URL). |
...
await client.createJob({
    ...
    onTrigger: async event => {
        const agent = TerseAgent.create({
            prompt: "You enrich new deals with CRM and messaging context.",
...
Use `TerseAgent.create(...)` inside `onTrigger` (or anywhere job context is active) to configure the prompt, optional **skills**, and optional **tool approvals**. When created inside a running workflow, session metadata is picked up from job context so you do not wire run IDs manually.

### `TerseAgent.create(...)`

| Field             | Type                 | Description                                                                                         |
| ----------------- | -------------------- | --------------------------------------------------------------------------------------------------- |
| `prompt`          | `string`             | System instructions for the model.                                                                  |
| `skills`          | `Skill[]` (optional) | Capabilities the model may use during `run` and `runAndWait`. Omit for prompt-only or tools-only flows. |
| `ToolApprovals`   | `string[]` (optional) | Tool names that require human approval before execution (property name matches the SDK). See [tool approvals](#tool-approvals). |
...
for await (const chunk of agent.run("Summarize pipeline risk.")) {
    if (chunk.type === EventType.TEXT) {
        process.stdout.write((chunk as TextResult).text)
    }
}
```

Use `run` when you want to stream output progressively. Import `EventType` and `TextResult` from `terse-sdk` when you branch on chunk types.
...
Use `agent.tools.*` for guaranteed side effects. These wrappers are available when the integration is present in the workflow's triggers or in the agent's `skills` array. By contrast, `TerseAgent.create({ skills: [...] })` controls which integrations the model can use during `run` and `runAndWait`.
...
```ts
const agent = TerseAgent.create({
    prompt: "You score companies for ICP fit.",
...
````ts
const raw = await agent.runAndWait(prompt)
...
await client.createJob({
    ...
    onTrigger: async (event: Trigger) => {
        const agent = TerseAgent.create({
            prompt: "You enrich new deals and post a Slack summary.",
            skills: [Attio.skill({ lists: [AttioList.Pipeline.NewDeals] }), Apollo.skill(), Slack.skill({ channel: SlackChannel.DealDesk })],
...
await client.createJob({
    ...
    onTrigger: async (event: Trigger) => {
        const agent = TerseAgent.create({
            prompt: "You summarize enrichment data and write CRM notes.",
