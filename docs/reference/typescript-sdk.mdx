...
- **User tokens** are the API tokens you create in the Terse app or receive from the CLI (`terse login`, or a fresh token when you switch organizations). Tokens minted by the CLI are named **CLI Login** and expire **90 days** after creation; run `terse login` again to mint a new one. Use user tokens for local runs, the CLI, and self-hosted **`handleTrigger`** servers. They authenticate to the full set of SDK routes your workflow needs (including deploy, codegen, and runtime calls).
...
- Tokens with an **expiration** stop working when they expire; the API responds with **401** and an expired-token message. Older **CLI Login** tokens and app-created tokens without an expiration keep working until you revoke them.
