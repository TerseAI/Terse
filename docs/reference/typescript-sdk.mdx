...
createJob({
    ...
    onTrigger: async (event, state) => {
        const count = (await state.get("seenCount")) ?? 0
        await state.set("seenCount", count + 1)
    }
})
```
