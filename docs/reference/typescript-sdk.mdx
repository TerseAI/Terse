...
| `remoteServerUrl` | `string`                          | Optional public base URL of the server where you mount `Terse.handleTrigger` (including path prefix). Must match `terse deploy` when using `TERSE_REMOTE_SERVER_URL` or `TERSE_JOB_URL`. |
