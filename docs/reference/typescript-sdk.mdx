...
To point the SDK at a non-default backend, set `TERSE_BACKEND_URL` (it defaults to `https://api.useterse.ai`). For self-hosted deployments, set `remoteServerUrl` in `terse.config.json` instead.
