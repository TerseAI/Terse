...
- **Organization membership:** User tokens are scoped to the organization they were created in. Terse requires an **active** membership in that organization on every authenticated SDK and CLI request. If you are removed from the organization, the token stops working immediately and requests return **401** (`Invalid API token`), even when the token has not expired and has not been revoked in the app. Create a new token after you rejoin, or use a token from an organization where you still have access.
