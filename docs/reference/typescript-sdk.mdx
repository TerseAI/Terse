...
<Warning>
    Never log `TERSE_API_KEY`, `TERSE_SIGNING_SECRET`, or a workflow's full [webhook URL](/reference/triggers#webhook). When debugging HTTP failures in your handler, log message and status fields only—raw `fetch` or Axios error objects can include `Authorization` headers from the SDK.
</Warning>
