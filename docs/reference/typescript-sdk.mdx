...
<Note>
**TerseAgent** maps SSE **`error`** events to thrown exceptions (for example billing blocks, credit limits, or agent failures). Handle them like any other error in your workflow code. Activity updates and workflow notifications configured for **errors** apply once Terse records a final run outcome; they are not tied to every failure that appears only on the agent stream.
</Note>
