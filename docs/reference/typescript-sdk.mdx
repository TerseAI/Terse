...
After `terse generate`, integration trigger builders and skill factories are grouped under top-level **`Triggers`** and **`Skills`** objects (for example `Triggers.github.onPROpened(...)`, `Skills.slack({ channel: ... })`). System-style helpers live there too: `Triggers.schedule.cron(...)`, `Triggers.webhook.onRequest()`, and `Triggers.webMonitor.onEvent(...)`. Prefer these namespaces instead of older per-integration root exports such as `GitHub.*`, `Slack.*`, or `Schedule.*` / `Webhook.*`.
...
## Consecutive run failures (hosted)

For workflows **running on Terse** (hosted Modal or your self-hosted server after deploy), each run that ends in **Failed** increments a consecutive-failure counter for that workflow. After **three** failures in a row, Terse automatically turns the workflow **off** so it stops receiving triggers until you fix the underlying issue and turn it back **on** from **Workflows** in the Terse app. A **successful** hosted run resets the counter to zero, and turning the workflow back **on** also resets it.

If you receive **error** run-failure notifications (Slack or email), the copy reflects where you are in that sequence: first failure, a warning on the second consecutive failure, then a notice when Terse has paused the workflow, with a link to workflow settings.
...
- **`Triggers`**: per-integration trigger builders (when connected) plus **`Triggers.schedule`**, **`Triggers.webhook`**, and **`Triggers.webMonitor`**
