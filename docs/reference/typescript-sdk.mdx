...
<Note>
Hosted agent runs—production executions and **`terse test`** runs that call **`TerseAgent.run`** / **`runAndWait`** against the Terse API—use your workspace model settings. When that model is served by **Anthropic**, Terse enables Anthropic prompt caching on the server for eligible prompt prefixes. You do not add SDK fields for this; it is handled when the request is processed.
</Note>
