...
- `getJobContext`, `runWithJobContext`, type `TerseJobContext`: async job context (session id, run id, API base URL). The SDK reads this **when each request is made** so agent runs and deterministic tool calls stay attributed to the right session and run. Outside `runWithJobContext`, set **`TERSE_BACKEND_URL`** to the Terse API base URL if needed. Production is **`https://api.useterse.ai`** (what the CLI uses). When the variable is unset, **`terse-sdk`** defaults to **`https://app.useterse.ai/api`**, which proxies to the same backend. Optionally set **`TERSE_RUN_ID`** for run attribution.
