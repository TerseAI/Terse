...
`handleTrigger` does not return until `onTrigger` finishes. If you run the webhook server yourself, add a health route for your load balancer and drain in-flight requests on **SIGTERM** before exit so deploys do not cut off active runs. See [Production readiness](/self-hosting#production-readiness).
