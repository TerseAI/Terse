...
`Terse`, `TerseAgent`, and self-hosted **`handleTrigger`** servers call the Terse API at **`https://api.useterse.ai`** by default (same as the CLI). Set **`TERSE_BACKEND_URL`** when you point at a preview stack, a local backend, or another Terse control plane. Do not use `https://app.useterse.ai/api`; the web app no longer proxies API traffic.

Hosted Modal runs receive **`TERSE_BACKEND_URL`** from Terse automatically.
...
Pass **`skills`**, **`toolApprovals`**, and the agent **`prompt`** to **`TerseAgent.create()`** inside `onTrigger`, not on `createJob`. For self-hosted deployments, set the public URL of your webhook server in **`remoteServerUrl`** inside **`terse.config.json`** (see [`terse deploy`](/reference/cli#terse-deploy)), not on `createJob`.
