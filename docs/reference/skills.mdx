...
Web search and extraction tools available to all workflows.
...
Image editing is a separate built-in skill. Use **`Skills.imageEdit()`** when you want the agent to call **`edit_image`**; you do not need **`Skills.web()`** for that tool alone.
