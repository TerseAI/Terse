...
`slack_send_message` inputs:

| Parameter        | Description |
| ---------------- | ----------- |
| `channelId`      | Destination channel id (public, private, or an existing DM). Use `SlackChannel.*.channelId` from `src/terse.generated.ts`. If both `channelId` and `slackUserId` are set, `channelId` wins. |
| `slackUserId`    | Member id (`U…`) to DM instead of a channel; Terse opens the DM if needed. Use `SlackUser.*.userId` from `src/terse.generated.ts`. |
...
| `thread_ts`      | Reply in a thread. Pass the thread root's `ts` (e.g. `"1712345678.123456"`), never a reply's own `ts`. Every send returns a `thread_ts` for follow-up replies; Slack trigger events carry it as `event.threadTs`. A malformed value fails the send instead of posting an unthreaded message. |
