...
| `thread_ts`      | Reply in a thread. Pass the thread root's `ts` (e.g. `"1712345678.123456"`), never a reply's own `ts`. Every send returns a `thread_ts` you can pass to later sends to keep replying in the same thread; Slack trigger events carry it as `event.threadTs` and `slack_read_conversation` messages as `threadTs`. A malformed value fails the send instead of posting an unthreaded message. |
