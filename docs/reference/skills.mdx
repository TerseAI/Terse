...
Read campaign performance, sync custom audiences, and send offline conversions via the Conversions API. Optionally scope to a default ad account.

```ts
import { MetaAdsAdAccount, Skills } from "./terse.generated"

skills: [Skills.metaAds({ adAccount: MetaAdsAdAccount.Primary })]
```

`adAccount` is optional. Omit it and the model discovers ad accounts with `meta_ads_read_campaigns`.

| Tool                             | Description                                                            | Access |
| -------------------------------- | ---------------------------------------------------------------------- | ------ |
| `meta_ads_read_campaigns`        | List ad accounts, campaigns, or ad sets with status and budget         | Read   |
| `meta_ads_read_insights`         | Spend, impressions, clicks, and conversions at campaign/ad set level   | Read   |
...
`meta_ads_read_insights` follows Graph API pagination up to 2000 rows and sets `truncated: true` in the result when more rows exist; narrow the date range or filter by campaign/ad set IDs to fetch the remainder.
