...
When **`searchPosthogEvents`** runs with **`countByEventNameOnly: true`** (the default), optional **`eventName`**, **`dateFrom`**, and **`dateTo`** filters are passed to PostHog as bound HogQL parameters. Format **`dateFrom`** and **`dateTo`** as UTC timestamps: `YYYY-MM-DD HH:mm:ss` (for example `2026-02-06 14:00:00`). Do not use ISO strings with `T`/`Z` or relative values like `-7d`.
...
Web search and image editing are separate skills. Add one or both to your workflow.
...
| Tool         | Description                | Access |
| ------------ | -------------------------- | ------ |
| `image_edit` | Edit and transform images  | Read   |

Terse caches **`image_edit`** results per organization. Reusing the same **`image_url`** and **`prompt`** in the same org returns the cached image; another org with the same inputs gets a separate stored result.
