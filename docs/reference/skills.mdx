...
For **`search_code`** and **`grep_code`**, pass plain search text in **`query`** / **`pattern`**. Do not embed GitHub search qualifiers such as `repo:`, `user:`, or `path:` in those fields—Terse applies the repo allowlist from your skill config. Optional filters (`language`, `filename`, `fileExtension`, `path`) must not contain whitespace, colons, or quote characters. The tools reject values that would escape the configured repositories.
