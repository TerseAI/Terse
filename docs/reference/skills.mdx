...
### Connecting, and why it stays connected

Terse connects through **Facebook Login for Business** using a configuration you create in the Meta App Dashboard. The configuration, not Terse, decides which permissions and assets are requested, and it is set to issue a **business integration system user token**.

That choice is what keeps an automation alive. A classic Facebook user token lasts about 60 days and cannot be renewed without the person logging in again, so a scheduled job would silently stop working roughly two months after setup. A system user token defaults to never expiring, so the connection survives without anyone touching it.

The token dies in exactly one way: the client removes your app under Business settings, Connected apps. Terse checks each connection against Meta on its maintenance pass and reports a revoked token rather than discovering it mid-run.

To connect, open Integrations, choose Meta Ads, and approve the business and assets Meta lists. Testing the flow requires a role on the app **and** full control of the client business.
...
`model` picks cost against quality: `dop-lite`, `dop-turbo` (default), `dop-standard`.

### Motions

A motion is a named camera move that Higgsfield applies on top of your prompt: a dolly, an orbit, a zoom, and so on. It controls how the camera behaves during the animation, where the prompt controls what happens in the frame. Reach for one when you want a specific, repeatable camera language across a set of ads instead of whatever the prompt happens to produce.

Motion IDs are opaque strings you cannot guess, so `higgsfield_list_motions` exists to discover them. Pass one as `motionId` on `higgsfield_generate_video`, and scale it with `motionStrength` between 0 and 1 (defaults to 1). Omit `motionId` entirely and the prompt alone drives the movement, which is usually fine for a single ad.
...
Open Integrations, choose Google Search Console, and sign in with the Google account that owns or is delegated on the properties. Property access follows that Google account, not your Terse organization, so connect the account that actually appears in Search Console.

<Note>
    Search Console uses the `webmasters` scope, which Google classes as sensitive. While the OAuth client is still in Testing status, only Google accounts added to its test-user list can connect, capped at 100. If a connection attempt fails with an access-blocked error, that list is the first place to look.
</Note>
