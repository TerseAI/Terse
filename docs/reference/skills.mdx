...
Read ad performance, create and retire ads, sync custom audiences, and send offline conversions via the Conversions API. Optionally scope to a default ad account and Page.
...
Both options are optional. Omit `adAccount` and the model discovers ad accounts with `meta_ads_read_campaigns`; omit `pageId` and it discovers Pages with `meta_ads_read_pages`. Pinning both is recommended when an account has more than one of either, so the model cannot publish under the wrong brand.

| Tool                             | Description                                                              | Access |
| -------------------------------- | ------------------------------------------------------------------------ | ------ |
| `meta_ads_read_campaigns`        | List ad accounts, campaigns, or ad sets with status and budget           | Read   |
| `meta_ads_read_insights`         | Spend, impressions, clicks, and conversions at campaign/ad set/ad level   | Read   |
| `meta_ads_read_ads`              | List ads with the creative attached to each one                          | Read   |
| `meta_ads_read_audiences`        | List custom audiences with approximate sizes                             | Read   |
| `meta_ads_list_pixels`           | List pixels (Conversions API dataset IDs) in an ad account               | Read   |
| `meta_ads_read_pages`            | List the Facebook Pages the connected user manages                      | Read   |
| `meta_ads_update_audience_users` | Add or remove people on a custom audience (emails/phones hashed)         | Write  |
| `meta_ads_send_conversions`      | Send offline conversion events via the Conversions API                   | Write  |
| `meta_ads_create_ad`             | Create an ad creative and the ad that uses it in an existing ad set     | Write  |
...
`meta_ads_create_ad` takes `pictureUrl` rather than image bytes. Meta downloads that URL once at creation time and stores its own copy in the ad account's image library, so a signed URL with a short expiry is fine, including the URLs returned by `higgsfield_generate_image`.

New ads enter `PENDING_REVIEW` before reaching the status you requested, and can come back `DISAPPROVED`. Re-read `effective_status` with `meta_ads_read_ads` before treating an ad as live.
...
Generate ad creative images from a text prompt.
...
Generation is asynchronous and the tool waits for it to finish, so a call can take a while. Each image is copied into Terse-managed storage and returned as a signed URL valid for 24 hours. That URL can be shown to a human for approval and passed straight to `meta_ads_create_ad` as `pictureUrl`.

Every call spends Higgsfield credits, so the tool is marked as a write and can be gated behind approval. Use `batchSize: 4` when you want variants to compare, and match `size` to the placement: a landscape size such as `2048x1152` for feed ads, a portrait size such as `1152x2048` for stories.
