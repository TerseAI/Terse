...
When embedding inline images via `image_urls` (signed internal bucket URLs referenced as `cid:…` in `html_body`), Terse downloads at most **10** images per send and rejects any single image larger than **25 MB**. Extra URLs are dropped with a warning.
