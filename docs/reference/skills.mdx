...
Pass the LaunchDarkly flag key (for example `my-feature-flag`) as `flagKey`. Terse resolves it within the project you configured on the skill; path-like values are treated as literal keys, not alternate API routes.
