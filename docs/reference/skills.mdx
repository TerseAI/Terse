...
Read-only access to logs and RUM events. Connect with `terse integrate connect datadog` (or **Integrations** in the app). When you rotate keys or change region, run `connect` again with `--force` so stored credentials and region metadata stay aligned.
