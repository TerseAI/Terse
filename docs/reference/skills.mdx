...
`aggregate_rum_events` accepts optional `pageLimit` for pagination buckets (default **25**, max **1000**), matching the other Datadog RUM tools.
...
After `terse integrate connect launchdarkly`, Terse does not infer a token name from the LaunchDarkly API. The connection is labeled by integration id (and email when available) in the app and in `terse integrate list`.
