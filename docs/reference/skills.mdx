...
In TypeScript, pass the `skills` array to `TerseAgent.create({ prompt, skills })` inside your job's `onTrigger` handler (not on `createJob`). Terse uses the skills from the agents you run so the product UI stays in sync with your code.
...
```ts
TerseAgent.create({
    prompt: "Your workflow instructions",
