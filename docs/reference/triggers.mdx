...
await client.createJob({
  ...
  triggers: [
    WebMonitor.onEvent({
      query: "What is going on in the Strait of Hormuz?",
      frequency: { number: 1, unit: "hour" },
      outputSchema: outputSchema,
    }),
  ],
  onTrigger: async (event) => {
    // Can safely infer the type of summary/sentiment coming from event
    const summary = event.payload.summary;
    const sentiment = event.payload.sentiment;

    const prompt = `News on the strait of hormuz: ${summary} Sentiment: ${sentiment}`;
    ...
    await agent.runAndWait(
      "Analyze the news and post a summary to the Slack channel: " + prompt,
    );
  },
});
```
