...
<Warning>
    The path segment in each webhook URL is the trigger's only authentication. It does not rotate on redeploy. Treat the full URL like a password: store it in a secret manager, never commit it, and never paste it into application logs, error trackers, or chat.
</Warning>
