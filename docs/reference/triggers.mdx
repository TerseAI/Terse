...
<CodeGroup>```ts TypeScript triggers: [Gmail.onNewEmail()] ```</CodeGroup>
...
When you run `terse test` (or list cached samples), Terse only hydrates Web Monitor sample events for monitors that belong to your organization.
...
<CodeGroup>```ts TypeScript triggers: [Schedule.cron("0 9 * * 1")] // Every Monday at 9am ```</CodeGroup>
...
<CodeGroup>```ts TypeScript triggers: [Webhook.onRequest()] ```</CodeGroup>
