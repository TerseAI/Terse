...
HeyReach fires when LinkedIn outreach events occur (messages, connection requests, campaign milestones, and more). Connect with `terse integrate connect hey_reach` or under **Integrations** in the Terse app, then run **`terse generate`** so **`Triggers.heyReach`** and typed **`HeyReachCampaign`** constants appear in `src/terse.generated.ts`.
