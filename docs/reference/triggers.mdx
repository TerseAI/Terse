...
Each webhook URL is rate limited to **100 requests per minute** per token. Bursts beyond that receive **429** with a **`Retry-After`** header. Design callers to back off instead of tight retry loops.
