...
Every event, whatever its trigger, carries `triggeredAt`: a `Date` fixed to the moment Terse received the trigger. Use it instead of `Date.now()` for any date math the run depends on. See [trigger time](/reference/typescript-sdk#trigger-time).
...
| `Triggers.github.onIssueCreated()` | `GithubIssueCreatedTrigger` | An issue is created |
| `Triggers.github.onIssueComment()`   | `GithubIssueCommentCreatedTrigger` | A comment is created on an issue or pull request |
| `Triggers.github.onPRCommentCreated()` | `GithubPRCommentCreatedTrigger` | A comment is created on a pull request |
| `Triggers.github.onPRCommentEdited()` | `GithubPRCommentEditedTrigger` | A comment is edited on a pull request |
| `Triggers.github.onPRComment()`      | `GithubPRCommentTrigger` | A comment is created or edited on a pull request |
...
- `sender` - `{ login, email }`
Push and PR events also include:
...
Issue-created events (`issues.opened`) add:

- `issue` - `{ id, number, title, body?, state, url, author, isPullRequest: false }`

Issue and PR comment events add:
...
```ts
import type { WebMonitorTriggerFor } from "./terse.generated";
import * as z from "zod";
...
<CodeGroup>
```ts TypeScript
triggers: [Triggers.schedule.cron({ expression: "0 9 * * 1", timezone: "America/New_York" })] // Every Monday at 9:00am Eastern
```
</CodeGroup>
