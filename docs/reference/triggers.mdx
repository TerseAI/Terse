...
- `sender` - `{ login, email }` (`login` is always set on events Terse delivers)
- `commits` - array of `{ sha, message, fileDiffs: [{ filename, diff }] }`

<Note>
    Some GitHub webhook deliveries (bot-driven or app-only events) do not include `sender.login`. Terse ignores those deliveries, so your workflow will not run and they will not appear in `terse test` sample lists.
</Note>
