...
<Expandable title="Event payload">
  ...
  - `threadTs` - the thread root's `ts` when this message is in a thread, otherwise `null`. Pass it back to `slack_send_message` as `thread_ts` to reply in the same thread
