...
## How events reach your workflow

| Trigger kind | Where the event lands first | What you configure in code |
| ------------ | ----------------------------- | -------------------------- |
| Integration (GitHub, Gmail, Slack, …) | Terse | Connect the integration in the app, then use **`Triggers.*`** from `terse.generated.ts` |
| `Triggers.webhook.onRequest` | Terse (generated URL) | Point your caller at the webhook URL from **`terse deploy`** |
| Any trigger with [self-hosting](/self-hosting) | Your HTTP server | Mount **`TERSE_JOB_WEBHOOK_TRIGGER_PATH`** and call **`terse.handleTrigger`** with **`TERSE_SIGNING_SECRET`** |

**GitHub** and **Gmail** integration triggers are not HTTP endpoints you implement. Terse receives provider webhooks, verifies them, then dispatches matching workflows. Connect the integration under **Integrations** in the app—your workflow code only declares the trigger (for example **`Triggers.github.onPROpened`** or **`Triggers.gmail.onEmail`**).
...
**Config:** Scoped to specific repositories via `repo` parameter. Events flow through the Terse GitHub App; Terse rejects forged deliveries before your workflow runs.
...
Connect Gmail under **Integrations** so Terse can subscribe to your mailbox. New-mail notifications arrive via Google Pub/Sub; Terse verifies each push before dispatching workflows.
