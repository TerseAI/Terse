...
The CLI reads these optional overrides from the process environment:

| Variable | Default | Used by |
| -------- | ------- | ------- |
| `TERSE_BACKEND_URL` | `https://api.useterse.ai` | API calls (`terse test`, `terse deploy`, `terse generate`, `terse listen`, and others) |
| `TERSE_FRONTEND_URL` | `https://app.useterse.ai` | `terse dashboard` |
| `TERSE_DOCS_URL` | `https://docs.useterse.ai` | `terse docs` |

Set `TERSE_BACKEND_URL` when you point the CLI at a self-hosted Terse backend or a local dev server.
