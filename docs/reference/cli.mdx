...
| Trigger builders            | `Triggers.github.onPROpened()`, `Triggers.slack.onMessage()`, `Triggers.attio.onRecordCreated()`, `Triggers.heyReach.onMessageReplyReceived()`, `Triggers.schedule.cron()`, `Triggers.webhook.onRequest<BodyType>()` |
...
The CLI uses **`openListenStream`** from **`terse-sdk`** against **`/sdk/listen`**. See the [TypeScript SDK reference](/reference/typescript-sdk#live-event-streams) if you wire your own listener.
...
Weekly workflow reviews produce patch suggestions in the Terse app (**Improvements** tab on each workflow). Apply them from your repo with:

### `terse list improvements`

Lists pending improvement cards across all workflows in your organization.
...
Downloads the suggested patch, runs **`git apply`** locally (with 3-way and **`--reject`** fallbacks if your tree has drifted), and marks the improvement **Applied** in Terse.
...
Omit **`improvement-id`** to pick from an interactive list. In **`--non-interactive`** mode, pass the id explicitly (use **`terse list improvements`** to find ids).

See [Self-improvement](/core-concepts/self-improvement) for how reviews are generated and when workflows are skipped.
...
You can also create **user** API tokens in the app under **Settings > API Tokens** (when your workspace has the SDK interface enabled) and set **`TERSE_API_KEY`** yourself. See [Authentication and `TERSE_API_KEY`](/reference/typescript-sdk#authentication-and-terse_api_key).
...
Prints the signed-in user and active organization.
...
Switches the active organization for CLI commands. Omit **`org-id`** to pick interactively.
...
Those values are **user** API tokens (the same kind you create under **Settings > API Tokens** in the app). Hosted executions use separate **project-scoped** tokens that Terse injects for you; you do not paste those into `.env`.
