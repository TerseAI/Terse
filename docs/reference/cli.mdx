...
Commands that call the Terse API honor **`TERSE_BACKEND_URL`** when set (otherwise the CLI uses its packaged default). Use this for local development or staging stacks. **`terse-sdk`** reads the same variable for outbound API calls when job context does not supply a base URL ([TypeScript SDK reference](/reference/typescript-sdk)).
