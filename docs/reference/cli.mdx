...
| Form  | Prompts for credentials or account details in the terminal                 | Datadog, HeyReach, PostHog, Snowflake, LaunchDarkly, Attio |
...
<Note>
    **Datadog credential updates:** When you refresh Datadog with `terse integrate connect datadog --force`, Terse stores your API and application keys before it updates the saved region. If the request fails after the keys are saved, Datadog tools keep using the keys you just submitted—re-run the same `connect` command to sync the region shown in **Integrations**.
</Note>
