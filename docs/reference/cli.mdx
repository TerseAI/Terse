...
Before you run `terse deploy`, set `remoteServerUrl` in `terse.config.json` to the URL where your Terse SDK server is running. The URL must use HTTPS and resolve to a public address; Terse rejects private or reserved IPs. See [Remote server URL](/self-hosting#remote-server-url) for handshake, redirect, and error-message behavior.
...
- Self-hosted: the CLI reads `remoteServerUrl` from `terse.config.json`, validates it with the Terse API, and configures the platform to call your server instead of uploading source
