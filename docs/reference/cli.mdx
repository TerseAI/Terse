...
| Category                    | Examples                                                                                       |
| --------------------------- | ---------------------------------------------------------------------------------------------- |
| Resource constants          | `Repos.MyOrg.MyRepo`, `SlackChannel.Engineering`, `SlackUser.OnCall.userId`, `AttioList.Pipeline.NewDeals`                |
...
**Consecutive failures on Terse:** When a **deployed** workflow fails on Terse repeatedly, the platform tracks consecutive **Failed** runs. After **three** in a row, Terse automatically disables that workflow until you turn it back **on** under **Workflows**. A successful hosted run resets the streak. Run-failure notifications (when you have **error** notifications enabled) use escalating copy—first failure, warning, then paused—with a link to workflow settings. Local **`terse test`** runs do not update this hosted counter.
