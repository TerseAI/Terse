...
### `terse resume --run-id <id>`

Resumes an interrupted durable run from its journal in `.terse/data`.

```bash
terse resume --run-id wrun_abc123
```

Use this to continue a run that was paused by `sleep()` or `waitForInput()`, or to re-drive a failed durable run under your current local code. Pending and running runs resume where they left off; completed steps replay from the journal instead of re-executing.

Options:

| Flag                  | Description                              |
| --------------------- | ---------------------------------------- |
| `-v, --verbose`       | Show job stream output during the run (default: enabled) |
| `--no-verbose`        | Hide job stream output                   |

The run's journal must be present locally (for example restored from a snapshot). In production, the Terse Cloud data plane resumes suspended runs automatically when a timer fires or a human responds in Slack.
