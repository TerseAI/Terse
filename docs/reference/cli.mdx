...
<Note>
    The Terse API rejects API tokens that are unknown, revoked, or no longer resolve to an active user in the token's organization. If commands such as `terse test`, `terse deploy`, or `terse generate` start failing with **401** and a message like `Invalid API token` or `API token is no longer valid for this organization`, run **`terse login`** again or create a new API token in the Terse app (**Settings**) and update **`TERSE_API_KEY`**.
</Note>
