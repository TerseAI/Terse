...
The CLI calls the hosted Terse API at **`https://api.useterse.ai`** by default. The TypeScript SDK defaults to **`https://app.useterse.ai/api`**; both reach the same hosted control plane (the web app proxies **`/api`** to **`api.useterse.ai`**).

Set **`TERSE_BACKEND_URL`** when you point at a preview stack, a local backend, or another Terse deployment—for example `http://localhost:3001` during backend development.
