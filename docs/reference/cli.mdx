...
- For each workflow that includes a **webhook** trigger, the CLI prints a **Webhook URL** line after deploy so you can wire up external systems without opening the app. Copy it into a secret store—the URL is a permanent credential; see [Webhook trigger](/reference/triggers#webhook).
