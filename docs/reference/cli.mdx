...
For form-based integrations, `connect` submits the provided field values immediately. For OAuth integrations, `connect` opens the authorization URL in your default browser when it is **http** or **https**, then exits 2 with a handoff payload (`{ "handoff": "oauth", "url", "waitCommand" }` in `--json` mode, or an `ACTION REQUIRED` line otherwise). If auto-open is skipped, use the printed URL. Run the printed `waitCommand` (e.g. `terse integrate wait slack`) to block until authorization completes.
...
`terse login`, OAuth flows in `terse integrate connect`, `terse dashboard`, and `terse docs` can launch your default browser. The CLI only auto-opens **http** or **https** URLs. If a URL uses another scheme or does not parse, the CLI skips auto-open and still prints the link in the terminal.
...
If `TERSE_FRONTEND_URL` is set, the CLI opens that URL instead of production. Use an **http** or **https** value so auto-open works; otherwise copy the printed URL from the terminal.
...
2. Opens your browser to a verification page when the URL is **http** or **https** (otherwise open the printed link manually)
...
If `TERSE_DOCS_URL` is set, the CLI opens that URL instead of the public docs. Use an **http** or **https** value so auto-open works; otherwise copy the printed URL from the terminal.
