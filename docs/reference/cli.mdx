...
- Hosted: the CLI zips the current project directory, uploads it, and Terse hosts the workflow for you. Each trigger runs in an isolated runtime; Terse verifies the environment is healthy before your handler runs and retries transient provisioning issues automatically.
