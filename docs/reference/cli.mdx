...
Executes a deployed workflow inside a Terse Cloud sandbox (or a Modal-backed self-hosted sandbox). The platform sets `TERSE_CLI_ENABLE_RUN` in the sandbox environment, which registers this command; a normal local CLI install does not expose it.

The platform passes the trigger event from the run record. You can also supply one explicitly when debugging:

```bash
terse run my-workflow --event-file fixture.json
```
...
When a sandbox dependency image was built with locally hoisted Terse packages (a dev-only backend setting for monorepo contributors), the first line of run output is:

```
[terse] running locally-hoisted packages: terse-types@0.0.49, terse-sdk@0.1.54, terse-cli@0.1.65 (abc123def456)
```

That line confirms the run used workspace builds of `terse-types`, `terse-sdk`, and `terse-cli` instead of the npm registry versions. It appears in the Activity run log. Production sandboxes never emit it.
