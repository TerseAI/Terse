...
The `terse-cli` npm package is licensed under the **Sustainable Use License** (version 1.0). npm metadata uses `SEE LICENSE IN LICENSE.md`; the full text is in the [Terse repository](https://github.com/TerseAI/Terse/blob/main/packages/terse-cli/LICENSE.md).

You may use and modify the CLI for your own internal business purposes, or for non-commercial or personal use. You may distribute it to others only when you do so free of charge for non-commercial purposes. Read the license for patent grants, notice requirements, termination, and other conditions.
