...
| ----- | -------------------------------------------------------------------------- | --------------------------------------------------------------- |
| OAuth | Opens a browser for authorization and waits for the connection to complete | Attio, GitHub, Gmail, Linear, Notion, Slack                     |
| Form  | Prompts for credentials or account details in the terminal                 | Datadog, HeyReach, LaunchDarkly, PostHog, Snowflake, WorkOS     |
...
<Note>
    OAuth flows are bound to the browser session that started them. Finish authorization in the browser Terse opened—do not paste the provider URL into another browser or device. Each attempt expires after **10 minutes**; run `terse integrate connect` again if it times out.
</Note>
