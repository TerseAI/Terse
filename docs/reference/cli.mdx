...
Saved API keys follow the same rules as tokens in the Terse app: they must map to an **active** organization membership. If you are removed from the organization, CLI commands that call the API return **401** until you run **`terse login`** again (or set **`TERSE_API_KEY`** from a valid token for an org you still belong to).
...
User tokens stop authenticating when your **organization membership** ends (for example after you are removed from the org in WorkOS). The API responds with **401** even if the token string is unchanged. Run **`terse login`** or rotate **`TERSE_API_KEY`** after your access is restored.
