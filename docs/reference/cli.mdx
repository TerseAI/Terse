...
If you use **Claude Code**, install the **Terse** plugin from the marketplace and run the **`/terse:init`** skill to scaffold with `terse init` and walk through common setup issues (missing CLI, existing npm project, authentication).
...
2. Fetches sample events from Terse for integration triggers such as GitHub, Slack, or Attio, and **stored webhook payloads** when your workflow has already received HTTP traffic
...
| Trigger builders            | `Triggers.github.onPROpened()`, `Triggers.slack.onMessage()`, `Triggers.schedule.cron()`, `Triggers.webhook.onRequest<BodyType>()` |
...
When **stdin or stdout is not a TTY** (for example in some CI jobs or when stdin is redirected), the CLI **skips interactive prompts** such as “log in again with a different account?” It can still start the **device login** flow if there is **no saved key** or the saved key is **invalid**, so automated environments may need a human step or a prior `terse login` on the machine.

Pass **`--non-interactive`** (`-y`) on **`terse init`** or **`terse attach`** when you want **fail-fast** behavior: those commands **require a valid API key already saved** by `terse login`. They **exit with an error** instead of device login when credentials are missing or no longer valid—use this when you must not block on browser authorization (for example in strict CI).
