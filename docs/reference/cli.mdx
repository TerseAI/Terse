...
CLI authentication lives under the `auth` command group. All commands below store or read credentials from your **user-level** config (not the project directory).

### `terse auth login`

Authenticates with Terse using a WorkOS device authorization flow and saves a Terse API key for later CLI commands.
...
4. Sends the short-lived **WorkOS access token** to Terse (`POST /sdk/auth/identify`, then `POST /sdk/auth/device-token-exchange`). Terse verifies the token against WorkOS before minting a **`terse_` API key** scoped to the organization you pick
5. Saves that API key so later CLI commands can reuse it

If you have no organizations yet, the CLI opens the org-creation page in your browser and polls until one exists. Finish that step within a few minutes of approving the device login. If the WorkOS access token expires first, the CLI reports that the session expired and you should run `terse auth login` again.
...
Shows the email and active organization for your saved API key, or tells you to run `terse auth login`.

### `terse auth org list` / `terse auth org switch`

Lists organizations you belong to and switches the active org by minting a new API key for that org (without repeating the full browser login).
...
Those values are **user** API tokens (the same kind you create in the Terse app). They are not WorkOS access tokens. Hosted executions use separate **project-scoped** tokens that Terse injects for you; you do not paste those into `.env`.
