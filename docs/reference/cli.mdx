...
Fixtures that wrap the trigger in a `data` envelope with hand-written `formattedContent` and `debugLog` (the pre-fixture format) are no longer accepted: move the fields inside `data` to the top level and drop the rest.
