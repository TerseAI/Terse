...
CLI authentication is **organization-aware**. Commands under `terse auth` sign you in with WorkOS, mint an org-scoped API key for the organization you choose, and cache keys per organization so you can switch without logging in again.

### `terse auth login`

Authenticates with Terse using a device authorization flow and saves an org-scoped API key to your user-level config file.
...
1. Requests a device code from WorkOS and opens your browser to a verification page
2. Polls until you complete browser authorization
3. Loads your Terse account and organization memberships
4. If you have no organizations yet, opens the Terse app so you can create one, then waits until membership appears
5. Prompts you to pick an organization when you belong to more than one
6. Exchanges the WorkOS token for an org-scoped API key and saves it as the active organization

If you are already logged in, the CLI shows your current user and organization. With multiple memberships, you can switch organizations or log in with a different account.

### `terse auth status`

Shows the current user and active organization.
...
Removes all saved credentials (every cached org key and the active-organization pointer).
...
Lists organizations you belong to. The active organization is marked with `*`.
...
Switches the active organization. Reuses a cached API key when you have logged into that org before; otherwise mints a new org-scoped key.

```bash
terse auth org switch
terse auth org switch org_abc123
```

Pass `org-id` to skip the interactive picker (required in non-interactive environments).
...
2. The API key for your active organization, saved by `terse auth login` (keys are cached per organization)

Those values are **user** API tokens scoped to an organization (the same kind you create in the Terse app). Hosted executions use separate **project-scoped** tokens that Terse injects for you; you do not paste those into `.env`.

After `terse init` or `terse auth login`, you usually do not need a project `.env` file for the CLI because authentication is stored per user on your machine.
