...
- 429 Too many requests: you hit an API rate limit; wait for the **`Retry-After`** window (the CLI surfaces this as a stream failure)
- 429 Too many concurrent SSE connections: you already have five open session streams for this user; close extra terminals or wait ~30 seconds and retry
...
The device-token exchange step is rate limited to **five attempts per hour** per user or client IP. A sixth attempt within that hour is blocked for one hour and returns **429**.
