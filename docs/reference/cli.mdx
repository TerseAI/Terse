...
If your workflow uses a webhook trigger but no deliveries are stored yet, the CLI prints a `curl` example against your webhook URL instead of opening an empty picker. Run `terse deploy` first so the CLI can show the URL, send a test POST, then run `terse test` again.
...
Opens a browser to confirm the session, then saves an API key to your user config so later commands don't need it inline.
