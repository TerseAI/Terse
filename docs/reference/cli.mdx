...
The scaffolded `src/terse.jobs.ts` includes a minimal starter workflow: `createJob` and `TerseAgent` from `terse-sdk`, `Triggers` from `./terse.generated`, and `z` from `zod` (already in the scaffolded `package.json`) for the example `runAndWait` schema. It type-checks with `npm run build` immediately after init.
