...
- On **hosted** projects, if a local `.env` file has valid secret names that are not yet stored on Terse, the CLI may prompt you to upload them before deploy (or run `terse secrets import .env` yourself)
...
Manage **project secrets** for hosted (Terse-managed) projects. Secrets are key-value pairs stored on Terse and injected into the hosted runtime as environment variables when a workflow runs. They are not uploaded with your deploy bundle (`.env` is excluded from the ZIP).

Project secrets are **not** available for self-hosted projects — manage runtime env vars on your own server instead.

Secret names must match `^[A-Z][A-Z0-9_]{0,63}$` and cannot start with `TERSE_`. Values are limited to 32 KB each.
...
| `add <NAME>` | Create or update one secret |
| `remove <NAME>` | Delete one secret |
| `import <file>` | Bulk import from a `.env`-format file (up to 500 entries per request) |
...
| ---- | -------- | ----------- |
| `--json` | `list` | Emit JSON |
| `--value-stdin` | `add` | Read the secret value from stdin (avoids shell history) |
| `--yes` | `remove` | Confirm removal without a prompt |
| `--overwrite` | `import` | Update secrets that already exist on the server |

In the Terse app, open your project to view and delete secret names. Add and update secrets from the CLI.
...
For **hosted** workflows, store third-party API keys and other runtime credentials as **project secrets** (`terse secrets`) so Terse injects them into the sandbox environment. Use a local `.env` for development and `terse secrets import` (or the deploy-time upload prompt) to sync eligible entries to Terse. Integration credentials (GitHub, Slack, and so on) stay on the integration — connect them with `terse integrate`, not `terse secrets`.

For **self-hosted** workflows, set `TERSE_API_KEY` and any other env vars on the server where your code runs.
