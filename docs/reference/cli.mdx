...
For **GitHub**, install the Terse GitHub App only through `terse integrate connect github` or **Integrations** in the Terse app. Both paths use the same OAuth handoff as other integrations; there is no separate installation URL API for workflow projects.
