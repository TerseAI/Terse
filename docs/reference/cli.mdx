...
Resumes an interrupted durable run from its journal in `.terse/data`.

```bash
terse resume --run-id run_abc123
```

Use this after a durable run was suspended (for example by `sleep()` or `waitForInput()`) or interrupted locally. Pending or in-progress runs continue where they left off; failed runs are re-driven under your current code with completed steps replayed from the journal.

The run's journal must be present in `.terse/data` (for example restored from a snapshot). When resuming a run that was waiting on human input, the backend sets `TERSE_RESUME_HOOK_TOKEN` and `TERSE_RESUME_HOOK_PAYLOAD` in the environment before invoking the CLI.
...
| `-v, --verbose`       | Show agent stream output during the run  | Enabled |
| `--no-verbose`        | Hide agent stream output                 |         |
| `--run-id <id>`       | Terse run id to resume (required)        |         |
...
| `--status <list>` | Comma-separated statuses: `success`, `failed`, `cancelled`, `skipped`, `in_progress`, `awaiting_approval`, `suspended` |
...
| `--include-test`  | Include runs from `terse test` (hidden by default)                                                          |
