...
Set **`TERSE_BACKEND_URL`** when the CLI or your workflow should call a Terse API other than the default. If you run that API from the open-source monorepo, optional billing integration for the server is documented with **`BILLING_ENABLED`**, **`BILLING_SERVICE_URL`**, and **`BILLING_JWT_SECRET`** in **`backend/.env.example`**.
