...
Invokes a read-only toolbox tool ad hoc and prints its JSON result. No job or deploy required, so you can check external state (does a duplicate deal exist? which channels are in this workspace?) straight from the terminal.
...
Tool names are accepted as `attio_records` or `attio.records`. Only read-only tools can run ad hoc: write tools and approval-gated tools are rejected and run inside jobs. Params use the tool's wire shape; see `terse integrate tool <type> <tool-name> --json` for the input schema. The result prints to stdout as raw JSON, and errors go to stderr with a nonzero exit code.

When the integration has several connections, the CLI resolves which one to call in this order: `--integration`, then the project pin set with `terse integrate use`, then the sole connection. An ambiguous call fails with the list of available connections.
...
Lists an integration's connections with their IDs and which one this project pins. This is the non-interactive counterpart to the `terse integrate use` picker: list here, then pin with `terse integrate use <type> <connection-id>`.
