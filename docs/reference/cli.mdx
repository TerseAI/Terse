...
6. Executes the workflow locally without a production run id, so agent and tool callbacks do not write to another run's Activity record (see [Run attribution](/reference/typescript-sdk#run-attribution) in the SDK reference)
