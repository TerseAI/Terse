...
6. Executes the workflow locally. If a tool in **`toolApprovals`** runs, the CLI prompts for approval on the session stream (spinner pauses until you approve or reject). Set **`TerseAgent.onApprovalRequired`** in your workflow to handle approvals in code instead; the CLI skips its prompt when that callback is active.
...
- Notification settings for deployed workflows are set to notify on **run failures** and **tool approval requests**
