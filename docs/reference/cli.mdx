...
The CLI and SDK call the hosted Terse API at **`https://api.useterse.ai`** by default. Set **`TERSE_BACKEND_URL`** when you point at a preview stack, a local backend, or another Terse control plane. Do not use `https://app.useterse.ai/api`; the web app no longer proxies API traffic.
