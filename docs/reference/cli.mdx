...
CLI commands use the **active Terse organization** for integrations, deploy, and sample events. OAuth connections (including GitHub) are stored per organization.
...
### `terse auth org list` and `terse auth org switch`

List organizations you belong to or change which organization the CLI targets.
...
Run `terse integrate connect github` (and other OAuth integrations) while the intended organization is active so tokens and generated `Repos` helpers match that org's GitHub installation.
...
2. The API key saved by `terse auth login` for the active organization
