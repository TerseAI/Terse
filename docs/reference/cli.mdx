...
`event.triggeredAt` defaults to the time you run the test. Add a top-level `triggeredAt` ISO 8601 timestamp (for example `"2026-07-01T09:00:00.000Z"`) to pin it, so a job that derives date windows from the trigger time produces the same window on every run.
