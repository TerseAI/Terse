...
1. Authenticates with Terse, prompting you to pick which organization owns this project when your account belongs to more than one
2. Prints the `TERSE_API_KEY` for the chosen organization so you can add it to your server environment
3. Creates a Terse project if the repo is not already linked
4. Writes `terse.config.json` with self-hosted mode enabled
...
- For each deployed job, the CLI prints a link to its page in the app (`/app/jobs/<id>`)
...
- For managed (non-self-hosted) projects with a local `.env`, the CLI offers to upload any variables not already stored as project secrets before the deploy runs. Use [`terse secrets`](#terse-secrets) for finer-grained control.

Deployment modes:

- Hosted: the CLI zips the current project directory, uploads it, and Terse hosts the workflow for you
- Self-hosted: the CLI reads `remoteServerUrl` from `terse.config.json` and configures Terse to call your own server instead of uploading source. On the first self-hosted deploy, the CLI prints a new `TERSE_API_KEY` and `TERSE_SIGNING_SECRET` — save these into your server's environment, they will not be shown again.
...
Manages project secrets for managed (non-self-hosted) projects. Secrets are stored on Terse and injected into hosted runs as environment variables. Self-hosted projects manage their own runtime env vars and these commands return an error there.
...
Terse surfaces suggested code patches for your deployed jobs based on past runs. These commands let you browse and apply them locally.

### `terse list improvements`

Lists pending improvements grouped by job.
...
Those values are **user** API tokens (the same kind you create in the Terse app). Hosted executions use separate **project-scoped** tokens that Terse injects for you automatically — you don't paste those into `.env` and you don't need to set anything in your hosted runtime.
...
For **self-hosted** deployments, your own server needs `TERSE_API_KEY` in its environment so it can authenticate with Terse at runtime. `terse attach` prints the per-organization key, and `terse deploy` prints a project-scoped key and `TERSE_SIGNING_SECRET` the first time you deploy in self-hosted mode — copy these into your server's secret store. `terse secrets` is for your own application secrets (database URLs, third-party API keys); it intentionally refuses any name that starts with `TERSE_`.
