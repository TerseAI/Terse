...
Before the run starts, the CLI labels the runtime as **durable** or **direct** based on whether the workflow has `durable: true`. During the run, a progress indicator tracks execution while your workflow's `console.log` output scrolls cleanly above it — logs no longer collide with the spinner. When the run needs your input (tool approvals or `waitForInput()`), the indicator pauses with "Awaiting input" until you respond.
