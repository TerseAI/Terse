...
2. Fetches sample events from Terse for integration triggers such as GitHub, Slack, or Attio
3. Synthesizes local sample events for cron and webhook triggers
4. Prompts you to choose one sample event
5. Executes the workflow locally
...
Use this command when you want a stable handle for a sample event without opening the interactive picker. The id stays the same as long as the underlying trigger payload stays the same.
...
| Trigger builders            | `GitHub.onPROpened()`, `Slack.onMessage()`, `Schedule.cron()`, `Webhook.onRequest<BodyType>()` |
...
Each integration generates typed helpers from real workspace data such as repositories, channels, lists, projects, and tool definitions, so your workflows reference actual resources instead of raw ids.
