...
Each login mints a new **CLI Login** API token for that organization. The token expires **90 days** after it is created. When it expires, API calls return **401** until you run `terse login` again. Logging in from another machine creates a separate token (one per device). You can revoke individual tokens from the API token list in the Terse app.
