...
<Note>
Integration setup endpoints (used by **`terse integrate`** and **`terse generate`** when loading teams, projects, repos, Notion resources, and similar metadata) return only a fixed **`error`** string on **500** responses—for example `"Failed to fetch Linear teams"` or `"Failed to disconnect integration"`. Upstream provider or database messages are not included. **4xx** responses such as `"Unauthorized"` or `"integrationId is required"` are unchanged. If codegen fails with a generic message, refresh the integration connection or contact Terse support; do not rely on parsing **500** bodies for third-party error text.
</Note>
