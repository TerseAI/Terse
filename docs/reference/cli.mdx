...
1. Authenticates — silently reuses valid saved credentials, or runs the device login flow when you are not signed in (no prompt to switch accounts if you are already logged in)
2. Creates the target directory and scaffolds `package.json`, `tsconfig.json`, `src/terse.jobs.ts`, `.env.example`, and `.gitignore`
3. Detects `pnpm` or `npm` and installs dependencies
4. Creates a Terse project and writes `terse.config.json`
5. Reviews your current integrations
6. Runs `terse generate`

If any step fails after the new project directory was created, init removes that directory. Use `terse auth org switch` or `terse auth logout` then `terse auth login` to change organization or account.
...
- Prints dashboard links for each workflow (`/app/agents/:id` on `app.useterse.ai`) so you can open them in the Terse app
...
1. If saved credentials are still valid, reuses them without opening the browser
2. Otherwise requests a device code from WorkOS, opens your browser to a verification page, and polls until authorization completes
3. Exchanges the token with the Terse backend for an API key and saves it for later CLI commands

To switch organizations, use `terse auth org switch`. To sign in as a different user, run `terse auth logout` then `terse auth login`.

#### `terse auth org switch [org-id]`

Switches the active organization for CLI commands. Pass an org id to skip the picker, or run `terse auth org list` to see ids.

### `terse auth logout`

Removes saved credentials from your user config.
