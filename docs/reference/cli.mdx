...
- On **hosted** projects, the CLI reports **Building dependency image** and **Building source image** while Terse prepares cached runtimes from your `package.json`. If those stages fail due to a transient error, run `terse deploy` again—you do not need to change project code to recover
