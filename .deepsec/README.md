...
Currently configured project: `ai-product-owner` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/ai-product-owner/SETUP.md` to fill in
   `data/ai-product-owner/INFO.md`.
...
`--project-id ai-product-owner` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/ai-product-owner/`.
