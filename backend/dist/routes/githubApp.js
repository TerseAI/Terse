...
export async function githubAppInstallationCallback(req, res) {
    ...
    let user = await db().users.findUnique({ where: { github_username: body.username } });
    if (!user) {
...
export async function githubAppInstallationDeleted(req, res) {
...
export async function githubAppRecievedCommit(req, res) {
    const body = req.body;
...
export async function githubAppRecievedIssueComment(req, res) {
    const body = req.body;
}
export default {
    getInstallationUrl
