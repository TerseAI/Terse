...
export class LinearAdapter {
  ...
  async configureWebhook(): Promise<{
    ...
    console.log(
      chalk.green("✅ Webhook created:"),
      chalk.cyan(webhook.webhookId),
    );
  ...
  async getAllTickets(): Promise<Ticket[]> {
    ...
    return await Promise.all(
      tickets.nodes.map(async (ticket) => this.convertLinearTicket(ticket)),
    );
  }
  ...
  async structuredSearch(
    ...
    if (options?.assigneeEmails && options.assigneeEmails.length > 0) {
      if (options.assigneeEmails.length === 1) {
        ...
        const users = await Promise.all(
          options.assigneeEmails.map(async (email) => {
    ...
    if (options?.createdByEmails && options.createdByEmails.length > 0) {
      if (options.createdByEmails.length === 1) {
        ...
        const users = await Promise.all(
          options.createdByEmails.map(async (email) => {
            const user = await this.resolveUser(email);
            return user;
          }),
        );
    ...
    // Build sort options
    let sortOptions: any =
      options?.sortBy == "updatedAt" ? "updatedAt" : "createdAt";
    ...
    console.log(
      chalk.green("✅ Found"),
      chalk.cyan(issues.nodes.length),
      chalk.yellow("issues"),
    );
  ...
  async associateCommitsToTicket(
    ticketId: string,
    ...
    for (const commit of commits) {
      ...
      const existing = comments.nodes.find((c) =>
        c.body.includes(commit.sha.substring(0, 8)),
      );
      if (existing) {
  ...
  async getTickets(ids: string[]): Promise<Ticket[]> {
    ...
    return await Promise.all(
      issues.nodes.map(async (issue) => this.convertLinearTicket(issue)),
    );
  }
  ...
  async createTicket(input: CreateTicketInput): Promise<Ticket> {
    ...
    console.log(
      "Assignee id",
      await this.userIdFromEmail(input.assignee || ""),
    );
    let issuePayload;
    try {
    ...
    console.log(
      chalk.green("✅ New issue created:"),
      chalk.cyan(newIssue.title),
      chalk.yellow(`(${newIssue.id})`),
    );
  ...
  async commentOnTicket(id: string, comment: string): Promise<void> {
    ...
    console.log(
      chalk.green("✅ Comment added:"),
      chalk.cyan(newComment.comment),
    );
  }
  ...
  async getLabels(
    ...
    Array<{ id: string; name: string; color: string; teamId: string }>
  > {
    const labels = teamId
  ...
  async getProjects(
    ...
    Array<{ id: string; name: string; description?: string; teamId: string }>
  > {
  ...
  async getIssueComments(
    ...
    Array<{ id: string; body: string; authorId: string; createdAt: string }>
  > {
  ...
  async getIssueRelations(
    issueId: string,
  ): Promise<Array<{ id: string; type: string; relatedIssueId: string }>> {
    const issue = await this.client.issue(issueId);
    if (!issue) throw new Error("Issue not found");
