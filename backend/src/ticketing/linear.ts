...
export class LinearAdapter implements TicketManager {
  ...
  async getTeam(
    teamId: string
  ): Promise<{ id: string; name: string; key: string; description?: string } | null> {
    try {
  ...
  async getStates(
    teamId?: string
  ): Promise<Array<{ id: string; name: string; type: string; color: string; teamId: string }>> {
    const states = teamId
  ...
  async getLabels(
    teamId?: string
  ): Promise<Array<{ id: string; name: string; color: string; teamId: string }>> {
    const labels = teamId
  ...
  async getProjects(
    teamId?: string
  ): Promise<Array<{ id: string; name: string; description?: string; teamId: string }>> {
  ...
  async getIssueComments(
    issueId: string
  ): Promise<Array<{ id: string; body: string; authorId: string; createdAt: string }>> {
  ...
  private async convertLinearTicket(issue: Issue): Promise<Ticket> {
    return {
      ...
      dueDate:
        issue.dueDate instanceof Date ? issue.dueDate.toISOString() : issue.dueDate || undefined,
