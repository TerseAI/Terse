...
export class JiraAdapter {
  ...
  static async validateCredentials(
    ...
    try {
      ...
      console.error(
        chalk.red("Invalid Jira credentials"),
        chalk.yellow(baseUrl),
        chalk.yellow(email),
        chalk.yellow(apiToken),
      );
      console.error(error);
      return false;
    }
  ...
  async structuredSearch(
    jql: string,
    _options?: StructuredSearchOptions,
  ): Promise<Ticket[]> {
    ...
    const issue = await this.client.addNewIssue({
      fields: {
        ...
        assignee: input.assignee
          ? { id: await this.userIdFromEmail(input.assignee) }
          : undefined,
      },
    });
  ...
  async me(): Promise<User | null> {
    const me = await this.client.getCurrentUser();
    return me
      ? { id: me.accountId, name: me.displayName, email: me.emailAddress }
      : null;
  }
  ...
  async associateCommitsToTicket(
    ...
    for (const commit of commits) {
      ...
      const existing = comments.comments.find((c: any) =>
