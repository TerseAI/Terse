    ...
    OAuthIntegrationInstallation<IntegrationType.ATLASSIAN>
{
  ...
  async processInstallationCallback(
    ...
    try {
      ...
      if (accountId) {
        try {
          ...
          if (existing?.webhook_id) {
            try {
              ...
              logger.warn(
                "⚠️  Could not delete existing webhook, continuing with creation",
                { error },
              );
            }
          }
        ...
        logger.warn(
          "⚠️  Could not determine accountId, skipping webhook creation",
        );
      }
  ...
  async processWebhookEvent(event: JiraWebhookPayload): Promise<void> {
    ...
    if (!userEmail && !issueUrl) {
      logger.info(
        "⚠️  [JIRA INTEGRATION MANAGER] No user email or issue URL found in webhook payload",
      );
      return;
    }
    ...
    if (matchingIntegrations.length === 0) {
      logger.info(
        `⚠️  [JIRA INTEGRATION MANAGER] No integrations found for user email: ${
          userEmail || "N/A"
        } or base URL: ${baseUrl || "N/A"}`,
      );
      return;
    }

    logger.info(
      `✅ [JIRA INTEGRATION MANAGER] Found ${matchingIntegrations.length} matching integration(s)`,
    );
    ...
    for (const integration of matchingIntegrations) {
      ...
      try {
        ...
        await runWithUserContext(user, async () => {
          ...
          try {
            ...
            if (
              ...
              logger.info(
                `📊 [JIRA INTEGRATION MANAGER] Using webhook payload for issue ${event.issue.key}`,
              );
            }
          } catch (error) {
            logger.info(
              `⚠️  [JIRA INTEGRATION MANAGER] Error enriching context: ${error}`,
            );
        ...
        logger.error(
          `❌ [JIRA INTEGRATION MANAGER] Error processing event for integration ${integration.id}`,
          {
            error,
  ...
  async setupAgentTrigger(
    ...
    try {
      ...
      if (!integration) {
        logger.warn(
          `⚠️  Integration ${integrationId} not found, skipping webhook setup`,
          { integrationId },
        );
        return;
      }
      ...
      if (!accountId) {
        logger.warn(
          `⚠️  Could not determine accountId for integration ${integrationId}, skipping webhook creation`,
          { integrationId },
        );
        return;
      }
      ...
      logger.error(
        `❌ Error setting up webhook for integration ${integrationId}`,
        {
          error,
          integrationId,
        },
  ...
  async teardownAgentTrigger(
    ...
    try {
      ...
      if (!integration.cloud_id) {
        logger.warn(
          `⚠️  Integration ${integrationId} missing cloud_id, cannot delete webhook`,
          { integrationId },
        );
      ...
      if (!accessToken) {
        logger.warn(
          `⚠️  Could not get valid access token for integration ${integrationId}, cannot delete webhook`,
          { integrationId },
        );
        // Still clear the webhook_id from the database
        await db().atlassian_integrations.update({
      ...
      try {
        ...
        logger.error(
          `⚠️  Failed to delete webhook from Jira, but clearing from database`,
          { error, integrationId },
        );
      }
      ...
      logger.error(
        `❌ Error tearing down webhook for integration ${integrationId}`,
        {
          error,
          integrationId,
        },
...
export class JiraEvent extends InputEvent {
  ...
  formatForAgentRunner(): string {
    ...
    sections.push(
      `User: ${this.data.user.displayName} (${this.data.user.emailAddress})`,
    );
    ...
    if (this.data.comment) {
      ...
      commentSections.push(
        `Comment on Issue: ${this.data.issue?.key || "Unknown"}`,
      );
      commentSections.push(
  ...
  matchesAgentTrigger(automationInput: AgentTriggerWithConfigs): boolean {
    logger.debug(
      `Checking if Jira event matches automation input: ${automationInput.config_type}`,
      { configType: automationInput.config_type },
    );
  ...
  createTriggerMetadata(): RunHistoryTrigger {
    ...
    if (issue) {
      ...
      if (commentIssue) {
        ...
        source =
          commentIssue.fields.project.name || commentIssue.fields.project.key;
      } else {
        title = "Comment";
        source = "Jira";
      }
