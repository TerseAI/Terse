...
export class SlackIntegrationManager
  implements
    Integration<
      SlackIntegration,
      SlackMessageEvent,
      typeof SlackIntegrationMetadata
    >,
    OAuthIntegrationInstallation<IntegrationType.SLACK>
{
  constructor() {}
  ...
  async getInstallationUrl(
    ...
    const scope =
      "channels:history,channels:manage,groups:history,groups:write,im:history,im:write,mpim:history,mpim:write,channels:read,groups:read,mpim:read,im:read,users:read,chat:write,app_mentions:read,reactions:read,reactions:write";
    const user_scope = isBotUser
      ? ""
      : "channels:history,channels:read,groups:history,groups:read,im:history,im:read,mpim:history,mpim:read,users:read,channels:write,groups:write,mpim:write,im:write,chat:write,reactions:read,reactions:write";
...
export class SlackEvent extends InputEvent implements Identifiable {
  ...
  formatForAgentRunner(): string {
    ...
    return `
        ...
        ${
          this.data.threadTimestamp
            ? `Thread: ${this.data.threadTimestamp}`
            : ""
        }
  ...
  debugLog(): string {
    ...
    return `Slack Event: ${
      isDM ? "DM" : this.data.channelName || this.data.channelId
    } - ${this.data.userName || this.data.userId}}`;
  }
  ...
  matchesAgentTrigger(agentTrigger: AgentTriggerWithConfigs): boolean {
    ...
    const matchesUserFilter =
      !slackConfig.user_ids ||
      slackConfig.user_ids.length === 0 ||
      slackConfig.user_ids.includes(this.data.userId);

    const matchesChannelOrGroup =
      isChannelOrGroup &&
      this.data.channelId === slackConfig.channel_id &&
      matchesUserFilter;
    const matchesDM =
      isDM && slackConfig?.listen_to_user_dms && matchesUserFilter;
...
function extractChannelName(
  ...
  if ("name" in channel && channel.name) {
    ...
    // Group DM
    const channelName =
      "name" in channel && channel.name ? channel.name : "Group Message";
 ...
 */
function extractUserName(userResult: {
  success: boolean;
  data?: { user?: SlackUser };
  error?: string;
}): string | undefined {
...
async function handleSlackMessage(
  ...
  try {
    ...
    const workspaceUserIntegrations =
      await db().user_slack_integrations.findMany({
        where: {
    ...
    const isInChannel = async (integration: UserSlackIntegrationWithUser) => {
      try {
        ...
        let membersRes:
          | Awaited<ReturnType<typeof botClient.conversations.members>>
          | undefined;
        try {
        ...
        if (
          ...
          return (
            channelMemberIds.includes(integration.authed_user_id) ||
            channelMemberIds.includes(integration.slack_integration.bot_user_id)
          );
        } else {
          const errorMsg =
            membersRes.error ||
            (membersRes.members?.length === 0 ? "no members" : "unknown error");
    ...
    logger.debug(
      `📡 Fetching additional Slack data for channel ${messageEvent.channel}, user ${messageEvent.user}, message ${messageEvent.ts}`,
      {
        channel: messageEvent.channel,
    ...
    const [channelInfo, userInfo, permalinkResult, fullMessageResult] =
    ...
    // Extract permalink
    const permalink = permalinkApiResult.success
      ? permalinkApiResult.data?.permalink
      : undefined;
    if (permalink) {
    ...
    if (
      fullMessageApiResult.success &&
      fullMessageApiResult.data?.messages?.[0]
    ) {
      ...
      try {
        ...
        await runWithUserContext(user, async () => {
            ...
            if (
              ...
              logger.info(
                `User ${user.email}: ${results.length} automation(s) matched`,
                {
                  userId: userSlackIntegration.user.id,
                  email: user.email,
 ...
 */
type SlackApiSettledResult<T = unknown> = PromiseSettledResult<
  SlackApiResponse<T>
>;
