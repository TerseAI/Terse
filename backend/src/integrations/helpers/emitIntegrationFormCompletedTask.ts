...
export async function emitIntegrationFormCompletedTaskIfNeeded(
    ...
    try {
        ...
        if (instances.length === 0) {
            logger.warn("[Integration Form] No integration instances found after submission", {
                userId,
                integrationType: defaultIntegrationType
            })
            return
        ...
        const integrationType = (statePayload.integrationType as IntegrationType) || defaultIntegrationType

        logger.info("[Web Chat Integration Form] Form submission successful, emitting task", {
            integrationType,
            integrationId,
            userId,
