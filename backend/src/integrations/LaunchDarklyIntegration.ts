...
export class LaunchDarklyIntegrationManager
  implements
    Integration<
      LaunchDarklyIntegration,
      never,
      typeof LaunchDarklyIntegrationMetadata,
      LaunchDarklyProject
    >,
    FormIntegrationInstallation<IntegrationType.LAUNCHDARKLY>
{
  constructor() {}
          ...
          data: {
            details:
              validationResponse.status === 401
                ...
                : `API returned status ${validationResponse.status}`,
          },
        };
      }
      ...
      try {
        ...
        if (tokensResponse.ok) {
          ...
          const tokens = Array.isArray(tokensData)
            ? tokensData
            : tokensData.items || tokensData.tokens || [];
