    ...
    FormIntegrationInstallation<IntegrationType.CRON_JOB>
{
  ...
  async setupAgentTrigger(
    ...
    if (!agentTrigger.time_trigger_config) {
      logger.warn(
        "⚠️  No time_trigger_config found for channel input, skipping scheduler setup",
        {
    ...
    if (!cronExpression) {
      logger.warn(
        "⚠️  No cron expression configured, skipping scheduler setup",
        {
          inputId: agentTrigger.id,
        },
      );
  ...
  private getWebhookUrl(inputId: string): string {
    ...
    return `${baseUrl}${ApiRoutes.WEBHOOKS.SCHEDULE_BY_INPUT_ID.build(
      inputId,
    )}`;
  }
}
...
function isSchedulerJobNotFoundError(error: unknown): boolean {
  ...
  return (
    ...
    anyError.message.includes("NOT_FOUND")
  );
}
