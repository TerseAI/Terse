            ...
            if (!accessToken) {
              ...
              if (event.type === "Issue" && event.data?.id) {
                try {
                  ...
                  logger.warn(
                    `⚠️  [LINEAR INTEGRATION MANAGER] Could not enrich issue context`,
                    {
                      error,
...
export class LinearEvent extends InputEvent {
  ...
  debugLog(): string {
    if (this.data.type === "Issue" && this.data.data) {
      ...
      const comment = this.data.data as any; // Comment events have different structure
      return `Linear ${this.data.type} Event: Comment on issue ${
  ...
  matchesAgentTrigger(agentTrigger: AgentTriggerWithConfigs): boolean {
    logger.debug(
      `Checking if Linear event matches channel input: ${agentTrigger.config_type}`,
      {
  ...
  createTriggerMetadata(): RunHistoryTrigger {
    ...
    if (this.data.type === "Issue" && this.data.data) {
      ...
      subheader = `${this.data.data.identifier} - ${
        this.data.data.state?.name || "Unknown"
      }`;
