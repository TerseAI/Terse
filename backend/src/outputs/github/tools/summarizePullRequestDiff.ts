...
import { AgentType, builderProviderDataModelSettings, runnerFactory } from "../../../modules/agents/runner"
import { buildUserMessage } from "../../../modules/agents/userMessage"
...
You can optionally provide high-level context about what you're looking for in the PR, which will help the sub-agent focus its analysis.`,
    ...
    execute: async ({ repository, pullNumber, page, context }, runContext) => {
        ...
        try {
            ...
            const history: AgentInputItem[] = [buildUserMessage(userPrompt)]
...
function buildSummarizerUserPrompt(prDiff: any, context?: string): string {
    const filesList = prDiff.filesChanged.map((file: any) => `  - ${file.filename} (${file.status}): +${file.additions}/-${file.deletions} lines`).join("\n")
