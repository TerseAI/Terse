...
async function findUserAccountId(cloudId: string, accessToken: string, email: string): Promise<string | null> {
    try {
        const response = await fetch(`https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/user/search?query=${encodeURIComponent(email)}`, {
            method: "GET",
            headers: {
...
async function getAvailableTransitions(cloudId: string, accessToken: string, issueKey: string): Promise<Array<{ id: string; name: string }>> {
    try {
        const response = await fetch(`https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/issue/${issueKey}/transitions`, {
            method: "GET",
            headers: {
                Authorization: `Bearer ${accessToken}`,
                Accept: "application/json"
...
- Update description: Set description (supports plain text or markdown format)`,
    parameters: z.object({
        ...
        labels: z
            ...
            .optional()
            .describe("The labels for the ticket (array of label names). This replaces all existing labels."),
        dueDate: z
            .string()
            .nullable()
            .optional()
    ...
    execute: async ({ integrationId, issueKey, title, description, status, assignee, priority, labels, dueDate }, runContext?: RunContext<SessionWithTracking<Session>>) => {
        ...
        try {
            ...
            if (Object.keys(fields).length > 0) {
                const updateResponse = await fetch(`https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/issue/${issueKey}`, {
                    method: "PUT",
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
                        Accept: "application/json",
            ...
            if (status) {
                ...
                const matchingTransition = transitions.find(t => t.name.toLowerCase() === status.toLowerCase())

                if (matchingTransition) {
                    const transitionResponse = await fetch(`https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/issue/${issueKey}/transitions`, {
                        method: "POST",
                        headers: {
                            Authorization: `Bearer ${accessToken}`,
                            Accept: "application/json",
            ...
            const issueResponse = await fetch(`https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/issue/${issueKey}`, {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                    Accept: "application/json"
