...
async function findUserAccountId(
  cloudId: string,
  accessToken: string,
  email: string,
): Promise<string | null> {
  try {
    const response = await fetch(
      `https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/user/search?query=${encodeURIComponent(
        email,
      )}`,
      {
        method: "GET",
    ...
    if (!response.ok) {
      logger.warn(`Failed to search for user by email: ${email}`, {
...
async function getAvailableTransitions(
  ...
  try {
    ...
    if (!response.ok) {
      logger.warn(`Failed to get transitions for issue: ${issueKey}`, {
        status: response.status,
      });
      return [];
    }
...
- Update description: Set description (supports plain text or markdown format)`,
  parameters: z.object({
    integrationId: z
      .string()
      .describe("The integration ID of the Atlassian/Jira integration to use."),
    issueKey: z
      .string()
      .describe(
        'The key of the Jira issue to update (e.g., "PROJ-123"). This is required.',
      ),
    title: z
      .string()
      .nullable()
      .optional()
      .describe("The issue title/summary."),
    description: z
      .string()
      .nullable()
      .optional()
      .describe("The issue description in plain text or markdown format."),
    status: z
      .string()
      .nullable()
      .optional()
      .describe(
        'The status name to transition to (e.g., "In Progress", "Done", "To Do").',
      ),
    assignee: z
      .union([
        z.object({
      ...
      .optional()
      .describe("The assignee of the ticket. Set to null to unassign."),
    priority: z
      .union([z.number(), z.null()])
      .optional()
      .describe("The priority of the ticket (number, typically 1-5)."),
    labels: z
      .union([z.array(z.string()), z.null()])
      .optional()
      .describe(
        "The labels for the ticket (array of label names). This replaces all existing labels.",
      ),
    dueDate: z
      .string()
      .nullable()
      .optional()
      .describe(
        'The due date for the ticket in format "yyyy-MM-dd" (e.g., "2024-12-31"). Note: Jira requires the due date format to be yyyy-MM-dd. Set to null to remove due date.',
      ),
  }),
  ...
  execute: async (
    ...
    logger.debug("🛠️ Executing jira_update_ticket tool", {
      integrationId,
      issueKey,
      updates: {
        title,
    ...
    const accessToken = await integrationManager.getAccessToken(
      integrationId,
      userId,
    );
    if (!accessToken) {
    ...
    if (!integration || !integration.cloud_id) {
      throw new Error(
        `Atlassian integration details not found for integrationId: ${integrationId}`,
      );
    }
    ...
    try {
      ...
      if (assignee !== undefined) {
        if (assignee === null) {
          ...
          const accountId = await findUserAccountId(
            cloudId,
            accessToken,
            assignee.email,
          );
      ...
      if (dueDate !== undefined) {
        if (dueDate === null) {
          ...
          logger.warn(
            `Invalid due date format: "${dueDate}". Expected format: yyyy-MM-dd. Skipping due date update.`,
          );
        }
      }
      ...
      if (Object.keys(fields).length > 0) {
        ...
        if (!updateResponse.ok) {
          ...
          logger.error("❌ Error updating Jira issue fields", {
      ...
      if (status) {
        ...
        if (matchingTransition) {
          ...
          if (!transitionResponse.ok) {
            ...
            logger.error("❌ Error transitioning Jira issue", {
          ...
          logger.warn(
            `Could not find transition to status "${status}". Available transitions: ${transitions
              .map((t) => t.name)
              .join(", ")}`,
          );
      ...
      const issueData = {
        ...
        description:
          updatedIssue.fields.description || description || undefined,
        state: updatedIssue.fields.status
          ...
          : { id: "", name: "Unknown" },
        priority: updatedIssue.fields.priority
          ? parseInt(updatedIssue.fields.priority.id)
          : priority || undefined,
      ...
      const updateSummary =
        Object.keys(fields).join(", ") + (status ? ", status" : "");
      const action = {
