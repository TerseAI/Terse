...
- Combined: "project = PROJ AND status = 'In Progress' AND text ~ 'bug'"`,
  parameters: z.object({
    ...
    jql: z
      ...
      .optional()
      .describe(
        "JQL (Jira Query Language) query to search for issues. If not provided, will search all issues.",
      ),
    text: z
      .string()
      .nullable()
      .optional()
      .describe(
        'Text to search for in issue titles and descriptions. If provided, will be converted to JQL: text ~ "search term"',
      ),
    projectKey: z
      .string()
      .nullable()
      .optional()
      .describe('Filter by Jira project key (e.g., "PROJ", "TEAM")'),
    assigneeEmail: z
      .string()
      .nullable()
      .optional()
    ...
    status: z
      ...
      .nullable()
      .optional()
      .describe('Filter by status name (e.g., "In Progress", "Done", "To Do")'),
    limit: z
      .number()
      .nullable()
      .optional()
      .describe(
        "Maximum number of issues to return. Defaults to 50 if not provided.",
      ),
    nextPageToken: z
      .string()
      .nullable()
      .optional()
      .describe(
        "Token from a previous search response to retrieve the next page of results. Use the nextPageToken value from the previous response to paginate through all results.",
      ),
  }),
  execute: async (
    {
    ...
    try {
      ...
      if (assigneeEmail) {
        const assigneeQuery = `assignee = "${assigneeEmail.replace(
          /"/g,
          '\\"',
        )}"`;
        if (finalJql) {
      ...
      const issues = issuesArray.map((issue: any) => {
        ...
        return {
          ...
          priority: issue.fields.priority
            ? parseInt(issue.fields.priority.id)
            : undefined,
      ...
      const action = {
        ...
        details: `Found ${issues.length} issue(s) matching search criteria${
          total > issues.length ? ` (${total} total)` : ""
        }`,
