...
- Ensure you have the correct projectKey for the project where you want to create the issue`,
  parameters: z.object({
    ...
    projectKey: z
      .string()
      .describe(
        'The Jira project key (e.g., "PROJ", "TEAM"). This is required.',
      ),
    issueType: z
      .string()
      .nullable()
      .optional()
      .describe(
        'The Jira issue type (e.g., "Task", "Bug", "Story", "Epic", "Subtask", "Improvement", "New Feature")',
      )
      .default("Task"),
    assignee: z
      .union([
    ...
    dueDate: z
      ...
      .optional()
      .describe(
        'The due date for the ticket in format "yyyy-MM-dd" (e.g., "2024-12-31"). Note: Jira requires the due date format to be yyyy-MM-dd.',
      ),
  }),
  ...
  execute: async (
    ...
    logger.debug("🛠️ Executing jira_create_ticket tool", {
      ...
      projectKey,
      issueType,
      otherFields: { description, assignee, priority, labels, dueDate },
    });
    ...
    try {
      ...
      if (
        ...
        dueDate &&
        (dueDate.trim() === "" || !/^\d{4}-\d{2}-\d{2}$/.test(dueDate))
      ) {
        logger.warn(
          `Invalid due date format: "${dueDate}". Expected format: yyyy-MM-dd. Skipping due date.`,
        );
      }
      ...
      if (!issueResponse.ok) {
        logger.warn("Could not fetch full issue details, using basic info", {
      ...
      const issueData = {
        ...
        state: fullIssue?.fields?.status
          ...
          : { id: "", name: "Unknown" },
        priority: fullIssue?.fields?.priority
          ? parseInt(fullIssue.fields.priority.id)
          : priority || undefined,
