...
export const snowflakeExecuteQueryTool = tool({
    ...
    description:
        "Execute a read-only SQL query against a Snowflake data warehouse. Returns rows and column metadata. SQL safety is enforced by the Snowflake role configured for the integration — use a read-only role. Include a LIMIT clause in your query to control the number of rows returned.",
    parameters: z.object({
        integrationId: z.string().describe("The integration ID of the Snowflake connection to use."),
        query: z.string().describe("The SQL query to execute. Should be a read-only SELECT statement. Include a LIMIT clause to control result size.")
    }),
