...
export class ConfluenceOutput extends Output<ConfluenceConfig> {
  ...
  protected getSystemInstructionsForConfigs(
    ...
    for (const config of configs) {
      ...
      configList.push(
        `  • Integration ID: ${config.integration_id} - Page Name: ${
          pageName || "N/A"
        }, Page ID: ${pageId || "N/A"}`,
      );
    }
...
This tool returns the current state of the Confluence page including all metadata, properties, and content body.`,
  parameters: z.object({
    integrationId: z
      .string()
      .describe(
        "The integration ID of the Atlassian/Confluence integration to use.",
      ),
  ...
  execute: async (
    ...
    if (!accessToken) {
      throw new Error(
        `Atlassian integration not found or access denied for integrationId: ${integrationId}`,
      );
    }
    ...
    if (!integration || !integration.cloud_id) {
      throw new Error(
        `Atlassian integration cloud ID not found for integrationId: ${integrationId}`,
      );
    }

    if (!integration.base_url) {
      throw new Error(
        `No base_url found in Atlassian integration for integrationId: ${integrationId}`,
      );
    }
    ...
    try {
      ...
      const { ancestors, descendants } =
        extractAncestorsAndDescendants(pageInfo);
...
To find the correct position, first call confluence_query_page to see the page content, then identify the text range you want to comment on.`,
  parameters: z.object({
    ...
    comment_text: z
      .string()
      .describe("The text content of the comment to add."),
    ...
    start_position: z
      ...
      .optional()
      .describe(
        "Optional: The start character position (offset) in the page storage format where the comment should be attached. Required if text_to_comment_on is not provided.",
      ),
    end_position: z
      .number()
      .nullable()
      .optional()
      .describe(
        "Optional: The end character position (offset) in the page storage format where the comment should be attached. Required if text_to_comment_on is not provided.",
      ),
  }),
  ...
  execute: async (
    ...
    logger.debug(
      "[Confluence Add Comment] Executing confluence_add_comment tool",
      {
        integrationId,
        pageId,
        comment_text,
    ...
    try {
      ...
      if (
        ...
        if (plainTextIndex === -1) {
          throw new Error(
            `Could not find the text "${text_to_comment_on}" in the page content. Please check the text and try again, or use start_position and end_position instead.`,
          );
        }
      ...
      if (startPos === undefined || endPos === undefined) {
        throw new Error(
          "Either text_to_comment_on or both start_position and end_position must be provided.",
        );
      }
      ...
      const selectedText =
        text_to_comment_on && text_to_comment_on !== null
          ? text_to_comment_on
          : stripHtmlTags(storageContent.substring(startPos, endPos));

      // Calculate match index and count
      const plainTextStartPos = mapStorageToPlainTextPosition(
        storageContent,
        startPos,
      );
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to add inline comment: ${response.status} ${response.statusText} - ${errorText}`,
        );
      }
      ...
      throw new Error(
        `Failed to add Confluence inline comment: ${
          error instanceof Error ? error.message : String(error)
        }`,
      );
    }
  ...
  return {
    ...
    space: pageInfo.space
      ? {
          ...
          key:
            pageInfo.space.key || pageInfo.spaceId || String(pageInfo.space.id),
          name:
            pageInfo.space.name ||
            pageInfo.spaceId ||
            String(pageInfo.space.id),
    ...
    url:
      pageInfo._links?.webui ||
      (pageInfo._links?.base && pageInfo._links?.webui
        ? pageInfo._links.base + pageInfo._links.webui
        : undefined),
  };
}
...
function extractAncestorsAndDescendants(pageInfo: any): {
  ...
  const descendants: AncestorOrDescendant[] =
    pageInfo.descendants &&
    typeof pageInfo.descendants === "object" &&
    "results" in pageInfo.descendants
      ? (pageInfo.descendants as { results: any[] }).results.map(
          (descendant: any) => ({
...
function mapPlainTextToStoragePosition(
  ...
  for (
    let i = 0;
    i < storageContent.length && plainTextPos < plainTextPosition;
    i++
  ) {
    if (storageContent[i] === "<") {
      inTag = true;
...
function mapPlainTextRangeToStoragePositions(
  ...
  const startPos = mapPlainTextToStoragePosition(
    storageContent,
    plainTextStart,
  );
  ...
  for (
    let i = startPos;
    i < storageContent.length && endPlainTextCount < endPlainTextPos;
    i++
  ) {
    if (storageContent[i] === "<") {
      inTag = true;
...
function findTextMatches(
  ...
  while (
    (searchIndex = plainTextContent.indexOf(selectedText, searchIndex)) !== -1
  ) {
