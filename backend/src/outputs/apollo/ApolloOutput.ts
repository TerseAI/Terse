...
export class ApolloOutput extends Output<ApolloOutputConfig> {
    ...
    protected getSystemInstructionsForConfigs(configs: ApolloOutputConfig[]): string {
        ...
        return [
            ...
            "Enrich people (apollo_enrich_person, apollo_bulk_enrich_people), enrich companies (apollo_enrich_organization), search for prospects (apollo_search_people), and list job postings as hiring signals (apollo_list_job_postings).",
            "Enrichment consumes Apollo export credits per matched record — prefer apollo_bulk_enrich_people for lists and only enrich records you will use.",
            "apollo_search_people is credit-free but returns no emails; pass result ids to apollo_bulk_enrich_people to unlock contact data.",
