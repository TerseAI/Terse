...
function verifySignature(headerSignatureString: string | undefined, rawBody: Buffer): boolean {
    ...
    try {
        ...
        const computedSignature = crypto.createHmac("sha256", LINEAR_WEBHOOK_SECRET).update(rawBody).digest()
...
export async function getLinearTeams(req: Request, res: Response) {
    ...
    try {
        const response = await fetchLinearTeams(user.organizationId, integrationId)
...
export async function fetchLinearTeams(organizationId: string, integrationId: string): Promise<LinearTeam[]> {
...
export async function getLinearProjects(req: Request, res: Response) {
    ...
    try {
        ...
        res.status(500).json({ error: "Failed to fetch Linear projects" })
    }
}
