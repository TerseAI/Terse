...
export class FeatureFlagService {
    ...
    async isFeatureFlagEnabled(flagKey: string, distinctId: string, properties?: Record<string, any>): Promise<boolean> {
        ...
        try {
            const isEnabled = await client.isFeatureEnabled(flagKey, distinctId, { personProperties: properties })
            return isEnabled ?? false
        } catch (error) {
            logger.error("Error checking feature flag", { error, flagKey, distinctId })
            return false
        }
    }
