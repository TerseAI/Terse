...
Use the schema's format_example field to construct properties correctly. Pay special attention to the difference between "select" and "status" types.`,
  ...
  execute: async ({}, runContext?: RunContext<NotionSession>) => {
    ...
    for (const [propertyName, propertyConfig] of Object.entries(
      databaseInfo.properties as Record<string, any>
    )) {
      ...
      switch (propertyConfig.type) {
        ...
        case "multi_select":
          baseSchema.options =
            propertyConfig.multi_select?.options?.map((opt: any) => opt.name) || [];
...
Use notion_query_database first to see existing property names and structure.`,
  ...
  execute: async ({ page_id, properties_json }, runContext?: RunContext<NotionSession>) => {
    ...
    const validPageId =
      page_id && page_id.length > 30 && !page_id.includes("/") && page_id !== "." ? page_id : null;
