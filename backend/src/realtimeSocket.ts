...
import {
  ...
  markRunFailed,
  storeChatEvent,
} from "./agent/AgentRunner/runHistory";
import { RunContext } from "./agent/AgentRunner/SystemPromptBuilder";
import {
  DirectiveTask,
  directiveTaskQueue,
} from "./agent/DirectiveAgent/DirectiveAgent";
...
export async function initializeRealtimeSocket(
  server: HttpServer,
): Promise<Server> {
  ...
  if (redisUrl && redisUrl.length > 0) {
    try {
      ...
      logger.warn(
        "REDIS_URL should be in format: redis://host:port or rediss://host:port",
      );
    }
  } else {
    logger.info(
  ...
  io.use(async (socket: Socket, next) => {
    ...
    try {
      ...
      if (!jwks) {
        logger.warn(
          "Socket.IO auth failed: JWKS not available (missing clientId)",
        );
  ...
  io.on(SocketEvents.CONNECT, (socket: Socket) => {
    ...
    const orgRoom = organizationId
      ...
      : null;
    const sessionRoom = workosSessionId
      ? SocketRooms.session(workosSessionId)
      : null;
    ...
    socket.on(
      SocketEvents.AGENT_CHAT_MESSAGE,
      async (payload: { runId: string | null; message: SendModelRequest }) => {
        const { runId, message } = payload;
        logger.info(
          `[agent:chat:message] Received message for runId: ${runId}`,
        ...
        const agent: AgentWithRelations | null =
        ...
        if (!agent) {
          logger.error(
            `[agent:chat:message] Agent not found for automation id: ${runRecord.automation.id}`,
            { automationId: runRecord.automation.id, userId },
          );
          return;
        }
        ...
        if (!agent.outputs || agent.outputs.length === 0) {
          logger.error(
            `[agent:chat:message] No output integrations found for agent: ${agent.id}`,
        ...
        try {
          ...
          logger.error(
            `[agent:chat:message] Failed to create outputs for agent: ${agent.id}`,
            { error, agentId: agent.id, userId },
          );
          return;
        }
        ...
        const userMessageEventId = await storeChatEvent(
          runId,
          userMessageEvent,
        );
        if (organizationId) {
        ...
        try {
          ...
          logger.error(
            `[agent:chat:message] Error running agent: ${errorMessage}`,
            { error, runId, agentId: agent.id, userId },
          );
