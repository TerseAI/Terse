  ...
  for (const row of paginatedResults) {
    ...
    if (row.sub_activity_summary) {
      ...
      if (row.commit_sha) {
        ...
        const existingCommit = subActivity.commits.find((c) => c.sha === row.commit_sha);
...
export async function getDailyActivitySummary(req: Request, res: Response) {
  ...
  try {
    ...
    if (eventCount === 0) {
      ...
      try {
        summary = await callOpenAISummary(
          `Summarize the following activity events for a daily team update.\n\n${prompt}`
        );
