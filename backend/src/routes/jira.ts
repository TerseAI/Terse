...
export const getJiraCredentials = async (req: Request, res: Response) => {
  ...
  res
    .status(200)
    .json({ apiKey: creds.api_token, baseUrl: creds.base_url, email: creds.jira_user_email });
};
...
export const indexJiraTicket = async (jiraUserId: string, body: JiraWebhookPayload) => {
  try {
    ...
    console.log(
      chalk.green("✅ Indexed Jira ticket"),
      chalk.yellow(body.issue.key),
      `(ID: ${ticketId})`
    );
