...
export const fetchNotionResources = async (organizationId: string, integrationId: string, search: string = "", typeFilter?: string): Promise<NotionResourcesResponse> => {
    if (!integrationId) {
    ...
    const integrationToken = await getSecret("notion_integrations", integration.id, "integration_token", integration.integration_token)
    if (!integrationToken) {
...
export const getNotionResources = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const response = await fetchNotionResources(user.organizationId, integrationId, search, typeFilter)
