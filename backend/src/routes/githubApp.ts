...
export async function getCurrentGithubIntegration(req: Request, res: Response) {
  ...
  const user_github_relation: UserGithubRepository | null =
    await db().user_github_repositories.findFirst({ where: { user_id: user.id } });
  ...
  const repository: GithubRepository | null = await db().github_repositories.findUnique({
    where: { id: user_github_relation.github_repository_id },
  });
...
export async function githubAppInstallationCallback(req: Request, res: Response) {
  ...
  let user: User | null = await db().users.findUnique({
    where: { github_username: body.username },
  });
  if (!user) {
...
export async function githubAppInstallationDeleted(req: Request, res: Response) {
  ...
  const repositories: GithubRepository[] = await db().github_repositories.findMany({
  ...
  await db().user_github_repositories.deleteMany({
    where: { github_repository_id: { in: repositories.map((repo) => repo.id) } },
  });
...
export async function githubAppRecievedCommit(req: Request, res: Response) {
  ...
  // get the repository
  const repository: GithubRepository | null = await db().github_repositories.findFirst({
    where: {
...
async function resolveUserGithubRelation(
  user: User,
  username: string,
  repositoryName: string,
  installationId: number
  ...
  return await db().$transaction(async (tx) => {
    ...
    if (!repository) {
      console.log(
        chalk.yellow(
          "Drift detected. This repository is not in our DB but it is a registered repository in the github app. Creating it..."
        )
      );
...
export async function githubAppUnifiedEvent(req: Request, res: Response) {
  ...
  console.log(
    chalk.blue("githubAppUnifiedEvent"),
    body.eventType,
    body.repositoryName,
    body.username
  );
  ...
  try {
    ...
    let user: User | null = await db().$transaction(async (tx) => {
      ...
      if (!foundUser) {
        ...
        console.log(
          chalk.yellow("User not found, creating placeholder user with fake email " + email)
        );
    ...
    const repository: GithubRepository = await resolveUserGithubRelation(
      user,
      username,
      repositoryName,
      installationId
    ...
    console.log(
      chalk.blue("Processing event for user:", user.github_username, "team:", session.teamId)
    );
    ...
    res
      .status(500)
      .json({
        message: "Error processing GitHub event",
        error: error instanceof Error ? error.message : "Unknown error",
      });
  }
}
    ...
    data: {
      ...
      event_type:
        event.eventType === "push"
          ? "PUSH"
          : event.eventType === "pull_request.opened"
            ? "PULL_REQUEST_OPENED"
            : event.eventType === "pull_request.synchronize"
              ? "PULL_REQUEST_UPDATED"
              : event.eventType === "pull_request.merged"
                ? "PULL_REQUEST_MERGED"
