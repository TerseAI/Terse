...
if (!process.env.GMAIL_CLIENT_ID) {
  throw new Error(
    "GMAIL_CLIENT_ID is not set in environment variables (required for Google login)"
  );
}
if (!process.env.GMAIL_CLIENT_SECRET) {
  throw new Error(
    "GMAIL_CLIENT_SECRET is not set in environment variables (required for Google login)"
  );
}
...
export async function googleCallback(req: Request, res: Response) {
  ...
  console.log(
    chalk.blue("🔗 Google OAuth callback received:"),
    chalk.cyan(JSON.stringify(req.query, null, 2))
  );
