  ...
  const agent = await prisma.$transaction(async (tx) => {
    ...
    if (knowledgeBases && knowledgeBases.length > 0) {
      for (const kb of knowledgeBases) {
        ...
        await createKnowledgeBaseConfig(
          tx,
          newKnowledgeBase.id,
          kb.config,
          userId
    ...
    if (toolApprovals && toolApprovals.length > 0) {
      const uniqueToolApprovals =
        validateAndDeduplicateToolApprovals(toolApprovals);
...
export async function updateAgentForUser(
  ...
  const prisma = db();
  const existingAgent: AgentWithTriggerRelations | null =
    await prisma.automations.findFirst({
      where: {
        id: agentId,
        organization_id: organizationId,
...
export async function getUserAgents(req: Request, res: Response) {
  ...
  try {
    ...
    if (
      agents.length > 0 &&
      !agents.some((agent) => agent.outputs && agent.outputs.length > 0)
    ) {
  ...
  try {
    ...
    const response = agents.map((agent) => {
      ...
      return {
        ...
        lastEventProcessedAt: lastEventTimestamp
...
export async function updateAgent(req: Request, res: Response) {
  ...
  try {
    const { id } = await updateAgentForUser(
      userId,
      organizationId,
      agentId,
      update
...
function transformAgentToFrontendFormat(
  ...
  return {
    ...
    knowledgeBases:
      (agent as any).knowledge_bases &&
      (agent as any).knowledge_bases.length > 0
