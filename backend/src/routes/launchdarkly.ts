...
export async function getLaunchDarklyProjects(req: Request, res: Response) {
    ...
    try {
        ...
        const response = await fetchLaunchDarklyProjects(req.session.user.organizationId, integrationId)
...
export async function fetchLaunchDarklyProjects(organizationId: string, integrationId: string, query: string = ""): Promise<{ projects: Array<{ key: string; name: string }> }> {
    ...
    const apiKey = await getSecret({ type: "integration", params: { integrationType: IntegrationType.LAUNCHDARKLY, recordId: integration.id, field: SecretField.ApiKey } })
    if (!apiKey) {
    ...
    if (query) {
        ...
        projects = projects.filter((p: any) => p.name?.toLowerCase().includes(queryLower) || p.key?.toLowerCase().includes(queryLower))
    }
