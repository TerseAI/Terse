...
export async function handleGmailWebhook(req: Request, res: Response) {
  ...
  try {
    ...
    const decoded: GmailWebhookData = JSON.parse(Buffer.from(message.data, "base64").toString());
        ...
        for (const messageId of messageIds) {
          ...
          if (parsedEmail) {
            ...
            if (lastProcessedDate && emailDate <= lastProcessedDate) {
              console.log(
                chalk.gray(`Skipping old message ${parsedEmail.id} from ${emailDate.toISOString()}`)
              );
