...
import { GithubEventData, githubEventDataSchema } from "terse-types"
...
export async function processRepository(repositoryData: Repository, user: User, installationId: number): Promise<{ name: string; status: string; error?: string }> {
