...
export async function fetchConfluenceResources(
  ...
  if (!searchResponse.ok) {
    ...
    throw new Error(
      `Confluence Search API error: ${searchResponse.status} ${searchResponse.statusText} - ${errorText}`,
    );
  }
 ...
 */
function mapSearchResultsToConfluencePages(
  ...
  return results
    .map((result) => {
      ...
      if (missingFields.length > 0) {
        logger.warn(
          `Missing fields for search result "${
