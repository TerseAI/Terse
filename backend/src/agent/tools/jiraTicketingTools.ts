...
const searchJiraTicketTool = tool({
  ...
  parameters: z.object({
    ...
    assigneeEmails: z
      ...
      .describe("MUST BE A VALID EMAIL. Filter by assignee emails."),
    createdByEmails: z
      .union([z.array(z.string()), z.null()])
      .describe("MUST BE A VALID EMAIL. Filter by created by emails."),
...
const getJiraIssueTypesTool = tool({
  ...
  parameters: z.object({
    projectKey: z
      ...
      .describe("Optional project key to get issue types for a specific project"),
  }),
...
const createJiraTicketTool = tool({
  ...
  parameters: z.object({
    ...
    estimate: z
      ...
      .describe("The time estimate for the ticket (in seconds)"),
    dueDate: z
      .union([z.string(), z.null()])
      .describe("The due date for the ticket (YYYY-MM-DD format)"),
    associatedCommits: z
  ...
  execute: async (
    ...
    if (!project) {
      console.error(
        chalk.red.bold(
          `❌ Project with key "${projectKey}" not found. Available projects:`,
          userContext.teams.map((t) => t.key).join(", ")
        )
      );
...
const commentOnJiraTicketTool = tool({
  ...
  parameters: z.object({
    issueId: z
      .string()
      .describe('The ID or key of the Jira issue to comment on (e.g., "PROJ-123")'),
