      ...
      .array(
        z.object({
          ...
          associatedCommits: z
            .array(z.number())
            .describe(
              "The indices of commits to associate with this sub activity (0-based, from the event context)"
            ),
          associatedPullRequests: z
            ...
            .describe(
              "The pull requests to associate with this sub activity. OK to be empty if no pull requests are associated."
            ),
        })
      )
      .describe("A list of summaries for each sub activity"),
    associatedProjectDescription: z
      .string()
      .describe(
        "The description of the project to associate with this event. OK to be empty if no project is associated."
      ),
    associatedTicketDescription: z
      .string()
      .describe(
        "The description of the ticket to associate with this event. OK to be empty if no ticket is associated."
      ),
  }),
  execute: async (
    { summary, subActivitySummaries, associatedProjectDescription, associatedTicketDescription },
    runContext?: RunContext<SessionWithTracking>
  ) => {
    ...
    for (const subActivitySummary of subActivitySummaries) {
      console.log(
        "Creating sub activity summary associated commits",
        subActivitySummary.associatedCommits
      );
    ...
    const overview: ActivityOverview = {
      ...
      project_activity_events:
        associatedProjectDescription && runContext.context.commitContext?.project
          ? [
              {
      ...
      ticket_activity_events:
        associatedTicketDescription && runContext.context.commitContext?.ticket
          ? [
              {
