...
const searchTicketTool = tool({
  ...
  parameters: z.object({
    ...
    assigneeEmails: z
      ...
      .describe(
        "MUST BE A VALID EMAIL. Filter by assignee emails. You should use this if you are looking for a specific user. It helps narrow down the results tremendously."
      ),
    createdByEmails: z
      .union([z.array(z.string()), z.null()])
      .describe(
        "MUST BE A VALID EMAIL. Filter by created by emails. You should use this if you are looking for a specific user. It helps narrow down the results tremendously."
      ),
...
const createTicketTool = tool({
  ...
  parameters: z.object({
    ...
    labels: z
      ...
      .describe("The labels for the ticket"),
    ...
    project: z
      ...
      .describe("The project for the ticket"),
    milestone: z
      .union([
        z.object({
      ...
      .describe("The milestone for the ticket"),
    associatedCommits: z
      .union([z.array(z.number()), z.null()])
      .describe(
        "The indices of commits to associate with this ticket (0-based, from the event context)"
      ),
  }),
  execute: async (
    {
    ...
    if (associatedCommits && associatedCommits.length > 0) {
      ...
      if (commitContext) {
        const commitAssociations = associatedCommits
          .map((index) => {
        ...
        if (commitAssociations.length > 0) {
          await ticketManager.associateCommitsToTicket(
            ticket.id,
            commitAssociations,
...
const updateTicketTool = tool({
  ...
  parameters: z.object({
    ...
    state: z
      .string()
      .describe(
        "The state of the issue. This is just an ID from the user context. The name is not important."
      ),
    assignee: z
      .union([
        z.object({
