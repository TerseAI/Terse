...
class SlackChatInterface extends ChatInterface {
    ...
    async getUserTimezone(): Promise<string | null> {
        ...
        try {
            ...
            logger.warn("Failed to fetch Slack user timezone", {
                error,
    ...
    private async handleOAuthIntegrationWithoutConfig(integration: IntegrationType, integrationManager: OAuthIntegrationInstallation<IntegrationType>): Promise<string> {
        try {
            ...
            if (!this.organizationId) {
                throw new Error("Organization ID is required for OAuth installation")
            }
            ...
            if (this.webClient && this.sessionId && this.channel) {
                try {
                    ...
                    logger.error("Failed to post preliminary message for OAuth button", {
                        error,
                        integration,
                        userId: this.userId
                    })
            ...
            const additionalStatePayload: Record<string, string> | undefined =
                this.sessionId && this.channel
                    ...
                    : undefined
            const installationDetails = await integrationManager.getInstallationUrl(this.userId, this.organizationId, undefined, additionalStatePayload)
    ...
    async promptForIntegration(integration: IntegrationType): Promise<string> {
        ...
        if (!this.organizationId) {
            logger.error("Cannot prompt for integration: organizationId is not available")
            return "Unable to get authorization URL. Please ensure you are properly authenticated."
        }
    ...
    async processMessageEnd(sessionId: string, finalOutput: string): Promise<void> {
        ...
        if (this.messageTsToReplace && this.webClient) {
            try {
                ...
                logger.info("Successfully replaced message", {
                    messageTs: this.messageTsToReplace
                })
                return
            } catch (error) {
