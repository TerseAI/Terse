...
import {
    INTEGRATION_COMPLETED_TASK_NAME,
    type IntegrationCompletedTask
} from "../../../integrations/IntegrationCompletedTask"
import {
    INTEGRATION_FORM_COMPLETED_TASK_NAME,
    type IntegrationFormCompletedTask
} from "../../../integrations/IntegrationFormCompletedTask"
import { integrationFormTaskQueue, integrationTaskQueue } from "../../../integrations/IntegrationTaskQueues"
import {
    SURVEY_ANSWER_TASK_NAME,
    type SurveyAnswerTask
} from "../SurveyAnswerTask"
...
abstract class ChatInterface {
     ...
     */
    protected async waitForIntegrationCompletion(
        integration: IntegrationType,
        options?: { timeoutMs?: number }
    ): Promise<{ integrationId: string; integrationType: IntegrationType }> {
        const timeoutMs = options?.timeoutMs ?? 120_000
        const org = this.organizationId ?? ""

        const oauthPredicate = (task: IntegrationCompletedTask) =>
            task.integrationType === integration &&
            task.userId === this.userId &&
            (task.statePayload?.organizationId ?? "") === org

        const formPredicate = (task: IntegrationFormCompletedTask) =>
            task.integrationType === integration &&
            task.userId === this.userId &&
            task.organizationId === (this.organizationId ?? "")
     ...
     */
    protected async waitForSurveyAnswer(
        questionId: string,
        options?: { timeoutMs?: number }
    ): Promise<string> {
        const timeoutMs = options?.timeoutMs ?? 120_000

        const predicate = (task: SurveyAnswerTask) =>
            task.questionId === questionId &&
            task.userId === this.userId &&
            task.sessionId === this.sessionId

        const task = await surveyAnswerTaskQueue.waitFor(
            SURVEY_ANSWER_TASK_NAME,
            predicate,
            { timeoutMs }
        )
