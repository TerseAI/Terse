...
export async function resumeChatAgentAfterFormCompletion(
    ...
    try {
        ...
        const slackChatInterface = new SlackChatInterface(channel, client, userId, userSlackIntegration.organization_id, userSlackIntegration.authed_user_id, chatId)
        ...
        if (messageTs) {
            await removeEyesReaction(client, {
                channel,
                ts: messageTs
            } as GenericMessageEvent)
        }

        logger.info("ChatAgent resumed after form completion", {
            userId,
            chatId,
            channel,
            integrationType,
        ...
        logger.error("Error resuming ChatAgent after form completion", {
            error,
            userId,
            chatId,
            channel,
...
async function showIntegrationSuccessMessage(client: WebClient, channel: string, messageTs: string, integrationName: string): Promise<void> {
    try {
        ...
        await addEyesReaction(client, {
            channel,
            ts: messageTs
        } as GenericMessageEvent)
    } catch (error) {
        logger.warn("Failed to update message after form completion", {
            error,
            messageTs
        })
    }
