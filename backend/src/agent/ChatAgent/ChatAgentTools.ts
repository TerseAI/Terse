...
import { applyAgentForUser, updateAgentForUser } from "../../routes/channels";
import type { AgentDraft } from "../../routes/channels";
...
export function buildChatAgentTools(chatInterface: ChatInterface): Tool<ChatAgentContext>[] {
    return [
        tool({
            name: 'applyAgent',
            description: 'Once you have all the information you need, you can use this tool to persist and apply the agent. You can use this to create and update agents. If you are creating, just leave the id empty.',
            parameters: z.object({
                agent: AgentSchema,
            ...
            execute: async ({ agent, id }, runContext?: RunContext<ChatAgentContext>): Promise<string> => {
                ...
                try {
                    ...
                    await chatInterface.buildButton("View Agent", `${frontendUrl}/app/agents/${result.id}`);
