...
function convertSingleItem(item: AgentInputItem, toolToIntegrationMap?: Map<string, string>): ModelEvent[] | null {
    ...
    if (isUserMessageItem(item)) {
        ...
        if (text) {
            // Filter out synthetic survey/integration answer messages - these are internal
            // messages generated when users respond to survey questions or integration prompts
            // and should not appear in the chat history UI
            if (isSyntheticSurveyAnswer(text)) {
                return null
            }
    ...
    if (isAssistantMessageItem(item)) {
        const text = extractTextFromMessageContent(item.content)
        if (text) {
            // Filter out internal survey/integration messages
            if (isInternalSurveyMessage(text)) {
                return null
            }
...
/**
 * Checks if a message is a synthetic survey/integration answer message.
 * These are generated by builderChatHandler when users respond to survey questions
 * or integration prompts (e.g., "The user answered: Yes").
 */
function isSyntheticSurveyAnswer(text: string): boolean {
    return text.startsWith("The user answered:")
}

/**
 * Checks if an assistant message is an internal survey/integration prompt message.
 * These are internal messages that should not appear in the chat history UI.
 */
function isInternalSurveyMessage(text: string): boolean {
    return text === "(Survey question sent; the user's answer will continue the conversation.)"
}
