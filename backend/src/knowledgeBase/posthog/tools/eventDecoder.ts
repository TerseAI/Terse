...
async function parseEncodedSnapshots(
    ...
    for (const line of items) {
        ...
        try {
            ...
            const windowIdNum = typeof windowIdStr === 'string' 
                ? registerFn(windowIdStr) 
                : windowIdStr;
            ...
            for (const event of snapshotLine.data) {
                ...
                if (
                    decompressedEvent &&
                    typeof decompressedEvent === 'object' &&
                    'type' in decompressedEvent &&
                    'timestamp' in decompressedEvent
                ) {
                    parsedLines.push({
 ...
 */
function extractMeaningfulEvents(
    events: RecordingSnapshot[],
                ...
                switch (event.data.source) {
                    case IncrementalSource.MouseInteraction:
                        ...
                        if (interactionType === MouseInteractions.Click || 
                            ...
                            interactionType === MouseInteractions.ContextMenu) {
                            summarized.push({
                                type: 'click',
                                timestamp: event.timestamp,
                                relativeTime,
...
export class PostHogSessionService {
    ...
    async getSessionSnapshots(sessionId: string): Promise<SessionSnapshot[]> {
        const response = await fetch(
            `${this.apiBaseUrl}/api/projects/${this.projectId}/session_recordings/${sessionId}/snapshots`,
            {
                headers: {
                    'Authorization': `Bearer ${this.apiKey}`,
                },
    ...
    async getSessionEvents(sessionId: string, sessionSnapshots: SessionSnapshot[]): Promise<string[]> {
        ...
        const response = await fetch(
            `${this.apiBaseUrl}/api/projects/${this.projectId}/session_recordings/${sessionId}/snapshots?source=${source}&start_blob_key=${startKey}&end_blob_key=${endKey}`,
    ...
    async fetchSessionEvents(
        ...
        const meaningfulEvents = extractMeaningfulEvents(
            allRawEvents,
            sessionStartTime,
            startSeconds,
            endSeconds
