...
export const getLaunchDarklyFlagDetailsTool = tool({
    ...
    execute: async (
        ...
        try {
                ...
                try {
                    ...
                    if (historyResponse.ok) {
                        ...
                        logger.debug("[LaunchDarkly] getFeatureFlagDetails - Audit log response parsed", {
                            ...
                            rawDataStructure: Array.isArray(auditLogData) ? `Array with ${auditLogData.length} items` : JSON.stringify(auditLogData).substring(0, 200)
                        })

                        // Extract audit log entries from response
                        const entries = Array.isArray(auditLogData) ? auditLogData : auditLogData.items || auditLogData.entries || []
            ...
            // Track the action
            const actionDetails = includeHistory ? `Retrieved details and ${historyEntries.length} history entries for flag "${flagKey}"` : `Retrieved details for flag "${flagKey}"`
