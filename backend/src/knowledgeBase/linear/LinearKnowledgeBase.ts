...
export class LinearKnowledgeBase extends KnowledgeBase<LinearKBConfig> {
    ...
    protected getSystemInstructionsForConfigs(configs: AgentKnowledgeBaseWithConfigs[]): string {
        ...
        sections.push(`
AVAILABLE TOOLS:
• linear_search_ticket: Search for Linear issues by query, or list issues by filters. Use to find tickets before reading details. Supports filtering by state using the stateNames parameter with values: Triage, Backlog, Todo, In Progress, In Review, Done, Canceled. You can omit or leave issueDescription empty to list all issues matching the provided filters.
