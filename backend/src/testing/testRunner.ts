...
function setupTestScenarios(testEnv: OwnerTestEnvironment) {
  ...
  testEnv.addScenario(
    OwnerTestEnvironment.createPushEventScenario("testuser", "test-repo", [
      {
        sha: "abc123",
  ...
  // PR opened scenario
  testEnv.addScenario(
    OwnerTestEnvironment.createPullRequestScenario(
      "pull_request.opened",
      "testuser",
      "test-repo",
      1
  ...
  const scenarioWithTickets: TestScenario = {
    ...
    ticketSystemState: {
      ...
      users: [{ id: "user-1", name: "Test User", email: "testuser@example.com" }],
      teams: [{ id: "team-1", name: "Backend Team", key: "BE" }],
      projects: [
        {
          id: "proj-1",
...
async function runInteractiveMode(testEnv: OwnerTestEnvironment) {
  ...
  console.log(
    "Use the predefined scenarios or modify the testRunner.ts file to add custom scenarios."
  );
}
