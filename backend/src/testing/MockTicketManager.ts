...
export class MockTicketManager implements TicketManager {
  ...
  private mockComments: Map<
    string,
    Array<{ id: string; authorId: string; body: string; createdAt: string }>
  > = new Map();
  ...
  async structuredSearch(query: string, options?: StructuredSearchOptions): Promise<Ticket[]> {
    ...
    if (options?.teamIds && options.teamIds.length > 0) {
      results = results.filter(
        (ticket) => ticket.team && options.teamIds!.includes(ticket.team.id)
      );
    }

    if (options?.assigneeEmails && options.assigneeEmails.length > 0) {
      results = results.filter(
        (ticket) => ticket.assignee && options.assigneeEmails!.includes(ticket.assignee.name)
      );
    }
    ...
    if (options?.priority && options.priority.length > 0) {
      results = results.filter(
        (ticket) => ticket.priority && options.priority!.includes(ticket.priority)
      );
    }

    if (options?.projects && options.projects.length > 0) {
      results = results.filter(
        (ticket) => ticket.project && options.projects!.includes(ticket.project.id)
      );
    }
  ...
  async updateTicket(id: string, input: UpdateTicketInput): Promise<Ticket> {
    ...
    if (input.assignee) {
      const assigneeUser = Array.from(this.mockUsers.values()).find(
        (u) => u.email === input.assignee
      );
  ...
  async searchItemsForProject(id: string): Promise<SearchItem[]> {
    ...
    const projectTickets = Array.from(this.mockTickets.values()).filter(
      (ticket) => ticket.project?.id === id
    );
  ...
  private initializeDefaultData() {
    ...
    const defaultProject: Project = {
      ...
      name: "Mock Project",
      description:
        "A project focused on implementing and maintaining secure user authentication and access control for the application.",
      updates: [],
    };
