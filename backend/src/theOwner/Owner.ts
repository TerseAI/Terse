...
class Owner {
  ...
  async handleUnifiedGitHubEvent(event: UnifiedGitHubEvent): Promise<ActivityOverview | null> {
    ...
    console.log(
      chalk.blue(`[${eventId}] The owner is handling a unified GitHub event`),
      event.eventType,
    ...
    console.log(
      chalk.blue(
        ...
        this.session.user.github_username,
        "Team ID:",
        this.session.teamId
      )
    );
    ...
    console.log(
      chalk.blue(`[${eventId}] Commits Shas`),
      event.commits.map((c) => c.sha)
    );
    ...
    if (!enrichmentResult) {
      console.warn(
        chalk.yellow.bold("✗ No enrichment result found. Unable to enrich activity event.")
      );
    }
    ...
    await analyzer.analyze(
      "No more commits to show. Generate a summary of the changes via the Tool CreateActionSummry."
    );
