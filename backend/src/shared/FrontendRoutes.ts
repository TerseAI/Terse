...
export const FrontendRoutes = {
  ...
  AGENTS: {
    ...
    NEW_WITH_TEMPLATE: {
      ...
      build: (templateId: string) =>
        `/app/agents/new/template/${encodeURIComponent(templateId)}`,
    ...
    HISTORY: (agentId: string) =>
      `/app/agents/${encodeURIComponent(agentId)}?tab=history`,
    RUN_HISTORY: (agentId: string, runId: string) =>
      `/app/agents/${encodeURIComponent(
        agentId,
      )}?tab=history&runId=${encodeURIComponent(runId)}`,
    // Relative path for external references (e.g., run history metadata)
    BY_ID_RELATIVE: (agentId: string) =>
