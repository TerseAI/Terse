...
export function createUpdatedApprovalMessage(options: {
  ...
  if ((options.status === 'rejected' || options.status === 'changes_requested') && options.rejectionReason) {
    ...
    blocks.push(
      createSectionBlock(`*${feedbackLabel}:*\n${options.rejectionReason}`)
    );
  }
...
export function createNotificationMessage(options: {
  ...
  blocks.push(
    createSectionBlock(`*${options.action}* - ${options.emoji} ${options.target}`)
  );
 ...
 */
export function createOAuthModal(options: {
  integrationType: string;
  oauthUrl: string;
  backButtonMetadata: string;
}): ModalView {
  return {
    type: 'modal',
    title: {
    ...
    blocks: [
      ...
      createActionBlock([
        createButton('Back', `back_to_config_${options.integrationType}`),
      ]),
    ],
  };
}
 ...
 */
export function extractImagesFromMessage(data: {
  blocks?: KnownBlock[];
