...
export function isValidSlackSig(req: Request) {
  ...
  console.log(
    "Body preview:",
    typeof body === "string" ? body.substring(0, 100) : "body is not string"
  );
...
Remember to format your response for Slack (use *bold* for emphasis, not **bold**).
    ...
    if (result.finalOutput) {
      ...
      await sendMessage(response, slackIntegration.access_token, event.channel);
    } else {
      await sendMessage(
