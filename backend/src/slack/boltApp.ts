...
export async function setupSlackBolt() {
  if (!settings.slack.signingSecret) {
    logger.warn(
      "⚠️  SLACK_SIGNING_SECRET not set - Slack Bolt app will not be initialized",
    );
    return null;
  }
  ...
  const slack = new SlackApp({
    ...
    authorize: async ({ teamId, enterpriseId }) => {
      ...
      return {
        ...
        enterpriseId:
          enterpriseId || slackIntegration.enterprise_id || undefined,
      };
    },
  });
    ...
    try {
      ...
      if (
        ...
        if (threadStartedByMention && messageEvent.ts) {
          logger.info(
            "Thread reply in app_mention thread, routing to ChatAgent:",
            {
              threadTs,
  ...
  slack.action(
    /^approval_approve_(.+)__(.+)$/,
    async ({ ack, body, action, respond, client }) => {
      await ack();
      ...
      try {
        ...
        if (!match) {
          logger.error(
            `[Slack Approval] Invalid action_id format: ${actionId}`,
          );
          await respond({
        ...
        logger.info(
          `[Slack Approval] Processing approve for runId: ${runId}, stepId: ${stepId}`,
        );
        ...
        if (!approvalMessage) {
          logger.error(
            `[Slack Approval] No approval message found for runId: ${runId}, stepId: ${stepId}`,
          );
          await respond({
        ...
        if (result.status === "failed" && result.error) {
          logger.error(
            `[Slack Approval] Approval processing failed: ${result.error}`,
          );
        } else {
          logger.info(
            `[Slack Approval] Successfully processed approve for runId: ${runId}, stepId: ${stepId}`,
          );
        }
      } catch (error) {
  ...
  slack.action(
    ...
    async ({ ack, body, action, respond, client }) => {
      ...
      try {
        ...
        const [, runId, stepId] = match;
        logger.info(
          `[Slack Approval] Opening request changes modal for runId: ${runId}, stepId: ${stepId}`,
        );
  ...
  slack.action(
    ...
    async ({ ack, body, action, respond, client }) => {
      ...
      try {
        ...
        const [, runId, stepId] = match;
        logger.info(
          `[Slack Approval] Processing hard reject for runId: ${runId}, stepId: ${stepId}`,
        );
        ...
        if (result.status === "failed" && result.error) {
          logger.error(
            `[Slack Approval] Hard reject processing failed: ${result.error}`,
          );
        } else {
          logger.info(
            `[Slack Approval] Successfully processed hard reject for runId: ${runId}, stepId: ${stepId}`,
          );
        }
      } catch (error) {
  ...
  slack.view(
    ...
    async ({ ack, body, view, client }) => {
      ...
      void (async () => {
        const submitterSlackUserId = (body as any)?.user?.id as
          | string
          | undefined;
        ...
        const notifySubmitter = async (text: string, channelId?: string) => {
          ...
          if (channelId) {
            try {
              ...
              logger.error(
                "[Slack Approval] Failed to post ephemeral message to submitter:",
                { error },
              );
            }
          }
          ...
          try {
            ...
            const dmChannelId = (opened as any)?.channel?.id as
              | string
              | undefined;
        ...
        try {
          ...
          logger.info(
            `[Slack Approval] Processing request changes with feedback for runId: ${runId}, stepId: ${stepId}`,
          );
          ...
          if (!userSlackIntegration) {
            ...
            await notifySubmitter(
              "Error: User integration not found",
          ...
          if (
            ...
            await notifySubmitter(
              "Error: You don't have permission to request changes",
              approvalMessage.slack_channel_id,
            );
            return;
          }
          ...
          if (result.status === "failed" && result.error) {
            logger.error(
              `[Slack Approval] Request changes processing failed: ${result.error}`,
            );
            await notifySubmitter(
              `Error processing request changes: ${result.error}`,
              approvalMessage.slack_channel_id,
            );
          } else {
            logger.info(
              `[Slack Approval] Successfully processed request changes with feedback for runId: ${runId}, stepId: ${stepId}`,
            );
          }
        } catch (error) {
          logger.error(
            "[Slack Approval] Error processing request changes modal submission:",
            { error },
          );
          await notifySubmitter(
            "Error processing request changes. Please try again.",
          );
        }
      })();
    },
    ...
    async ({ ack, body, action, respond, client }) => {
      ...
      try {
        if (!("action_id" in action) || !("value" in action)) {
          logger.error(
            "[Slack Integration Form] Action does not have required properties",
          );
          await respond({
            text: "Error: Invalid action format",
            response_type: "ephemeral",
        ...
        if (!match) {
          logger.error(
            `[Slack Integration Form] Invalid action_id format: ${actionId}`,
          );
          await respond({
            text: "Error: Invalid integration form request",
            response_type: "ephemeral",
          });
          return;
        }
        ...
        try {
          ...
          logger.error("[Slack Integration Form] Error decoding state token", {
            error,
          });
          await respond({
            text: "Error: Invalid request data",
            response_type: "ephemeral",
          });
          return;
        }
        ...
        if (!integrationManager) {
          logger.error("[Slack Integration Form] Integration not found", {
            integrationType,
          });
          await respond({
            text: `Error: Integration ${integrationType} not found`,
            response_type: "ephemeral",
          });
          return;
        }
        ...
        if (!isFormIntegrationInstallation(integrationManager)) {
          logger.error(
            "[Slack Integration Form] Integration does not support form installation",
            { integrationType },
          );
          await respond({
            text: `Error: Integration ${integrationType} does not support form installation`,
            response_type: "ephemeral",
          });
          return;
        }
        ...
        if (formFields.length === 0) {
          logger.error("[Slack Integration Form] No form fields defined", {
            integrationType,
          });
          await respond({
            text: "Error: No form fields defined for this integration",
            response_type: "ephemeral",
          });
          return;
        }
        ...
        if (!triggerId) {
          ...
          await respond({
            text: "Error: Unable to open form",
            response_type: "ephemeral",
          });
          return;
        }
        ...
        logger.error("[Slack Integration Form] Error opening form modal:", {
          error,
        });
        await respond({
          text: "Error opening integration form. Please try again.",
          response_type: "ephemeral",
        });
      }
    },
  ...
  async function processIntegrationFormSubmission(
    ...
    const updateModal = async (newView: ModalView) => {
      ...
      try {
        ...
        logger.error("[Slack Integration Form] Failed to update modal:", {
          error,
        });
      }
    };
    ...
    try {
      ...
      if (!result.success) {
        ...
        logger.error("[Slack Integration Form] Form submission failed", {
          error: errorMsg,
          integrationType,
          userId,
        });
      ...
      if (instances.length === 0) {
        logger.error(
          "[Slack Integration Form] No integration instances found after submission",
      ...
      logger.info("[Slack Integration Form] Form submission successful", {
        integrationType,
        integrationId,
        userId,
      });
      ...
      logger.error(
        "[Slack Integration Form] Error processing form submission:",
        { error, integrationType, userId },
      );
      await updateModal(
        createErrorModal({
          integrationType: integrationType,
  ...
  slack.view("integration_form_submit", async ({ ack, body, view, client }) => {
    ...
    try {
      ...
      logger.error(
        "[Slack Integration Form] Error decoding state token in submission",
        { error },
      );
      await ack({
        response_action: "errors",
    ...
    if (!userId || !integrationType) {
      logger.error(
        "[Slack Integration Form] Missing userId or integrationType in state payload",
      );
      await ack({
        response_action: "errors",
        errors: {
    ...
    if (
      !integrationManager ||
      !isFormIntegrationInstallation(integrationManager)
    ) {
      logger.error(
        "[Slack Integration Form] Integration not found or does not support forms",
        { integrationType },
      );
      await ack({
        response_action: "errors",
    ...
    processIntegrationFormSubmission(
      ...
      logger.error(
        "[Slack Integration Form] Unhandled error in form submission:",
        { error },
      );
    });
  });
  ...
  slack.action(
    ...
    async ({ ack, body, action, respond, client }) => {
      ...
      try {
        if (!("action_id" in action) || !("value" in action)) {
          logger.error(
            "[Slack Integration Config] Action does not have required properties",
          );
          await respond({
            text: "Error: Invalid action format",
            response_type: "ephemeral",
        ...
        if (!match) {
          logger.error(
            `[Slack Integration Config] Invalid action_id format: ${actionId}`,
          );
          await respond({
            text: "Error: Invalid integration config request",
            response_type: "ephemeral",
          });
          return;
        }
        ...
        try {
          ...
          logger.error(
            "[Slack Integration Config] Error decoding state token",
            { error },
          );
          await respond({
            text: "Error: Invalid request data",
        ...
        if (!integrationManager) {
          logger.error("[Slack Integration Config] Integration not found", {
            integrationType,
          });
          await respond({
            text: `Error: Integration ${integrationType} not found`,
        ...
        if (!isOAuthIntegrationInstallation(integrationManager)) {
          logger.error(
            "[Slack Integration Config] Integration does not support OAuth installation",
            { integrationType },
          );
          await respond({
            text: `Error: Integration ${integrationType} does not support OAuth installation`,
            response_type: "ephemeral",
          });
          return;
        }
        ...
        if (configFields.length === 0) {
          logger.error(
            "[Slack Integration Config] No configuration fields defined",
            { integrationType },
          );
          await respond({
            text: "Error: No configuration fields defined for this integration",
            response_type: "ephemeral",
          });
          return;
        }
        ...
        if (!triggerId) {
          ...
          await respond({
            text: "Error: Unable to open configuration",
            response_type: "ephemeral",
          });
          return;
        }
        ...
        logger.error(
          "[Slack Integration Config] Error opening configuration modal:",
          { error },
        );
        await respond({
          text: "Error opening integration configuration. Please try again.",
          response_type: "ephemeral",
        });
      }
    },
  ...
  async function handleOAuthInstallationFromConfigForm(
    ...
    try {
      ...
      logger.error(
        "[Slack Integration Config] Error decoding state token in submission",
        { error },
      );
      return {
        success: false,
        ackResponse: {
          response_action: "errors",
          errors: {
            [Object.keys(view.state.values)[0] || "config"]:
    ...
    if (!userId || !integrationType || !organizationId) {
      logger.error(
        "[Slack Integration Config] Missing userId or integrationType in state payload",
      );
      return {
        success: false,
        ackResponse: {
    ...
    if (
      !integrationManager ||
      !isOAuthIntegrationInstallation(integrationManager)
    ) {
      logger.error(
        "[Slack Integration Config] Integration not found or does not support OAuth",
        { integrationType },
      );
      return {
        success: false,
    ...
    const additionalStatePayload: Record<string, string> | undefined =
      statePayload.chatId && statePayload.channel
        ? {
            ...
            }), // Include messageTs from config button if available
          }
        : undefined;

    const installationDetails = await integrationManager.getInstallationUrl(
      userId,
      organizationId,
      options,
    ...
    const backButtonMetadataToken = jwt.sign(
      backButtonMetadata,
      jwtConfig.secret,
      { expiresIn: "7d" },
    );
  ...
  slack.view(
    ...
    async ({ ack, body, view, client }) => {
      ...
      for (const blockId in stateValues) {
        ...
        for (const actionId in block) {
          ...
          if (
            ...
            "value" in action.selected_option
          ) {
      ...
      const result = await handleOAuthInstallationFromConfigForm(
        view,
        configValues,
      );
  ...
  slack.action(
    ...
    async ({ ack, body, action, client }) => {
      ...
      try {
        if (!("action_id" in action)) {
          logger.error(
            "[Slack Integration Config] Back button action does not have required properties",
          );
          return;
        }
        ...
        if (!match) {
          logger.error(
            `[Slack Integration Config] Invalid back button action_id format: ${actionId}`,
          );
          return;
        }
        ...
        if (!viewId) {
          logger.error(
            "[Slack Integration Config] No view_id in back button body",
          );
          return;
        }
        ...
        if (!privateMetadata) {
          logger.error(
            "[Slack Integration Config] No private_metadata in back button view",
          );
          return;
        }
        ...
        try {
          ...
          logger.error(
            "[Slack Integration Config] Error decoding back button metadata",
            { error },
          );
          return;
        }
        ...
        try {
          ...
          logger.error(
            "[Slack Integration Config] Error decoding state token in back button",
            { error },
          );
          return;
        }
        ...
        logger.error("[Slack Integration Config] Error handling back button:", {
          error,
        });
      }
    },
  ...
  slack.action(
    /^(?!approval_(approve|reject|request_changes)_|open_integration_form_|open_integration_config_|back_to_config_).*$/,
    async ({ ack }) => {
      await ack();
    },
  );
  ...
  slack.event("tokens_revoked", async ({ body }) => {
    try {
      ...
      const tokens = (body as any).tokens as
        | { bot?: string[]; oauth?: string[] }
        | undefined;
  ...
  client: {
    conversations: {
      replies: (args: {
        ...
        ts: string;
        limit: number;
      }) => Promise<{ messages?: Array<{ text?: string }> }>;
    };
  },
  channel: string,
  threadTs: string,
