...
export async function setupSlackBolt() {
    ...
    const slack = new SlackApp({
        ...
        authorize: async ({ teamId, enterpriseId }) => {
            ...
            const botToken = await getSecret({ type: "integration", params: { integrationType: IntegrationType.SLACK, recordId: slackIntegration.id, field: SecretField.AccessToken } })
            if (!botToken) {
    ...
    slack.action(/^open_integration_form_(.+)$/, async ({ ack, body, action, respond, client }) => {
        ...
        try {
            ...
            const integrationManager = INTEGRATION_REGISTRY.find(int => int.integrationType === integrationType)
    ...
    async function processIntegrationFormSubmission(
        ...
        try {
            ...
            integrationFormTaskQueue.emit(new IntegrationFormCompletedTask(integrationType, integrationId, userId, organizationId, statePayload, new Date()))
    ...
    slack.view("integration_form_submit", async ({ ack, body, view, client }) => {
        ...
        processIntegrationFormSubmission(client, viewId, userId, organizationId, integrationType, formValues, stateToken, statePayload, integrationManager).catch(error => {
     ...
     */
    async function handleOAuthInstallationFromConfigForm(view: any, configValues: Record<string, string>): Promise<{ success: boolean; ackResponse?: any; error?: string }> {
        // Extract state token from private metadata (synchronous JWT verify)
        const stateToken = view.private_metadata
        let statePayload: OAuthStatePayload
        try {
        ...
        const installationDetails = await integrationManager.getInstallationUrl(userId, organizationId, options, additionalStatePayload)
