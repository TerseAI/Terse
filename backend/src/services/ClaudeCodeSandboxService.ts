...
export class ClaudeCodeSandboxService {
    ...
    async run(params: ClaudeCodeSandboxParams): Promise<ClaudeCodeSandboxResult> {
        ...
        try {
            ...
            t = performance.now()
            const depsProc = await sb.exec(
                ["sh", "-c", "apt-get update -qq && apt-get install -y -qq git unzip > /dev/null 2>&1"],
            ...
            if (sourceZip) {
                ...
                const unzipProc = await sb.exec(
                    ["sh", "-c", "cd /tmp && unzip -o code.zip -d project > /dev/null"],
                    { stdout: "pipe", stderr: "pipe" }
                )
            ...
            if (gitInit) {
                ...
                const gitProc = await sb.exec(
                    ["sh", "-c", "cd /tmp/project && git init && git add -A && git -c user.name=terse -c user.email=terse@terse.ai commit --allow-empty -m baseline"],
                    { stdout: "pipe", stderr: "pipe" }
                )
                await gitProc.wait()
            ...
            const installProc = await sb.exec(
                ["sh", "-c", "npm install -g @anthropic-ai/claude-code 2>&1"],
                { stdout: "pipe", stderr: "pipe" }
            )
            ...
            const claudeProc = await sb.exec(
                ["sh", "-c", "cd /tmp/project && cat /tmp/prompt.txt | claude -p --output-format json --max-turns " + maxTurns],
                {
                    stdout: "pipe",
                    stderr: "pipe",
                    env: {
            ...
            for (const filePath of outputFilePaths) {
                try {
                    const handle = await sb.open(filePath, "r")
                    outputFiles[filePath] = await handle.readText()
                    await handle.close()
                } catch (error) {
