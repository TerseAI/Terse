...
export async function reviewAllAgents(req: Request, res: Response) {
    ...
    if (dryRun) logger.info("[ReviewAgents] Dry-run mode: computing improvements without persisting or notifying")
    ...
    try {
            ...
            eligible.map(async automation => {
                ...
                logger.info("[ReviewAgents] Computed evaluation", {
                    ...
                    improvements: improvementRecords.map(improvement => ({
                        ...
                        patchPreview: improvement.suggestedPatch?.slice(0, 2000) ?? null
                    }))
                })
        ...
        if (!dryRun) for (const [userId, group] of emailGroups.entries()) {
