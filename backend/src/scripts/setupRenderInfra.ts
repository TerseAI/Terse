/**
 * Idempotent provisioning of the backend runtime infrastructure on Render for one environment:
 * the queue's dedicated Postgres, the ephemeral Redis, and the background worker.
 *
 * The queue migration makes REDIS_URL mandatory (settings.ts) and adds a second entry point
 * (worker.ts) that consumes the pg-boss queues (durable state lives in Postgres via
 * PGBOSS_DATABASE_URL, falling back to DATABASE_URL; Redis carries only ephemeral pub/sub,
 * Socket.IO adapter, and rate-limit keys). This script wires all of it for a target environment
 * so prod can be set up the same way Staging was set up by hand. It is the infra counterpart to
 * migrateFromCloudSchedulerToPgBoss.ts (run that to retire the old GCP crons once this is live).
 *
 * It does five things, each a no-op if already in the desired state:
 ...
 *   2. Ensure a dedicated Render Postgres named <postgres-name> exists for the pg-boss queues,
 *      internal-only, so queue churn (vacuum pressure, connection load) is isolated from the
 *      main application database.
 *   3. Ensure REDIS_URL on the base web service points at the Key Value's internal URL.
 *   4. Ensure PGBOSS_DATABASE_URL on the base web service points at the queue Postgres's
 *      internal URL.
 *   5. Ensure a background worker (<worker-name>) exists, mirroring the web service's build/runtime,
 *      running `pnpm run start:worker`, with the web service's env vars copied over (REDIS_URL and
 *      PGBOSS_DATABASE_URL pinned to the instances above), PR previews on, and auto-deploy on — so
 *      it deploys with every staging/prod push.
 *
 ...
 *   pnpm tsx src/scripts/setupRenderInfra.ts --web-service srv-xxxx              # dry run
 *   pnpm tsx src/scripts/setupRenderInfra.ts --web-service srv-xxxx --apply      # apply
 *
...
const DEFAULT_POSTGRES_PLAN = "basic_256mb"
...
async function main(): Promise<void> {
    ...
    const postgresName = args.postgresName ?? `${web.name.toLowerCase()}-queue-db`
    const workerName = args.workerName ?? `${web.name.toLowerCase()}-worker`
    const banner = args.apply ? "APPLY (mutating)" : "DRY RUN (no changes)"
    console.log(`\n=== Render infra setup — base=${web.name} (${web.environmentId}) — ${banner} ===\n`)

    const redisUrl = await ensureKeyValue(render, web, redisName, args)
    const queueDbUrl = await ensurePostgres(render, web, postgresName, args)
    await ensureWebEnvVar(render, web, "REDIS_URL", redisUrl, args)
    await ensureWebEnvVar(render, web, "PGBOSS_DATABASE_URL", queueDbUrl, args)
    await ensureWorker(render, web, workerName, { REDIS_URL: redisUrl, PGBOSS_DATABASE_URL: queueDbUrl }, args)
...
// ── Step 2: queue Postgres ────────────────────────────────────────────────────

/** Ensure the dedicated pg-boss Postgres exists; returns its internal URL (null in dry-run when it must still be created). */
async function ensurePostgres(render: RenderClient, web: Service, postgresName: string, args: Args): Promise<string | null> {
    const existing = await render.findPostgres(web.ownerId, postgresName, web.environmentId)

    if (existing) {
        console.log(`• Postgres "${postgresName}" exists (${existing.id}, status=${existing.status}, version=${existing.version})`)
        return (await render.getPostgresConnection(existing.id)).internalConnectionString
    }

    if (!args.apply) {
        console.log(`• [dry-run] would create Postgres "${postgresName}" (plan=${args.postgresPlan}, version=${args.postgresVersion}, region=${web.serviceDetails.region}, internal-only)`)
        return null
    }

    const created = await render.createPostgres({
        name: postgresName,
        ownerId: web.ownerId,
        environmentId: web.environmentId,
        plan: args.postgresPlan,
        region: web.serviceDetails.region,
        version: args.postgresVersion,
        ipAllowList: []
    })
    console.log(`• ✓ created Postgres "${postgresName}" (${created.id})`)
    await render.waitForPostgresAvailable(created.id)
    const url = (await render.getPostgresConnection(created.id)).internalConnectionString
    console.log(`  ✓ available`)
    return url
}

// ── Steps 3 & 4: env vars on the web service ──────────────────────────────────

async function ensureWebEnvVar(render: RenderClient, web: Service, key: string, url: string | null, args: Args): Promise<void> {
    const vars = await render.listEnvVars(web.id)
    const current = vars.find(v => v.key === key)?.value ?? null

    if (url && current === url) {
        console.log(`• ${key} on ${web.name} already correct`)
        return
    }
    if (!args.apply) {
        console.log(`• [dry-run] would set ${key} on ${web.name} to ${url ?? "the new instance's internal URL"}`)
        return
    }
    if (!url) throw new SetupError(`internal URL unavailable; cannot set ${key}`)
    await render.upsertEnvVar(web.id, key, url)
    console.log(`• ✓ set ${key} on ${web.name}`)
}

// ── Step 5: background worker ─────────────────────────────────────────────────

async function ensureWorker(render: RenderClient, web: Service, workerName: string, pinnedEnvVars: PinnedEnvVars, args: Args): Promise<void> {
    ...
    if (existing) {
        ...
        await reconcileExistingWorker(render, existing, pinnedEnvVars, args)
        return
    }
    ...
    if (!args.apply) {
        ...
        console.log(`  [dry-run] would copy env vars from ${web.name} and set ${Object.keys(pinnedEnvVars).join(", ")}`)
        return
    }

    const envVars = await buildWorkerEnvVars(render, web.id, resolvePinnedEnvVars(pinnedEnvVars))
...
async function reconcileExistingWorker(render: RenderClient, worker: Service, pinnedEnvVars: PinnedEnvVars, args: Args): Promise<void> {
    ...
    for (const [key, url] of Object.entries(pinnedEnvVars)) {
        const current = vars.find(v => v.key === key)?.value ?? null
        if (!url || current === url) continue
        if (args.apply) {
            await render.upsertEnvVar(worker.id, key, url)
            console.log(`  ✓ set ${key}`)
        } else {
...
/** All of the web service's env vars (non-null), with the queue infra URLs pinned to the instances provisioned above. */
async function buildWorkerEnvVars(render: RenderClient, webServiceId: string, pinned: EnvVarInput[]): Promise<EnvVarInput[]> {
    const pinnedKeys = new Set(pinned.map(v => v.key))
    const copied = (await render.listEnvVars(webServiceId)).filter(v => v.value !== null).map(v => ({ key: v.key, value: v.value as string }))
    const withoutPinned = copied.filter(v => !pinnedKeys.has(v.key))
    return [...withoutPinned, ...pinned]
}

function resolvePinnedEnvVars(pinnedEnvVars: PinnedEnvVars): EnvVarInput[] {
    return Object.entries(pinnedEnvVars).map(([key, url]) => {
        if (!url) throw new SetupError(`internal URL unavailable; cannot create worker with ${key}`)
...
class RenderClient {
    ...
    async getKeyValueConnection(id: string): Promise<ConnectionInfo> {
        return connectionInfoSchema.parse(await this.request("GET", `/key-value/${id}/connection-info`))
    }
    ...
    async findPostgres(ownerId: string, name: string, environmentId: string): Promise<Postgres | null> {
        const items = postgresListSchema.parse(await this.request("GET", `/postgres?ownerId=${ownerId}&limit=100`))
        return items.map(i => i.postgres).find(pg => pg.name === name && pg.environmentId === environmentId) ?? null
    }

    async createPostgres(input: CreatePostgresInput): Promise<Postgres> {
        return postgresSchema.parse(await this.request("POST", "/postgres", input))
    }

    async getPostgres(id: string): Promise<Postgres> {
        return postgresSchema.parse(await this.request("GET", `/postgres/${id}`))
    }

    async getPostgresConnection(id: string): Promise<ConnectionInfo> {
        return connectionInfoSchema.parse(await this.request("GET", `/postgres/${id}/connection-info`))
    }

    async waitForPostgresAvailable(id: string, attempts = 60, intervalMs = 5000): Promise<void> {
        for (let i = 0; i < attempts; i++) {
            const pg = await this.getPostgres(id)
            if (pg.status !== "creating") return
            await sleep(intervalMs)
        }
        throw new SetupError(`Postgres ${id} did not become available in time`)
    }
...
function parseArgs(argv: string[]): Args {
    const args: Mutable<Args> = {
        webServiceId: "",
        apply: false,
        redisPlan: DEFAULT_REDIS_PLAN,
        postgresPlan: DEFAULT_POSTGRES_PLAN,
        postgresVersion: DEFAULT_POSTGRES_VERSION,
        maxShutdownDelaySeconds: DEFAULT_MAX_SHUTDOWN_DELAY_SECONDS
    }
    for (let i = 0; i < argv.length; i++) {
        ...
        else if (flag === "--postgres-name") args.postgresName = value()
        else if (flag === "--worker-name") args.workerName = value()
        else if (flag === "--redis-plan") args.redisPlan = value()
        else if (flag === "--postgres-plan") args.postgresPlan = value()
        else if (flag === "--postgres-version") args.postgresVersion = value()
...
const postgresListSchema = z.array(z.object({ postgres: postgresSchema }))
const connectionInfoSchema = z.object({ internalConnectionString: z.string() })

type Service = z.infer<typeof serviceSchema>
type EnvVar = z.infer<typeof envVarSchema>
type KeyValue = z.infer<typeof keyValueSchema>
type Postgres = z.infer<typeof postgresSchema>
type ConnectionInfo = z.infer<typeof connectionInfoSchema>

type Mutable<T> = { -readonly [K in keyof T]: T[K] }
type PinnedEnvVars = Record<string, string | null>

interface Args {
    readonly webServiceId: string
    readonly apply: boolean
    readonly redisPlan: string
    readonly postgresPlan: string
...
main()
    .then(() => process.exit(0))
    .catch(error => {
        console.error("Render infra setup failed:", error instanceof Error ? error.message : error)
        process.exit(1)
    })
