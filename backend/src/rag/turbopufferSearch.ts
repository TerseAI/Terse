...
import { NamespaceToHydratorType, RAGNamespace, requireHydratorType } from "../types/rag"
...
import { DistanceMetric, EmbeddingError, EmbeddingProvider, RankingFunction, Search, SearchError, SearchItem, SearchOptions } from "./searchTypes"
...
export class TurboPufferSearch<M, N extends RAGNamespace = RAGNamespace> implements Search<NamespaceToHydratorType[N], M> {
    ...
    constructor(embeddingClient: EmbeddingProvider, namespace: N, userId: string, distanceMetric: DistanceMetric = "cosine_distance") {
    ...
    async search(query: string, options?: SearchOptions): Promise<NamespaceToHydratorType[N][]> {
        const { rankingColumn = "vector", rankingFunction = RankingFunction.ANN, filters, includeAttributes, topK } = options || {}

        const finalIncludeAttributes = ["entityType", "entityId", ...(includeAttributes || [])]
    ...
    async bulkInsert(items: SearchItem<M>[]): Promise<void> {
        ...
        if (invalidItem) {
            ...
            throw new SearchError(`SearchItem with id ${invalidItem.id} must have ${missingField} field for bulkInsert operation`)
        }

        try {
            // Generate embeddings in batch if possible, otherwise in parallel
            const contents = items.map(item => item.content!)
            const embeddings = this.embeddingClient.embedBatch ? await this.embeddingClient.embedBatch(contents) : await Promise.all(contents.map(content => this.embed(content)))
