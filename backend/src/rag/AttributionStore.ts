...
import { HydratorType, HydratorTypeMap, parseHydratorType } from "../types/rag"
...
export class AttributionStore {
    ...
    private extractUniqueIdentifiables(attributions: AttributionWithRef[]): Identifiable[] {
        ...
        // Remove duplicates
        return Array.from(new Map(identifiableRefs.map(ref => [`${ref.entityType}:${ref.entityId}`, ref])).values())
    }
    ...
    private async hydrateEvents(identifiableRefs: Identifiable[]): Promise<HydratedEvent[]> {
        ...
        const groupedByType = identifiableRefs.reduce(
            (acc, ref) => {
    ...
    private sortEventsByTimestamp(events: HydratedEvent[], timestampMap: Map<string, Date>): HydratedEvent[] {
