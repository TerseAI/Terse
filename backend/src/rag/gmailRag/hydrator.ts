...
import { GmailTriggerEventRuntime, fetchAndParseEmail, getOAuth2Client } from "../../integrations/GmailIntegration"
...
export class GmailEventHydrator extends Hydrator<GmailTriggerEventRuntime> {
    ...
    async hydrate(ref: Identifiable): Promise<GmailTriggerEventRuntime> {
    ...
    async hydrateBulk(refs: Identifiable[]): Promise<GmailTriggerEventRuntime[]> {
    ...
    private async fetchFromGmail(entityId: string): Promise<GmailTriggerEventRuntime | null> {
        ...
        try {
            ...
            return new GmailTriggerEventRuntime(parsed, integrationId)
        } catch (error) {
