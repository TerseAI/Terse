...
export function SdkJobListItem({ agent }: SdkJobListItemProps) {
    ...
    const isActive = location.pathname === buildRoute(FrontendRoutes.JOBS.BY_ID, { id: agent.id })

    return (
        <SidebarMenuSubItem>
            <SidebarMenuSubButton asChild isActive={isActive}>
                <Link to={buildRoute(FrontendRoutes.JOBS.BY_ID, { id: agent.id })} className="flex items-center gap-2">
