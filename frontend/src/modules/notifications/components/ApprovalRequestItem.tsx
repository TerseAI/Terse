...
import RunHistoryStatusBadge from "@/modules/runHistory/components/RunHistoryStatusBadge"
                ...
                <div className="ml-auto flex flex-shrink-0 items-center gap-2">
                    {orderedActions.map(action =>
                        action.type === "open_run_history" ? (
                            <Button key={`${approval.id}-${action.deepLink}`} type="button" size="icon-sm" variant="outline" aria-label="Open run history" onClick={() => onAction(action.deepLink)} title="Open run history">
