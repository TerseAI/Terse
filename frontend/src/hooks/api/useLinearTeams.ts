...
export function useLinearTeams(integrationId: string | null | undefined): UseLinearTeamsReturn {
    const { data, error, isLoading, isValidating, mutate } = useSWR<LinearTeam[]>(
        linearTeamsKey(integrationId),
        integrationId ? () => BackendProvider.getLinearTeams(integrationId) : null,
        {
