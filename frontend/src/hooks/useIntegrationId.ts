...
export function useIntegrationId(
    currentConfig: ConfigInstance | undefined,
    expectedConfigTypes: ConfigType | ConfigType[]
): [string | undefined, (value: string | undefined) => void] {
    // Normalize expectedConfigTypes to an array
    const expectedTypes = Array.isArray(expectedConfigTypes) 
        ? expectedConfigTypes 
        : [expectedConfigTypes];

    // Check if currentConfig matches one of the expected config types
    const isValidConfig = currentConfig 
        ? expectedTypes.includes(currentConfig.configType)
        : false;
    ...
    const validIntegrationId = isValidConfig && currentConfig
