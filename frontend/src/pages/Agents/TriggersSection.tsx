...
export const InputsSection = forwardRef<Map<string, HTMLDivElement>, InputsSectionProps>(({ inputs, setInputs, isLoading }, ref) => {
    ...
    const handleSelectIntegration = (integrationId: string, input: TransientAgentTrigger) => {
    ...
    return (
        ...
        >
            {!inputs.length ? (
                <EmptyTriggersSection onCreateNew={() => setShowAddModal(true)} />
            ) : (
                <InputCardsLayout 
                    inputs={inputs} 
...
function EmptyTriggersSection({ onCreateNew }: { onCreateNew: () => void }) {
    return (
        <Empty>
            <EmptyHeader>
