...
export function GitHubKnowledgeBaseIntegration({ knowledgeBase, variant, setConfig }: KnowledgeBaseSelectorProps) {
    ...
    if (variant === 'card') {
        ...
        const displayText = hasRepos
            ? `${githubConfig.repositoryIds.length} repo${githubConfig.repositoryIds.length !== 1 ? 's' : ''}`
            : (selectedIntegration ? 'Select repos' : 'Select integration');
        return (
    ...
    const updateIntegrationId = (integrationId: string) => {
        ...
        const newConfig = new GitHubKBConfig(
            integrationId,
            [],
            []
        );
    ...
    return (
        <div className="space-y-4">
            ...
            {selectedIntegrationId && (
                <div className="space-y-2">
                    <Label>Repositories <span className="text-destructive">*</span></Label>
                    <GithubResourceSelector
                        installationId={installationId}
                        selectedRepositoryIds={githubConfig.repositoryIds}
                        onSelect={updateRepositories}
                    />
                    {githubConfig.repositoryIds.length === 0 && (
