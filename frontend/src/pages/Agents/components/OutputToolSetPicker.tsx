...
export function OutputToolSetPicker({ onSelectIntegration }: { onSelectIntegration: (configType: ConfigType) => void }) {
    ...
    const filteredConfigTypes = useMemo(() => {
        ...
        return allConfigTypes.filter(config => {
            return config.name.toLowerCase().includes(query) || config.description.toLowerCase().includes(query)
        })
                ...
                {filteredConfigTypes.map(config => (
                    ...
                    >
                        ...
                        <div className="w-full flex flex-row justify-between items-center gap-2">
                            <div className="min-w-0 flex-1 text-sm font-medium leading-none truncate text-left">{config.name}</div>
                        </div>
                        ...
                        <div className="mt-auto text-xs text-muted-foreground text-center leading-snug line-clamp-2">{config.description}</div>
                    </button>
                ))}
            </div>
