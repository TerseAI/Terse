...
function Integrations() {
  ...
  const sourceControlIntegrations = integrations.filter(
    (integration) => integration === Integration.GITHUB
  );
  const issueTrackingIntegrations = integrations.filter(
    (integration) => integration === Integration.JIRA || integration === Integration.LINEAR
  );
  ...
  const noteTakingIntegrations = integrations.filter(
    (integration) => integration === Integration.NOTION
  );
  ...
  return (
    <div className="grid grid-cols-1 grid-rows-1 pt-4 pb-4">
      ...
      <div className="flex flex-col gap-8">
        <IntegrationSection
          integrations={sourceControlIntegrations}
          title="Source Control"
          fallback={<GithubIntegration />}
          isLoading={isLoading}
        />
        <IntegrationSection
        ...
        />
        <IntegrationSection
          integrations={noteTakingIntegrations}
          title="Note Taking"
          fallback={<AddNotion onIntegrationChange={refreshIntegrations} />}
          isLoading={isLoading}
        />
        <IntegrationSection
