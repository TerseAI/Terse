...
import { getDefaultChannelName, toChannelInput, toChannelOutputs, toChannelKnowledgeBase } from "@/utility/ChannelUtils";
...
export default function ChannelSetupTab({
    ...
    const channelOutputs = toChannelOutputs(outputs);
...
function OutputLayout({ outputs, setOutputs, isIncomplete }: { outputs: TransientChannelOutput[], setOutputs: (outputs: TransientChannelOutput[]) => void, isIncomplete: boolean }) {
    ...
    const handleSelectPlatform = (configType: ConfigType) => {
        const newOutputId = uuidv4(); // We need to mint a placeholder ID for the new output so that we can identify it later.
        const newOutput: TransientChannelOutput = { id: newOutputId, config: undefined, configType: configType };
        const newOutputs: TransientChannelOutput[] = [...outputs, newOutput];
    ...
    return (
        <div className="flex flex-col gap-3">
            <div className="flex flex-row gap-2 items-center mb-2">
                ...
                <SectionInfoIcon
                    isIncomplete={isIncomplete}
                    alertMessage="Add at least one skill destination and complete its configuration to remove this warning."
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(10rem,1fr))] gap-4 items-stretch">
                {outputs.map((output) => (
                    <Output key={output.id} output={output} outputs={outputs} setOutputs={setOutputs} handleRemove={handleRemove} />
                ))}
                <Button variant="outline" onClick={() => setShowAddModal(true)} className="w-full aspect-square h-auto">
...
function Output({ output, outputs, setOutputs, handleRemove }: { output: TransientChannelOutput, outputs: TransientChannelOutput[], setOutputs: (outputs: TransientChannelOutput[]) => void, handleRemove: (id: string) => void }) {
    ...
    const onSelect = (config: ConfigInstance) => {
        setOutputs(outputs.map(o => o.id === output.id ? { ...o, config, configType: config.configType } : o));
    };
                    ...
                    <div className="flex flex-col gap-4">
                        {outputUrl && (
                            ...
                            >
                                ...
                                Open in {CONFIG_DETAILS[output.configType].name}
                            </a>
                        )}
                        {needsConfiguration && (
                            <Badge variant="outline" className="border-yellow-500 text-yellow-600 dark:text-yellow-500 w-fit">
                                Needs Configuration
                            </Badge>
                        )}
                        <IntegrationSelector {...selectorProps} variant="dialog" />
