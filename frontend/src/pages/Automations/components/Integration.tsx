...
export function IntegrationBox({ children, isOutput }: { children: React.ReactNode, isOutput?: boolean }) {
    ...
    return (
        <div
            className={`w-${boxSize} h-${boxSize} flex items-center justify-center p-4 rounded-lg bg-[theme(background-light)] border border-[theme(border)] transition-all duration-200 hover:border-[theme(--color-accent)] hover:shadow-lg ${isOutput ? 'animate-breathe' : ''}`}
            style={isOutput ? {
