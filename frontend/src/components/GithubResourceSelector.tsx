...
export function GithubResourceSelector({ installationId, selectedRepositoryIds = [], onSelect, hideLabel = false, customLabel }: GithubResourceSelectorProps) {
    ...
    if (!installationId) {
        return <div className="text-sm text-muted-foreground">Please select a GitHub connection to view repositories.</div>
    }
    ...
    if (repositories.length === 0) {
        return <div className="text-sm text-muted-foreground">No GitHub repositories found. Make sure your GitHub integration has access to repositories.</div>
    }
        ...
        <div className="space-y-2">
            {(!hideLabel || customLabel) && (
                <div className="flex items-center justify-between">
                    ...
                    <Button onClick={handleRefresh} disabled={isRefreshing || isValidating} variant="ghost" size="sm" title="Refresh repository list">
            ...
            <MultiSelect
                ...
                displayText={count => (count > 0 ? `${count} repositor${count !== 1 ? "ies" : "y"} selected` : "Select repositories...")}
            />
