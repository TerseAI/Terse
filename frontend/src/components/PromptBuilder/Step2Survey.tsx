...
export function Step2Survey({ questions, answers, writeInAnswers, onAnswerChange, onWriteInChange, isLoading }: Step2SurveyProps) {
    if (isLoading) {
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-muted-foreground mb-4">Please answer all questions below</p>
            </div>
            ...
            <div className="space-y-8">
                {questions.map((question, idx) => {
                    ...
                    const selectedAnswers = isMultiple ? (Array.isArray(currentAnswer) ? currentAnswer : []) : typeof currentAnswer === "string" ? currentAnswer : undefined
                    ...
                    return (
                        <div key={idx} className="space-y-3 border-b pb-6 last:border-b-0">
                            <Label className="text-sm font-medium">
                                ...
                                {isMultiple && <span className="text-xs text-muted-foreground ml-2">(Select all that apply)</span>}
                            </Label>
                                ...
                                {isMultiple ? (
                                    (["a", "b", "c", "d", "e"] as const).map(option => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <Checkbox id={optionId} checked={isChecked} />
                                    ...
                                    <RadioGroup value={radioValue} onValueChange={(value: string) => onAnswerChange(idx, value, question.type)}>
                                        {(["a", "b", "c", "d", "e"] as const).map(option => {
                                            const optionId = `question-${idx}-${option}`
                                            return (
                                                <div key={option} className="flex items-center gap-3 hover:bg-accent p-2 rounded-md cursor-pointer">
                            ...
                            {question.allowWriteIn && (
                                <div className="pl-4 mt-3">
                                    <Label className="text-sm font-medium mb-2 block">Or provide your own answer:</Label>
                                    <Textarea
                                        value={writeInValue}
