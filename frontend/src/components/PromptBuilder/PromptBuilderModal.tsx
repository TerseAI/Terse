...
import { GenerateSurveyPromptRequest, GenerateSurveyQuestionsRequest, SKIP_OPTION, SurveyAnswers, SurveyConfigContext, SurveyQuestion, SurveyWriteInAnswers } from "@/shared/PromptBuilderTypes"
...
export function PromptBuilderModal({ isOpen, onClose, inputs, outputs, knowledgeBases, existingPrompt, onPromptGenerated }: PromptBuilderModalProps) {
    ...
    const prepareConfigContext = (): { inputConfigs?: SurveyConfigContext[]; outputConfigs?: SurveyConfigContext[]; knowledgeBaseConfigs?: SurveyConfigContext[] } => {
        ...
        const knowledgeBaseConfigs: SurveyConfigContext[] = (knowledgeBases || []).map(kb => ({
            type: kb.config.configType
        }))

        return {
            inputConfigs,
            outputConfigs: outputConfigs.length > 0 ? outputConfigs : undefined,
            knowledgeBaseConfigs: knowledgeBaseConfigs.length > 0 ? knowledgeBaseConfigs : undefined
        }
    }
    ...
    const areAllQuestionsAnswered = () => {
        ...
        return questions.every((_, idx) => {
            ...
            const hasAnswer = answer !== undefined && answer !== null && answer !== "" && (Array.isArray(answer) ? answer.length > 0 : true)
    ...
    const handleGenerateQuestions = async () => {
        ...
        try {
            const { inputConfigs, outputConfigs, knowledgeBaseConfigs } = prepareConfigContext()
            ...
            <DialogContent className="w-[50%] max-w-none sm:max-w-none max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        ...
                        Prompt Builder
                        <span className="text-sm font-normal text-muted-foreground">Step {currentStep} of 3</span>
                    </DialogTitle>
                </DialogHeader>
                ...
                <div>
                    ...
                    >
                        <Step>
                            <Step1Description description={description} setDescription={setDescription} isLoading={isLoadingQuestions} />
                        </Step>
                        <Step>
                            <Step2Survey
                                questions={questions}
                        ...
                        <Step>
                            <Step3Review generatedPrompt={generatedPrompt} isLoading={isLoadingPrompt} onRestart={handleRestart} onDone={handleDone} />
                        </Step>
                    </Stepper>
                </div>
            </DialogContent>
