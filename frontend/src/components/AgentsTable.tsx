...
import { useAgentMutations, useAgents } from "@/hooks/api/useAgents"

import { Agent } from "../shared/types"

import { ActionButtons, AppsList, EmptyState, LoadingState, PaginationControls, StatusToggle, TableContent } from "./Agents"
...
export function AgentsTable({ onEdit, onDelete, onCreateNew, searchQuery, statusFilter }: AgentsTableProps) {
    ...
    const columns: ColumnDef<Agent>[] = [
        {
            ...
            header: "Status",
            cell: ({ row }) => <StatusToggle agent={row.original} onToggle={handleToggleStatus} />
        },
        {
            accessorKey: "name",
            header: "Name",
        ...
        },
        {
            id: "actions",
            header: "Actions",
            cell: ({ row }) => <ActionButtons agent={row.original} onEdit={onEdit} onDelete={onDelete} />
        }
    ]
    ...
    return (
        <div className="space-y-4">
            ...
            <PaginationControls page={page} totalPages={totalPages} total={total} limit={limit} onPageChange={setPage} onLimitChange={handleLimitChange} />
        </div>
    )
}
