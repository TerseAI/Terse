...
export function JiraIntegration({ input, variant, setConfig }: InputConfigSelectorProps) {
    ...
    const { projects, isLoading: isLoadingProjects } = useJiraResources(selectedIntegrationId || null)
    ...
    function onSelectProject(projectKey: string, projectId: string) {
        if (selectedIntegrationId) {
            const jiraConfig = new JiraConfig(selectedIntegrationId, projectKey === "__none__" ? undefined : projectKey, projectId === "__none__" ? undefined : projectId)
            setConfig(jiraConfig)
        }
    }
    ...
    if (integrations.length === 0) {
        ...
        return (
            <div className="flex flex-col items-center gap-3 p-4 rounded-lg border border-dashed border-input bg-card text-center">
                <div className="text-sm text-muted-foreground">No Jira accounts connected</div>
    ...
    if (variant === "card") {
        ...
        const projectDisplay = currentConfig?.projectKey ? ` - ${currentConfig.projectKey}` : " - All Projects"
        return <div className="text-sm">{selectedOption ? `${selectedOption.label}${projectDisplay}` : "No connection selected"}</div>
    }
        ...
        <div className="flex flex-col gap-3">
            ...
            {selectedIntegrationId && (
                <div className="flex flex-col gap-1.5 mt-2 pt-3 border-t border-border">
                    ...
                    <p className="text-xs text-muted-foreground">Select a specific project to filter events, or leave as "All projects" to listen to all projects</p>
                </div>
            )}
        </div>
    )
