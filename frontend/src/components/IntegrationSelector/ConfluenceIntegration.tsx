...
export function ConfluenceIntegration({ input, variant, setConfig }: InputConfigSelectorProps) {
    ...
    if (integrations.length === 0) {
        ...
        return (
            <div className="flex flex-col items-center gap-3 p-4 rounded-lg border border-dashed border-input bg-card text-center">
                <div className="text-sm text-muted-foreground">No Atlassian accounts connected</div>
                <div className="text-xs text-muted-foreground">Confluence uses the same credentials as Jira. If you have a Jira connection, it will be available here.</div>
        ...
        <div className="flex flex-col gap-3 min-w-0 overflow-hidden">
            ...
            {selectedIntegrationId && (
                <div className="mt-3 pt-3 border-t border-border">
                    {!currentConfig?.pageId && <p className="text-sm text-muted-foreground mb-3">Select a page to continue</p>}
                    <ConfluenceResourceSelector
                        ...
                        onSelect={(resourceId, resourceTitle, spaceId, spaceName) => {
                            const updatedConfig = new ConfluenceConfig(selectedIntegrationId, spaceName, spaceId, resourceId, resourceTitle)
