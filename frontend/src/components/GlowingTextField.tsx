...
function GlowingTextField({
  ...
  autoFocus = false,
  focusOverride = null,
}: GlowingTextFieldProps) {
  const [currentPlaceholder, setCurrentPlaceholder] = useState<string | undefined>(
    placeholders ? placeholders[0] : undefined
  );
      ...
      {isLoading && (
        <div className="absolute inset-0 pointer-events-none overflow-visible">
          <div className="absolute left-1/2 top-1/2 w-full h-full animate-rect-orbit overflow-visible">
            <div
              className={`absolute ${compact ? "w-2.5 h-2.5" : "w-3 h-3"} rounded-full bg-purple-500/60 blur-sm shadow-[0_0_10px_rgba(168,85,247,0.6)] -translate-x-1/2 -translate-y-1/2 overflow-visible`}
            />
          </div>
        </div>
      )}
