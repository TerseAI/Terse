...
import { Calendar, CalendarDays, Check, Clock, Repeat } from "lucide-react"
        ...
        <div className={cn("space-y-5", className)}>
            ...
            {frequency && (
                <div className="flex flex-wrap items-center gap-3">
                    {frequency === "weekly" && <span className="text-sm text-muted-foreground">at</span>}
                    ...
                    {frequency === "daily" && <span className="text-sm text-muted-foreground">Every day at</span>}
                    {frequency === "monthly" && <span className="text-sm text-muted-foreground">at</span>}
                ...
                {nextRuns.length > 0 && (
                    <div className="pl-10">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {nextRuns.map((run, i) => (
                                <span key={i} className="text-xs px-2.5 py-1 rounded-md bg-background/60 text-muted-foreground border border-border/30">
            ...
            {/* Save Button */}
            <Button onClick={handleSave} disabled={!isConfigured || !hasChanges} className="w-full">
...
export function getCronDescription(cron: string): string {
    ...
    return getHumanReadable(state.frequency, state.hour, state.minute, state.daysOfWeek, state.dayOfMonth)
}
...
function buildCron(frequency: Frequency, hour: string, minute: string, daysOfWeek: string[], dayOfMonth: string): string {
...
function getHumanReadable(frequency: Frequency, hour: string, minute: string, daysOfWeek: string[], dayOfMonth: string): string {
    ...
    switch (frequency) {
        ...
        case "weekly":
            const sortedDays = [...daysOfWeek].sort((a, b) => parseInt(a) - parseInt(b))
            const dayNames = sortedDays.map(d => DAYS_OF_WEEK.find(day => day.value === d)?.label).filter(Boolean)
