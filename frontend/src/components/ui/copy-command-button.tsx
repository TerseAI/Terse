...
export function CopyCommandButton({ command, title, disabled, className }: CopyCommandButtonProps) {
    ...
    return (
        <Tooltip>
            <TooltipTrigger asChild>
                <button
                    ...
                    className={cn(
                        ...
                        copied ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400" : "border-border/70 bg-muted/40 text-foreground hover:border-border hover:bg-muted",
                        className
                    )}
                >
            ...
            <TooltipContent>{copied ? "Copied. Paste into your project terminal" : (title ?? "Copy. Then run in your project's terminal")}</TooltipContent>
        </Tooltip>
    )
}
