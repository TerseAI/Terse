...
export function AddToSlack() {
  ...
  const connectButton = (
    ...
    >
      {isLoading ? "Opening Slack..." : isPolling ? "Checking connection..." : "Connect to Slack"}
    </button>
  );
  ...
  return (
    <IntegrationCard
      ...
      connectionInfo={
        isPolling ? "Checking connection..." : hasSlack ? "Slack connected" : undefined
      }
      connectButton={connectButton}
      icon={
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 122.8 122.8">
          <path
